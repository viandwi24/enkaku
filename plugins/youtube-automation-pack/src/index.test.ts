...
describe('youtube-automation-pack manifest', () => {
  ...
  test('version matches package.json', async () => {
    ...
    expect(plugin.version).toBe('0.19.0')
    expect(plugin.version).toBe(pkg.version)
  })

  test('every script has a unique id and declares params and result schemas', () => {
    const ids = plugin.scripts.map((s) => s.id)
    expect(ids).toEqual(['search-channel', 'scroll-shorts', 'scroll-live', 'download-home', 'search-play', 'watch-video'])
  ...
  test('every member is presentable in Studio, checked against the real list', () => {
    const members: Array<{ id: string; title?: string; description?: string }> = [searchChannel, scrollShorts, scrollLive, downloadHome, searchPlay, watchVideo]
