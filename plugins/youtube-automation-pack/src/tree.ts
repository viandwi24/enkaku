 ...
 * reaches first.
 *
 * A YouTube search-results page is a `RecyclerView` of near-identical result
 * rows, which is exactly the shape that goes wrong: every row shares the same
 * ids, so `find()` silently returns row 0 and a script that meant "the channel
 * result" opens whatever happened to be first. `find()`/`waitFor()` are safe
 * here only for a selector already known to be unique; everything else dumps
 * once and walks the tree itself.
 *
