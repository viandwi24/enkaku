...
describe('the platform registry', () => {
  ...
  test('Instagram is postable, through the member its hardware walk produced', () => {
    expect(platformById('instagram')?.script).toBe('instagram/post-video@latest')
    expect(platformById('instagram')?.unsupportedReason).toBeNull()
  })

  test('postablePlatforms is exactly the rows with a script', () => {
    expect(postablePlatforms().map((p) => p.id)).toEqual(['tiktok', 'instagram', 'youtube'])
  })
