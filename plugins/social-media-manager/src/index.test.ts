...
describe('social-media-manager manifest', () => {
  ...
  test('version matches package.json', async () => {
    ...
    expect(plugin.version).toBe('0.63.0')
    expect(plugin.version).toBe(pkg.version)
  })

  test('every member is presentable in Studio', () => {
    expect(plugin.scripts.map((s) => s.id)).toEqual(['add-post', 'retry-failed', 'add-posts', 'add-group', 'recap-videos', 'start-group', 'retry-group', 'update-post', 'resolve-attempt', 'skip-platform', 'update-group', 'clean-phone-videos', 'sync-accounts', 'add-warmup', 'run-warmup'])
    ...
    const members: Array<{ id: string; title?: string; description?: string }> = [addPost, retryFailed, addPosts, addGroup, recapVideos, startGroup, retryGroup, updatePost, resolveAttempt, skipPlatform, updateGroup, cleanPhoneVideos, syncAccounts, addWarmup, runWarmup]
  ...
  test('every member parameter description fits the farm\'s 300-character limit', () => {
    ...
    for (const member of [addPost, retryFailed, addPosts, addGroup, startGroup, retryGroup, updatePost, resolveAttempt, skipPlatform, updateGroup, cleanPhoneVideos, syncAccounts, addWarmup]) {
...
describe('the surface', () => {
  ...
  /*
    ONE entry again (0.56.0). 0.53.0 split warm-up out on the argument that it
    is a different JOB from posting; the owner looked at the result and said to
    merge them — one Social Media Manager page, warm-up as a tab. The rule that
    survives is one entry per PRODUCT, not one per job.

    This test is the guard for it: a future change that gives warm-up (or
    captions, or accounts) its own menu entry has to come here and argue with
    the paragraph above first.
  */
  test('one entry, for the whole product', () => {
    expect(surface.nav).toHaveLength(1)
    expect(Object.keys(surface.views)).toEqual(['posts'])
    expect(surface.nav[0]?.view).toBe('posts')
    expect(surface.nav[0]?.label).toBe('Social Media Manager')
  })
