...
> **Status:** working end to end in software, and verified on Android 15 hardware for the original route facet: the agent installs unattended, the control channel answers, and a SOCKS5 full tunnel with username/password carries real traffic — confirmed by Android's own `IS_VALIDATED` connectivity check passing *through* the tunnel. Plan 90's three newer facets (mandatory provisioning, the keyboard, the screen label) and plan 221's two newest (`ui-tree`, `activity`, plus the keyboard-with-hardware preference and the complete status screen) are implemented and unit-tested but **not yet confirmed on real hardware** by their respective passes — see `docs/plans/90-m55-unified-guest-agent.md`'s consolidated hardware-pending table and `docs/plans/221-mvp-guest-agent.md` §7.4 for the exact commands. Known rough edges on the route facet are in [`docs/plans/44-m15v-proxy-end-to-end.md`](../../docs/plans/44-m15v-proxy-end-to-end.md) §8b.

## One app, six facets — and the rule that decides what goes in it
...
| **Inspector (`ui-tree`)** | `ui-tree` | `AccessibilityNodeInfo` has no shell equivalent that survives without `am instrument` — which is exactly the instrumentation process this facet replaces (plan 221 §4.2) |
| **Activity** | `activity` | The phone's own screen has to say what the farm is doing to it, and only an app can draw it (plan 221 §4.5) |
...
> Tier 3 is wired and pinned (plan 90 §90.1, plan 221 §4.12, closing plan 43 §5.5/§5.11): `packages/toolchain/manifest/enkaku-tools.json` carries a real `guest-agent` entry — `swappable: false`, like `scrcpy-server`, since the wire contract is shared with the core — and `.github/workflows/release.yml` builds, signs and publishes the APK on a `v*` tag, then runs `scripts/pin-guest-agent.ts` to write that same run's `version`/`url`/`sha256`/`sizeBytes`/`deviceArtifact.versionCode` into the manifest and commits the result to the default branch. **The manifest still lags one release behind the agent build it pins** — the APK has to exist before it can be checksummed, so the pin the workflow writes on tag `vN` is read by the **next** core release, not that same one. That one-release lag is inherent and stays documented; the manual two-tag dance a human used to run by hand is gone.
...
|---|---|---|
| Banner | The single worst thing true right now, in words a non-engineer can act on. `HELD` is unmistakable and says what it means: *traffic is blocked on purpose because the tunnel is not carrying it*, plus the Settings path to recover the device by hand; otherwise the running activity or actor the farm last pushed, or "idle" | `RouteState`, `VpnLink`, `ControlChannelState`, `ActivityMirror` |
| Now | The running job or actor the farm last pushed, each with a duration and who owns it; a stale warning once the farm has been silent longer than the control-channel timeout; "the farm has not sent an activity list" before the first push | `ActivityMirror` |
| Device | The farm's own facts about this device — label, number, group, tags, stable id — plus what the phone reads about itself: model, Android version, battery, screen on/off | `ActivityMirror`, `Build`, `BatteryManager`, `PowerManager` |
...
| Video | Whether the farm's own scrcpy server is running and at what resolution/fps — a note that this is what the farm STARTED, never a claim that anyone is watching | `ActivityMirror` |
| Inspector | Whether the `ui-tree` `AccessibilityService` is enabled and connected, whether a `ui.watch` subscription is live, the last dump's node count and timing, the last error verbatim | `UiTreeState`, `Settings.Secure` |
...
| Keyboard | Whether `EnkakuIme` is selected, enabled, or absent; whether a field is focused; whether the soft keyboard is actually showing; the per-device "show it anyway with a hardware keyboard" preference | `TextFacet.status`, `ImePrefs` |
| Label | Whether a screen label was ever applied and matches what this app last set, plus the renderer version | `WallpaperFacet.status` |
| Location | Whether a mock GPS fix is active, and the last fix set this process lifetime | `MockLocationState` |
| This build | `versionName`/`versionCode`, wire protocol, advertised capabilities, Android version, model, package, and "host expects build N" once the host's own pin is newer than this build — enough to tell an **outdated** agent from a **misbehaving** one | `PackageManager`, `Protocol`, `HostExpectation` |
...
## The inspector (`ui-tree`)

`ui-tree` gates `ui.dump`/`ui.find`/`ui.watch`/`ui.unwatch`/`ui.status` — `ui/UiTreeService.kt`, an `AccessibilityService` reading the same data source UiAutomator reads (`AccessibilityNodeInfo`) and emitting the SAME node shape `packages/protocol/src/ui-node.ts` already defines, so selectors and every consumer carry over unchanged (MVP 02 §4 phase 2, plan 221). Read-only: it declares no `canPerformGestures`, no `canRequestFilterKeyEvents`, no `canRequestTouchExplorationMode`.

**Enablement over adb is the risky part, and R4 (plan 200 §5) is explicit that no source settles whether an `adb install` is exempt from Android 13+'s restricted settings on every OEM.** The sequence, in order, always run in full — never assumed unnecessary:

```bash
adb shell cmd appops set dev.enkaku.guestagent ACCESS_RESTRICTED_SETTINGS allow
adb shell settings get secure enabled_accessibility_services
adb shell settings put secure enabled_accessibility_services <existing list, plus dev.enkaku.guestagent/dev.enkaku.guestagent.ui.UiTreeService>
adb shell settings put secure accessibility_enabled 1
adb shell settings get secure enabled_accessibility_services
adb shell settings get secure accessibility_enabled
```

A read-back always decides: `enabled` only when the list contains the component **and** the master switch reads `1`. A refusal is reported as `pending` with the platform's own line, never thrown — everything else the agent does keeps working, and the farm falls back to the `ui-server` inspector engine. The status screen's **Open accessibility settings** button is the fallback for a phone where the write never sticks: a human holding the device turns the service on from Settings directly, and the next provisioning pass sees it and turns the device ready on its own.
...
`text.status` also reports `softKeyboardShown` (is the soft keyboard actually on screen right now) and `showSoftKeyboardWithHardware` (the per-device preference) — both absent, never `false`, on a build that predates them (plan 221 §4.6). `text.prefs` writes the preference and answers with the read-back, never the value sent. It answers MVP 08 §1.2's UHID side effect: when scrcpy creates a virtual hardware keyboard, Android hides the soft keyboard by default, and an operator who wants to see it anyway has had no way to ask — `EnkakuIme.onEvaluateInputViewShown` reads the preference from a `SharedPreferences` file (`enkaku-ime`), so it survives both the session that set it and the next reboot.
...
**The `signatureSha256` committed today is provisional, not a placeholder to ignore.** It is real in shape and in origin — read back from an actual keystore generated for plan 90 step 90.1 (`keytool -genkeypair`, then `apksigner verify --print-certs` against a release APK actually built and signed with it) — but that keystore is **not** a production signing identity and is not committed anywhere in this repository. The exact hash is `packages/toolchain/manifest/enkaku-tools.json`'s own `deviceArtifact.signatureSha256` field, quoted only there rather than copied into this document a second time: plan 221 §9 Q1 found this file quoting a DIFFERENT hash than the manifest actually holds, and `scripts/pin-guest-agent.ts` deliberately never touches this field (only the owner knows which keystore `secrets.GUEST_AGENT_KEYSTORE_BASE64` holds), so a second copy here would just as easily drift out of step again. Before cutting a real release, the owner must either (a) adopt whichever keystore produced the manifest's current hash as `GUEST_AGENT_KEYSTORE_BASE64` (handed off out of band, never printed in this document or committed to git), or (b) generate a fresh production keystore and update the manifest field to match it. **Signing a real release with a keystore whose certificate hash does not match this field will make every device report `signature_mismatch` forever** — `AgentProvisioner` reads that as a `failed` state (never a crash, never a stale `ready`, per plan 90 §3.8's failure policy), but it is a fleet-wide outage that is entirely avoidable by checking this one field before the first real tag.
