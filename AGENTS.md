...
- `docs/plans/300-flow-program.md` and `301-*.md` to `313-*.md` — the **Flow** programme: the workflow editor rebuilt as a real graph editor (explicit edges, stored positions, a node catalog plugins can extend, an expression engine, data panes, run replay). Read 300 first — it carries the twelve-interaction parity parameter and the eight decisions every other plan in the series depends on. D4 (expressions) was ratified 2026-09-04; 301-307 and 309-312 are implemented (simulate, the script palette, presets, the `set` node); D5 (fan-out, plan 308) is still open. Plan 313 (Sequential Mode — the linear editor, the node toggle, the `shuffle` node) is draft and awaits the owner's yes on its S1 and S4.
