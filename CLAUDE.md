...
- `docs/plans/300-flow-program.md` and `301-*.md` to `308-*.md` — the **Flow** programme: the workflow editor rebuilt as a real graph editor (explicit edges, stored positions, a node catalog plugins can extend, an expression engine, data panes, run replay). Read 300 first — it carries the twelve-interaction parity parameter and the eight decisions every other plan in the series depends on. Two of those decisions (D4 expressions, D5 fan-out) need the owner's ratification before execution. All draft as of 2026-09-04.
