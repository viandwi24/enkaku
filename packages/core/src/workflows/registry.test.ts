...
describe('listNodeTypes — registry lists both (plan 303 G4)', () => {
  test('the eight core kinds are always present, one `core:<kind>` id each', () => {
    const h = setUp()
    const types = listNodeTypes({ plugins: h.runtime })
    const coreIds = types.filter((t) => t.source === 'core').map((t) => t.id)
    expect(coreIds.sort()).toEqual(['core:delay', 'core:finish', 'core:gate', 'core:script', 'core:set', 'core:shuffle', 'core:start', 'core:switch'])
  })
...
describe('listNodeTypes — an uninstalled/unresolvable plugin node does not crash the registry (plan 303 §6)', () => {
  test('a farm with no plugins at all still returns the eight core types', () => {
    const h = setUp()
    expect(() => listNodeTypes({ plugins: h.runtime })).not.toThrow()
    expect(listNodeTypes({ plugins: h.runtime })).toHaveLength(8)
  })
})
