...
import { createGroupRoutes } from './groups'
...
function seedDevice(db: Db, id: string, groupId: string | null = null): void {
  db.insert(devices)
    .values({ id, stableId: `stable-${id}`, serial: `serial-${id}`, label: `device ${id}`, status: 'online', groupId })
    .run()
}
...
function seedTcpDevice(db: Db, id: string, groupId: string | null = null, address = '10.0.0.5:5555'): void {
  seedDevice(db, id, groupId)
...
function seed(db: Db, n: number) {
  ...
  const ids: string[] = []
  for (let i = 0; i < n; i++) {
    const id = `group-${String(++seq).padStart(4, '0')}`
    ids.push(id)
    db.insert(groups)
...
describe('GET /api/groups keyset pagination', () => {
...
// `POST /:id/devices` (assign) and `DELETE /:id/devices/:deviceId`
// (unassign) are removed by plan 207 (MVP 07): `set-group` is an actions
// API verb now (`POST /api/actions/set-group`), tested in
// `packages/core/src/api/actions.test.ts`.

describe('GET /api/groups/:id/devices', () => {
  test('lists exactly this group\'s members', async () => {
    const db = setUp()
    const [c1] = seed(db, 1)
    seedDevice(db, 'a', c1)
    seedDevice(db, 'b', c1)
 ...
 */
describe('GET /api/groups/:id/devices — number (plan 89 §4.2, §4.3)', () => {
 ...
 * called `rowToDeviceInfo(r, tags, group, null, null, activityState)` with NEITHER
 ...
 * through its group's device list, read the honest-but-incomplete `TCP`.
 * Proven through the real HTTP route and the real response payload, not
 * `deriveConnection`/`rowToDeviceInfo` in isolation.
 */
describe('GET /api/groups/:id/devices — connection.medium (plan 88 §3.6, §4.1, residual gap)', () => {
 ...
 */
describe('GET /api/groups/:id/devices — activities (plan 205 §4.10)', () => {
  ...
  /**
   * The two tests above prove `createGroupRoutes` correctly threads
   ...
   */
  test('a control marker touched through the registry reaches GET /api/groups/:id/devices', async () => {
    const db = setUp()
    const [c1] = seed(db, 1)
    seedDevice(db, 'a', c1)
    const activities = createActivityRegistry({ log: createLogger('test'), controlIdleSec: () => 30, onChange: () => {} })
...
describe('DELETE /api/groups/:id (plan 22.0 §3.6, acceptance #3)', () => {
  test('unassigns members but deletes no device', async () => {
    ...
    const stillThere = db.select().from(groups).where(eq(groups.id, c1!)).get()
    expect(stillThere).toBeUndefined()
  })
})

describe('requirePermission("device.settings") on every group mutation (plan 34 §4.4, §4.5, acceptance #7)', () => {
  ...
  test('an operator (device.settings is an OPERATOR permission) may still create a group — no lockout', async () => {
    const db = setUp()
