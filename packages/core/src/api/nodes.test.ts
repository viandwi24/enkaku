...
import { queryNodesPage } from './nodes'
...
function seed(db: Db, n: number) {
  ...
  const ids: string[] = []
  for (let i = 0; i < n; i++) {
    const id = `node-${String(++seq).padStart(4, '0')}`
    ids.push(id)
    db.insert(nodes)
...
describe('queryNodesPage keyset pagination', () => {
  test('pages through 5 rows with limit=2: union is exactly the 5, no duplicates', () => {
    ...
    let pages = 0
    for (;;) {
      const result = queryNodesPage(db, { cursor, limit: 2 })
  ...
  test('a row inserted mid-paging (newer than everything) is never skipped or repeated', () => {
    ...
    const page1 = queryNodesPage(db, { cursor: null, limit: 2 })
    expect(page1.items).toHaveLength(2)

    seed(db, 1) // newer than everything already loaded

    const page2 = queryNodesPage(db, { cursor: page1.nextCursor, limit: 2 })
  ...
  test('a malformed cursor is rejected, not silently ignored', () => {
    const db = setUp()
    expect(() => queryNodesPage(db, { cursor: 'not-valid-base64!!!', limit: 50 })).toThrow()
  })
})
