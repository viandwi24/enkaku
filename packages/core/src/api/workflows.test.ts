...
describe('Workflow presets over HTTP (plan 311 §3.3, §4.4)', () => {
  test('POST creates, GET lists, PATCH updates, DELETE removes — kind = \'workflow\', no published script needed', async () => {
    const { db, registry, store, runs, pins, scheduler } = setUp()
    const { audit, calls } = fakeAudit()
    const app = withUser('operator', createWorkflowRoutes({ db, registry, store, runs, pins, scheduler, audit }))

    const created = await app.request('/scroll-fyp/presets', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ name: 'slow', params: { pace: 'slow' } }),
    })
    expect(created.status).toBe(201)
    const createdBody = (await created.json()) as { preset: { id: string; kind: string; ownerName: string; name: string } }
    expect(createdBody.preset.kind).toBe('workflow')
    expect(createdBody.preset.ownerName).toBe('scroll-fyp')
    expect(calls.at(-1)).toMatchObject({ action: 'workflow.preset.create', meta: { workflowName: 'scroll-fyp', name: 'slow' } })

    const listed = await app.request('/scroll-fyp/presets')
    expect(listed.status).toBe(200)
    const listedBody = (await listed.json()) as { items: { id: string; name: string }[] }
    expect(listedBody.items.map((p) => p.name)).toEqual(['slow'])

    const id = createdBody.preset.id
    const updated = await app.request(`/scroll-fyp/presets/${id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ params: { pace: 'fast' } }),
    })
    expect(updated.status).toBe(200)
    const updatedBody = (await updated.json()) as { preset: { params: unknown } }
    expect(updatedBody.preset.params).toEqual({ pace: 'fast' })

    const deleted = await app.request(`/scroll-fyp/presets/${id}`, { method: 'DELETE' })
    expect(deleted.status).toBe(200)
    expect(calls.at(-1)).toMatchObject({ action: 'workflow.preset.delete', meta: { workflowName: 'scroll-fyp', name: 'slow' } })

    const afterDelete = await app.request('/scroll-fyp/presets')
    const afterDeleteBody = (await afterDelete.json()) as { items: unknown[] }
    expect(afterDeleteBody.items).toEqual([])
  })

  test('a script preset and a workflow preset with the SAME owner name and preset name never collide (kind partitions the table)', async () => {
    const { db, registry, store, runs, pins, scheduler } = setUp()
    publishScriptRow(db, 'nightly-run', '1.0.0')
    const app = withUser('operator', createWorkflowRoutes({ db, registry, store, runs, pins, scheduler }))
    const res = await app.request('/nightly-run/presets', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ name: 'default', params: { pace: 'slow' } }),
    })
    expect(res.status).toBe(201)
  })

  test('DELETE on an unknown preset id is 404', async () => {
    const { db, registry, store, runs, pins, scheduler } = setUp()
    const app = withUser('operator', createWorkflowRoutes({ db, registry, store, runs, pins, scheduler }))
    const res = await app.request('/scroll-fyp/presets/nope', { method: 'DELETE' })
    expect(res.status).toBe(404)
  })
})
