 ...
 *
 * The house rule this file follows (F29, `groups/dispatch.ts`'s own
...
/** Fresh row for one more repetition, copying the template's dispatch-time fields and resetting every execution-time one — the same shape `groups/dispatch.ts`'s own `toJobRow` builds for repetition 0. */
 ...
 * `groups/status.ts`'s `recomputeBatchStatus` runs synchronously right
...
export function replanAfterRestart(deps: {
  ...
  for (const batch of nonTerminal) {
    ...
    for (const [deviceId, last] of lastByDevice) {
      ...
      // (`groups/status.ts`'s `recomputeBatchStatus`).
