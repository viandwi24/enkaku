 ...
 * every MVP 07 verb through, a target and all: `{ deviceIds } | { groupId } | { labelIds }`.
...
export const actionsRun = defineCapability({
  ...
  deadline: 60_000,
  effect: 'write',
  description: 'Run one action (MVP 07 verbs) on a target: { deviceIds } | { groupId } | { labelIds }. Answers per device; a warned device is not started until force is true.',
  handler: (ctx, input) => {
