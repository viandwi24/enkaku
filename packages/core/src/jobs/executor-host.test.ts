...
/**
 * executor-host tests (plan 36 §7, re-keyed to runs by plan 211 §4.5): the
 * final settle classifies every `failed` run, feeds plan 23's health tracker
 * ONLY for device-blaming failures, and requeues a batch member's run for
 * rebind instead of settling it terminally when the failure is infra and the
 * farm allows it. `job` is the intent (never touched by settle); `run` is
 * the execution `RunStore.settle` writes.
 */
...
function makeRun(overrides: Partial<JobRunRow> = {}): JobRunRow {
  return {
    id: 'run-1',
    jobId: 'job-1',
    seq: 1,
...
interface Recorded {
  ...
  requeueCalls: Array<{ runId: string; newDeviceId: string }>
...
function makeDeps(
  ...
  const recorded: Recorded = { settleCalls: [], requeueCalls: [], activityEnds: [], activityTouches: [], healthNotes: [], rebindEvents: [] }
  let currentRun = run

  const jobStore: Partial<JobStore> = {
    get: () => job,
    requeueForRebind: (runId, newDeviceId) => {
      recorded.requeueCalls.push({ runId, newDeviceId })
      currentRun = { ...currentRun, status: 'queued', deviceId: newDeviceId, infraAttempts: (currentRun.infraAttempts ?? 0) + 1 }
      return currentRun
    },
    renewHeartbeat: () => true,
  }
  ...
  const runs: Partial<RunStore> = {
    getRun: (runId) => (runId === run.id ? currentRun : null),
    settle: (runId, input) => {
      recorded.settleCalls.push({ runId, status: input.status, data: input })
      if (currentRun.status !== 'running') return null
      currentRun = { ...currentRun, status: input.status, error: input.error ?? null, failureClass: input.failureClass ?? null }
      return currentRun
    },
  }

  const watcher: RunWatcher = {
    waitForTerminal: () => Promise.resolve(currentRun),
    notify: () => {},
  }
  ...
  const deps: ExecutorHostDeps = {
    ...
    jobStore: jobStore as JobStore,
    runs: runs as RunStore,
    watcher,
...
describe('createExecutorHost — health is fed only for device-blaming failures (acceptance #5, #6)', () => {
  test('an infra failure (E_ADB_TIMEOUT) notes the health tracker', async () => {
    ...
    expect(recorded.settleCalls[0]?.data.failureClass).toBe('infra')
  })
  ...
  test('a load failure (E_ADB_BUSY) retries but never notes the health tracker', async () => {
    ...
    expect(recorded.settleCalls[0]?.data.failureClass).toBe('load')
  })
  ...
  test('a script failure never notes the health tracker', async () => {
    ...
    await Bun.sleep(10)
    expect(recorded.healthNotes).toEqual([])
    expect(recorded.settleCalls[0]?.data.failureClass).toBe('script')
  })
  ...
  test('a successful job never touches the health tracker or failureClass', async () => {
    ...
    await Bun.sleep(10)
    expect(recorded.healthNotes).toEqual([])
    expect(recorded.settleCalls[0]?.data.failureClass).toBeNull()
    expect(recorded.settleCalls[0]?.status).toBe('success')
  })
})
...
describe('createExecutorHost — batch rebind on an infra failure (plan 36 §3.6, acceptance #7)', () => {
  test('a batch member with rebindOnInfra requeues to another device instead of settling failed', async () => {
    ...
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run, { rebindOnInfra: true, pickRebindDevice: 'dev-2' })
    deps.registry.register('test-script', {
      validateParams: (p) => p,
      run: async () => {
  ...
  test('with rebindOnInfra: false, a batch member settles failed like any standalone job', async () => {
    const job = makeJob({ batchId: 'batch-1', batchSeq: 0 })
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run, { rebindOnInfra: false })
    deps.registry.register('test-script', {
      validateParams: (p) => p,
      run: async () => {
        throw Object.assign(new Error('device gone'), { code: 'DEVICE_DISCONNECTED' })
  ...
  test('a standalone job (no batchId) never rebinds even when rebindOnInfra is true', async () => {
    ...
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run, { rebindOnInfra: true })
    deps.registry.register('test-script', {
      validateParams: (p) => p,
      run: async () => {
        throw Object.assign(new Error('device gone'), { code: 'DEVICE_DISCONNECTED' })
  ...
  test('when no eligible sibling device is idle, requeueForRebind still runs, targeting the same device', async () => {
    const job = makeJob({ batchId: 'batch-1', batchSeq: 0 })
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run, { rebindOnInfra: true, pickRebindDevice: null })
    deps.registry.register('test-script', {
      validateParams: (p) => p,
      run: async () => {
        throw Object.assign(new Error('device gone'), { code: 'DEVICE_DISCONNECTED' })
...
describe('createExecutorHost — notifyCrash (plan 37 §4.4)', () => {
  test('a crash is delivered to a running job that registered ctx.onCrash, and settles APP_CRASHED', async () => {
    ...
    const handled = host.notifyCrash(run.id, { package: 'com.example.app', exception: 'java.lang.NullPointerException', message: 'boom' })
    expect(handled).toBe(true)
    await Bun.sleep(10)

    expect(recorded.settleCalls[0]?.status).toBe('failed')
    expect(recorded.settleCalls[0]?.data.error).toBe('com.example.app crashed')
    expect(recorded.settleCalls[0]?.data.failureClass).toBe('script')
    expect(recorded.healthNotes).toEqual([]) // never blames the device (acceptance #10)
    resolveRun?.()
  })

  test('notifyCrash for a run with no registered handler (or no longer running) is a no-op', async () => {
    const job = makeJob()
    const run = makeRun()
    const { deps } = makeDeps(job, run)
    const host = createExecutorHost(deps)
    expect(host.notifyCrash('no-such-run', { package: 'x', exception: 'y', message: 'z' })).toBe(false)

    deps.registry.register('test-script', { validateParams: (p) => p, run: async () => 'ok' })
    host.start(job, run)
    await Bun.sleep(10)
    // The run already finished (successfully, without ever calling
    // ctx.onCrash) — its handler entry is gone.
    expect(host.notifyCrash(run.id, { package: 'x', exception: 'y', message: 'z' })).toBe(false)
  })
})
 ...
 * and the run row's `peakRssBytes` column. No limit is enforced here or
 * anywhere in this step — only that the number reaches `runs.settle`.
 */
describe('createExecutorHost — ctx.onPeakRss reaches runs.settle (plan 98 §4.8, H1)', () => {
  test('a successful job that reported a peak carries it into settle()', async () => {
    const job = makeJob()
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run)
    deps.registry.register('test-script', {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('success')
    expect(recorded.settleCalls[0]?.data.peakRssBytes).toBe(314_572_800)
  })

  test('a FAILED job that reported a peak still carries it into settle() — the measurement is not conditional on success', async () => {
    const job = makeJob()
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run)
    deps.registry.register('test-script', {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('failed')
    expect(recorded.settleCalls[0]?.data.peakRssBytes).toBe(52_428_800)
  })
  ...
  test('an executor that never calls onPeakRss (no subprocess) omits the field entirely — never a bare 0', async () => {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('success')
    expect(recorded.settleCalls[0]?.data.peakRssBytes).toBeUndefined()
  })
})
 ...
 * SAME `runs.settle()` call `peakRssBytes` above already proves reaches.
 ...
 */
describe('createExecutorHost — the result outcome reaches settle() (plan 97 §3.3, §4.5)', () => {
  test('a script executor reporting a valid outcome writes all four result columns', async () => {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('success')
    const data = recorded.settleCalls[0]?.data ?? ({} as (typeof recorded.settleCalls)[number]['data'])
  ...
  test('a FAILED job with no reported outcome still leaves every result_* column untouched — the common case, nothing to report (unchanged by step 97.4)', async () => {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('failed')
    expect(recorded.settleCalls[0]?.data.resultStatus).toBeUndefined()
  })
  ...
  test('a FAILED job DOES get a resultStatus written when the executor reported one (97.4 closes the gap the test above used to pin)', async () => {
    ...
    expect(recorded.settleCalls[0]?.status).toBe('failed')
    expect(recorded.settleCalls[0]?.data.resultStatus).toBe('partial')
    ...
    expect(recorded.settleCalls[0]?.data.error).toBe('boom')
    expect(recorded.settleCalls[0]?.data.failureClass).toBeTruthy()
  })
  ...
  test('a FAILED job\'s reported partial never overwrites an already-recorded valid — existingStatus threads from run.resultStatus', async () => {
    const job = makeJob()
    const run = makeRun({ resultStatus: 'valid' })
    const { deps, recorded } = makeDeps(job, run)
    deps.registry.register('test-script', {
      validateParams: (p) => p,
      run: async (_job, ctx) => {
...
/**
 * Plan 205 §4.7 — the heartbeat interval also touches the run's own
 * `job:<runId>` activity, so a long-running job's marker never looks stale
 * to anything reading the registry (MVP 04 §1.1: "updatedAt: last
 * heartbeat, last progress, or last input").
 */
describe('createExecutorHost — the heartbeat touches the job activity (plan 205 §4.7)', () => {
  test('settle ends the job:<runId> activity on the device it ran on', async () => {
    const job = makeJob()
    const run = makeRun()
    const { deps, recorded } = makeDeps(job, run)
    deps.registry.register('test-script', { validateParams: (p) => p, run: async () => 'ok' })
