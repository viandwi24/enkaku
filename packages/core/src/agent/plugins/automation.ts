...
import { scriptGet, scriptList } from '../../capability/script'
import { pluginStage } from '../../capability/plugin'
 ...
 * where the trace is the part that explains a failure. Plan 210 (MVP 03 §2) replaces
 * `script_publish` with `plugin_stage`: a script exists only as a member of a plugin. */
export const automationPlugin = defineAgentPlugin({
  id: 'automation',
  title: 'Scripts and jobs',
  prompt: [
    '# Scripts and jobs',
    'script_list and script_get read the members of active plugins; plugin_stage ships a plugin from',
    'a workspace path (or an inline bundle) — it is a write, and never executes anything itself. A',
    'script cannot be published on its own.',
  ...
  tools: () => [scriptList, scriptGet, pluginStage, jobRun, jobGet, jobList, jobCancel, jobTrace, jobTraceUi, jobTraceFrame],
})
