  ...
  const daemonHandle: Daemon = {
    ...
    async start() {
      ...
      const app = createApp({
        ...
        vmRoutes: createVmRoutes({
          manager: vmManager,
          dataDir: cfg.dataDir,
          log,
          // The SAME toolchain instance every other tool goes through, never
          // a second one over the same directory.
          toolchainSdkmanager: () => toolchain.resolveToolPath('cmdline-tools').catch(() => null),
          installCmdlineTools: () => toolchain.ensureRequiredTools(['cmdline-tools']),
        }),
