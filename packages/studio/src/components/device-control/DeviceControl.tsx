      ...
      <div className="flex w-[274px] shrink-0 flex-col overflow-y-auto border-l border-line">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="flex min-h-0 flex-1 flex-col">
          <TabsList variant="compact" className="mx-3 mt-2">
            ...
            {/*
              Network lives inside this tab as a third chip section, next to
              Jobs and Files (`DeviceTab.tsx`) — the same "generic container
              tab, not one tab per feature" reasoning the handoff already
              applies to Jobs/Files, and the reason a four-item top-level row
              no longer fits the 274px column at the popup's narrower widths
              (owner report, 2026-09-13). `NetworkPanel` and everything under
              it survived the Studio rewrite intact and is restored here, not
              rewritten: already migrated to the `set-network` verb for
              writes and `GET /:id/network` for reads.
            */}
