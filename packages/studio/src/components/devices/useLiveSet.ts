...
/**
 * The Screens grid's live-set policy (plan 92 §3.2, §4.6; moved to
 * `components/devices/` and updated for always-on sessions by plan 214 §3.9)
 * — replaces "first N of the filtered list" (F13) with a policy an operator
 * can predict: what is on screen, what is already live, what is awake. Split
 * in two on purpose, per this step's own brief: keep the decision provable
 * without a browser.
 *
 ...
 *    returns the same outputs, so a test of the pure function can assert
 *    ordering, eviction, budgeting and the ramp gate directly, with no DOM at
 *    all (Studio has zero tests since plan 201; `useLiveSet.test.ts` itself
 *    was deleted, plan 214 §4.4).
 ...
 *    once while the grid fills in, §3.3).
 *
 * Together they fix:
 *  - **F13** — ordering within the budget is pinned → already-live-and-visible
 *    → newly-visible-and-awake → grid order, never fleet-list order, and
 *    eviction always takes the LOWEST-ranked member of the previous live set
 *    first (see `rankOf` below). Plan 214 §3.9 drops the old tier 1 (hot and
 *    visible ranks above already-live): under always-on sessions (plan 206
 *    §1.2) every online device's session is already open, so that tier no
 *    longer distinguished anything.
 *  - **F14** — this module has no opinion on when it is safe to call
 *    `useLiveSet` with a real `maxTiles`; the Screens grid holds the skeleton
...
export interface LiveSetInput {
  devices: DeviceInfo[]
  /**
   * The resolved video tile cap AS ACTUALLY APPLIED — plan 92 §5 step 92.3's
   ...
   * yet" (the caller's job to avoid, per the Screens grid's own loading
   * state), not "auto".
   */
  maxTiles: number
  /** `wall.rampConcurrency` (`/api/settings`) — how many tiles may newly ask for a stream at the same time while the grid fills in (§3.3). */
 ...
 *  1. visible AND already live — stability: a same-tier newcomer must not
 *     evict a tile that is already decoding.
 *  2. visible AND awake, never (yet) live — a genuinely new candidate.
 *  3. everything else eligible — not currently visible. This is exactly
 ...
 *
 * Plan 214 §3.9 drops the old tier 1 ("visible AND hot" — a hot device's
 * session is already open, promoting it cost one map lookup rather than a
 * fresh build): under always-on sessions every online device's session is
 * already open, so that tier no longer distinguished anything, and every
 * rank below it shifts up by one.
 */
function rankOf(d: DeviceInfo, visible: ReadonlySet<string>, pinned: ReadonlySet<string>, live: ReadonlySet<string>): number {
  ...
  if (isVisible && live.has(d.id)) return 1
...
export function computeLiveSet(input: LiveSetInput): LiveSetOutput {
  ...
  // Eligibility by status (§3.2 rule 1): `offline`/`quarantined` never
  // stream. Plan 214 §3.9 drops the old third branch (`asleep` blocked) —
  // under always-on sessions (plan 206 §1.2) an asleep device's session is
  // already open and its tile shows a dark screen, not a loading panel, so
  // it is a genuine candidate like any other online device.
  ...
  const ranked = [...eligible].sort((a, b) => {
    ...
    // Tiers 1-2 both require `visible`, so `visibleOrder` (newest-first) is
    // the right tie-break inside each of them; tier 3 (not visible at all)
    // falls back to grid order (see the file header on `now`).
    if (ra === 3) return (gridOrder.get(a.id) ?? 0) - (gridOrder.get(b.id) ?? 0)
  ...
  //
  // Membership follows the viewport (§3.2 rule 2) — a rank-3 device (not
  ...
  // exactly that (rank-3 entries sort last, but they would still fall
  // inside a slice that has room), so rank-3 devices are routed straight to
  // `budgeted` regardless of cap pressure, and the cap only ever competes
  // among genuine candidates (rank 0-2).
  const cap = Math.max(0, Math.floor(maxTiles))
  const target: DeviceInfo[] = []
  const budgeted: string[] = []
  for (const d of ranked) {
    const isCandidate = rankOf(d, visible, pinned, live) < 3
...
/**
 * The stateful half. The Screens grid calls this once with the full, flat
 * device list (never a per-section slice — the live-tile cap is a single
 * farm-wide budget, plan 92 §4.6's own note) and the resolved `maxTiles`/
