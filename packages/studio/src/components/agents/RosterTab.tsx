...
import { CopyIcon, DotsThreeIcon, PlusIcon, RobotIcon, TrashIcon } from '@enkaku/ui'
import { z } from 'zod'
import {
  AlertDialog,
  AlertDialogAction,
...
import { AgentResponseSchema, ListThreadsResponseSchema } from '@enkaku/protocol'
...
/**
 * The Agents page's Roster tab (plan 220 §4.4, moved from `app/agents/page.tsx`,
 * which this plan retires — plan 65 §4.6, plan 73 §3.3/§4.3). Agents/error/reload
 * now come from `AgentsPage` (fetched once so the tab-strip count and this
 * table never disagree); everything else — create/duplicate/delete, the New
 * agent dialog, the Delete confirmation with thread/run counts — is
 * byte-identical to the page this replaces.
 */
export function RosterTab({
  ...
  const create = () =>
    run('create', () => api('/api/agents', AgentResponseSchema, { method: 'POST', json: { name, slug } }), {
      ...
      onSuccess: (b) => onNavigate(`/agents/detail?id=${b.agent.id}`),
    })
  ...
  const duplicate = (a: Agent) =>
    run(
        ...
        }),
      {
        ...
        onSuccess: (b) => onNavigate(`/agents/detail?id=${b.agent.id}&tab=settings&focus=name`),
      },
    )
    ...
    <>
      ...
      <div className="px-5 pb-4">
        {error ? (
          ...
          <EmptyState
            icon={<RobotIcon className="size-4" aria-hidden />}
            title="No agents yet"
