  ...
  const matchesAccept = useCallback(
    (f: File) => {
      ...
      return patterns.some((pattern) => {
        ...
        // ".md"-style extension patterns — many text/code files carry an empty or generic MIME
        // type, so accept must be able to match on the filename too (native <input accept> does).
  ...
  const inner = (
    <>
      ...
      <form
        ...
        {...props}
      >
        {/* Token-driven input card: border/focus ring come from --input/--ring (coral) via
            InputGroup's has-[:focus-visible] classes; rounded-xl per the design language. */}
...
export const PromptInputTextarea = ({
  ...
  const handlePaste: ClipboardEventHandler<HTMLTextAreaElement> = useCallback(
    (event) => {
      // External handler runs first (same pattern as onKeyDown): callers can intercept e.g. long
      // text pastes. NOTE: `onPaste` must be destructured (not left in {...props}) or it would be
      // spread AFTER this handler and silently replace the file-paste staging below.
      onPaste?.(event);
...
export const PromptInputSelectContent = ({
  ...
  <SelectContent className={cn("p-1.5", className)} {...props} />
);
...
export const PromptInputSelectItem = ({
  ...
  <SelectItem className={cn("rounded-lg px-2.5 py-1.5", className)} {...props} />
);
