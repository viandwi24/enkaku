...
function Screenshot({ blobId }: { blobId?: string }) {
  if (!blobId) return <div className="grid h-16 w-28 shrink-0 place-items-center rounded border border-dashed text-[10px] text-faint">no image</div>
...
function StepRow({
  ...
  return (
    <li className="flex gap-3 rounded-lg border bg-panel p-3">
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-[11px] text-dim">gap {step.gapMs} ms</span>
        </div>
        ...
        {candidate && (
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            <span className="text-dim">candidate {JSON.stringify(candidate.selector)}</span>
            <Badge variant={candidate.count === 1 ? 'secondary' : 'outline'}>{candidate.count} match{candidate.count === 1 ? '' : 'es'}</Badge>
            <span className="text-faint">anchor {candidate.anchorAgeMs} ms old, {candidate.anchorStepsSince} step(s) since</span>
          </div>
        )}
        {target?.kind === 'point' && !candidate && (step.kind === 'tap' || step.kind === 'longPress') && (
          <p className="text-[11px] text-faint">No candidate — no inspector was attached when this step landed.</p>
        )}

        {step.kind === 'text' && typeof step.value === 'string' && (
          <div className="space-y-1.5 rounded-md border border-warn/40 bg-warn/5 p-2">
        ...
        {step.kind === 'text' && typeof step.value === 'object' && (
          <div className="flex flex-wrap items-center gap-2 text-[11.5px]">
            ...
            {priorLiteral !== undefined ? (
              ...
              <>
                <span className="text-[11px] text-faint">original text not kept — nothing to revert to</span>
    ...
    <>
      ...
      <div className="grid gap-4 px-5 py-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-3">
          ...
          {detail.detached && (
            <p className="rounded-md border bg-panel-2 px-3 py-2 text-[11.5px] text-dim">
          ...
          {steps.length === 0 && <p className="text-[12px] text-dim">Every step has been removed.</p>}
        </div>

        <aside className="space-y-4">
          <div className="space-y-2.5 rounded-lg border bg-panel p-3">
            <p className="rack-label text-text">Replay</p>
            <div className="space-y-1">
              <Label className="text-[11px] text-dim">Speed (× recorded gaps)</Label>
              <Input
                type="number"
                step="0.1"
                min="0.1"
            ...
            <div className="space-y-1">
              <Label className="text-[11px] text-dim">Max gap (ms)</Label>
              <Input
                type="number"
                min="0"
                value={maxGapMs}
            ...
            <div className="space-y-1">
              <Label className="text-[11px] text-dim">Cleanup on finish</Label>
              <Select
                value={cleanup}
                disabled={detail.detached}
                onValueChange={(v) => {
            ...
            <div className="space-y-1">
              <Label className="text-[11px] text-dim">Description</Label>
              <Textarea
                value={description}
                disabled={detail.detached}
                onChange={(e) => {
          ...
          {!detail.detached && (
            <div className="space-y-2.5 rounded-lg border bg-panel p-3">
              <p className="rack-label text-text">Publish</p>
              <div className="space-y-1">
                <Label className="text-[11px] text-dim">Version</Label>
          ...
          <div className="space-y-2.5 rounded-lg border bg-panel p-3">
            <p className="rack-label text-text">Ownership</p>
            {!detail.detached ? (
              <ConfirmDialog
                trigger={
              ...
              <p className="text-[11px] text-dim">Already detached — owned as an ordinary script file.</p>
            )}
            <ConfirmDialog
              trigger={
