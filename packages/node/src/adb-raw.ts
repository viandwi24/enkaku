...
import type { NodeToControl } from '@enkaku/protocol'
import type { DeviceSnapshotSource, Logger } from '@enkaku/session'

/**
 * Defence in depth (plan 28 §4.3): "a node cannot be pushed past its own
 ...
 * across the tunnel: pinning this to the default would make the node
...
/**
 * Node-side handler for the cloud adb endpoint (plan 28 §4.3): on
 * `adb.open.request`, opens a raw smartsocket stream against the node's OWN
  ...
  return {
    async openRequest(msg) {
      ...
      const reply = (payload: { ok: boolean; error?: { code: string; message: string } }) =>
        deps.send({ type: 'adb.open.reply', ...(msg.id ? { id: msg.id } : {}), payload } as NodeToControl)
      ...
      if (current >= MAX_STREAMS_PER_DEVICE) {
        reply({
          ok: false,
          error: { code: 'E_ADB_STREAM_LIMIT', message: `this node already has ${MAX_STREAMS_PER_DEVICE} adb streams open for ${deviceId}` },
        })
        return
      }
