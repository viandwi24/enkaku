...
describe('createAdbRawHost (plan 28 §4.3) — open/pipe/close', () => {
  ...
  test('the backend ending (device/node-adb side) sends adb.close exactly once and frees the per-device slot', async () => {
    const sent: NodeToControl[] = []
    const fake = fakeSocket()
    const client = { openRaw: async () => fake.socket } as unknown as AdbClient
    const host = createAdbRawHost({ client, devices: makeDevices([DEVICE]), send: (msg) => sent.push(msg), sendFrame: () => {}, log: fakeLogger() })
    ...
    expect(sent).toEqual([
      {
        type: 'adb.open.reply',
        id: 'req-over',
        payload: { ok: false, error: { code: 'E_ADB_STREAM_LIMIT', message: `this node already has ${CAP} adb streams open for dev-1` } },
      },
    ])
