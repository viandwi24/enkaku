...
/**
 * Backpressure (plan 25 §3.5): once the node's own outbound tunnel buffer
...
/**
 * Node-side handlers for the correlated shell requests (plan 25 §4.4):
 * `shell.exec.request` runs through the node's own `AdbClient.exec` with the
  ...
  return {
    async execRequest(msg) {
      ...
      const reply = (payload: unknown) =>
        deps.send({ type: 'shell.exec.reply', ...(msg.id ? { id: msg.id } : {}), payload } as NodeToControl)
    ...
    async streamRequest(msg) {
      ...
      const reply = (payload: unknown) =>
        deps.send({ type: 'shell.stream.reply', ...(msg.id ? { id: msg.id } : {}), payload } as NodeToControl)
      const snap = deps.devices.get(deviceId)
      if (!snap) {
        reply({ ok: false, error: { code: 'device_not_found', message: `no such device: ${deviceId}` } })
        return
      ...
      try {
        const handle = await deps.client.execStream(snap.serial, cmd, {
          onData: (chunk) => {
            // Backpressure (§3.5): the node's own outbound buffer is already
