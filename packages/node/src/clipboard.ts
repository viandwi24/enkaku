...
/**
 * Node-side handlers for the correlated clipboard requests (plan 38 §4.5) —
 ...
 * the node already builds for video (`hosts.ts`'s `sessions`): a device's
...
export function createClipboardHost(deps: {
  ...
  return {
    async getRequest(msg) {
      ...
      const reply = (payload: unknown) => deps.send({ type: 'clipboard.get.reply', id: msg.id, payload } as NodeToControl)
    ...
    async setRequest(msg) {
      ...
      const reply = (payload: unknown) => deps.send({ type: 'clipboard.set.reply', id: msg.id, payload } as NodeToControl)
      const session = deps.sessions.get(deviceId)
      if (!session) {
        reply({ ok: false, error: { code: 'device_not_ready', message: `no active session for device ${deviceId}` } })
        return
