 ...
 *
 * "Everywhere" now includes a plugin's screen (plan 111 §3.3), which is why
 * this moved out of Studio: `relativeTime` alone had 38 call sites in Studio,
 * and a plugin rendering `new Date(s * 1000).toLocaleString()` beside them —
 * which is exactly what the first tier-C pack did — makes the same fact read
