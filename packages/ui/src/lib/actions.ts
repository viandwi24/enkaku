...
/**
 * One path for every action that calls the core (plan 111 §3.3 — extracted
 * from `packages/studio/src/lib/actions.ts`, which is now this file).
 *
 ...
 * text, and some actions gave no feedback at all. A plugin author starting
 * from nothing writes the same thirty lines again, worse — which is what the
 * first tier-C pack did before this moved here.
 */
 ...
 *
 * `path` is resolved against `coreBase()`, which answers the same question
 * for Studio and for a plugin — see that function for the order it tries.
 */
export async function api<S extends z.ZodType>(
  ...
  const res = await fetch(`${coreBase()}${path}`, {
    ...
    // The session cookie, on a cross-origin call. Studio same-origin does not
    // need it and is unaffected; a plugin under `bun run dev:studio` reaches
    // the core on another port, where the default `same-origin` policy would
    // send no cookie at all and every read would 401 on a farm the operator
    // is plainly logged into (111.7's own `farm()` had to set this).
    credentials: 'include',
    ...rest,
    ...(json !== undefined
