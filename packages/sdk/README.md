...
This is a hard rule, not a convention: `enkaku publish` refuses an entry whose default export is not a `definePlugin()` result, and the refusal prints the wrapper so you can fix it from the error text. Publishing goes through `POST /api/plugins` (`enkaku publish`) or the `plugin.stage` capability; there is no per-script publish route, and a script has no version of its own: it carries its plugin's.
...
and reviews it. Publishing is parked for the MVP (docs/mvp/06-feature-scope.md
§2, plan 210): `POST /api/recordings/:slug/publish` answers `410` and writes
nothing — review, trim, parameterise and detach still work. When it does
publish (post-MVP, one plugin per recording), the resulting bundle's entry
point is generated, not hand-written, and it is deliberately thin:
...
A **workflow** (plan 99, M64; its own table since plan 210 — MVP 03 §2.2 rule 4) is a pipeline of published scripts, run as one workflow job on one device. Nothing about how you write a script changes to make it usable as a node: any published script can be one, referenced by `name@version` (or `name@latest`), and it runs exactly the way it runs on its own — its own child process, its own `timeout`/`retries`/`params`/`finish()`. A workflow has no version of its own and is edited in place; a job created from it snapshots the document at enqueue, so editing a workflow never changes a queued or running job. A workflow author cannot see your source, only your `paramsSchema` and, if you declare one, your output shape; they wire your script's parameters to a constant, a workflow parameter, or an earlier node's output through a closed binding grammar that never evaluates code (see `packages/protocol/README.md`'s "the rule that matters most").
