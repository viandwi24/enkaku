...
- **`@enkaku/ui/theme.css` is the farm's design tokens** — `bg-surface`, `text-fg-muted`, `text-led-ok`, and the `hover-none:` variant — read from the same file Studio compiles against, so the two cannot drift.
