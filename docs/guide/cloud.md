...
Video travels through the WebSocket tunnel. For a node on a healthy network that is fine, but on the internet with packet loss TCP holds the whole stream until the packet is retransmitted — the screen can freeze for a moment. The WebRTC path addresses this (see `docs/archive/plans/13-m9-webrtc-backend.md`).
