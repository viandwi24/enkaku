...
> Status: not started
> Ships: `packages/core/src/agent/{loop,thread,compaction,approval}/*`, `packages/protocol/src/messages/agent.ts`, `packages/core/src/server/ws-handlers-agent.ts`, one Drizzle migration
