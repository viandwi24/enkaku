...
7. **`@enkaku/host` makes a plugin UI module untestable by static import, and this plan did not notice.** Found while building plan 131 §10 item 4. A module that imports `@enkaku/host` cannot be reached by `import './that-module'` in a test file: the specifier resolves only through Studio's import map in a browser, so module resolution fails before any test code runs. A test must `mock.module('@enkaku/host', …)` and then `await import(...)` **dynamically** — `mock.module` need only precede evaluation, not resolution. `@enkaku/ui` is unaffected; it is a real workspace package on disk.

    This is a real tax on every tier-C plugin that uses a host component, it is a direct consequence of §3.4's design, and it belongs in the SDK scaffold's own comment beside the `declare module '@enkaku/host'` block — where an author will meet it — rather than only here.
