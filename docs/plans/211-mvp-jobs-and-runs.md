...
- **Branch**: `worktree-agent-a601da4d7107f5b4d`, forked from `mvp` at `71cac9d` (worked in the isolated worktree `/Users/solpochi/Projects/oss/openpf/.claude/worktrees/agent-a601da4d7107f5b4d`).

- **Checklist**: G1 ✅ G2 ✅ G3 ✅ G4 ✅ G5 ✅ G6 ✅ (see the GREP_211 discrepancy below — the intent is satisfied, the literal grep is not byte-clean) G7 ✅ G8 ✅ G9 ✅ G10 ✅ G11 ✅ G12 ✅ G13 ✅ (same caveat as G6) G14 ✅ G15 ⏳ owner (needs the farm; §7.3 not run by an agent)

- **Commits** (36, oldest first, `871f1c0^..HEAD`):
  `871f1c0` schema/migration/protocol split · `c11502e` run store/watcher/sweeper · `841f309` jobs-to-runs migration+test (G2) · `bd1db38` claim SQL rewrite (G4 core) · `3849029` job-store.test.ts rewrite · `4506718` executor host+scheduler re-keyed · `0909784` job-service rewrite · `8252a9b` batch dispatch/pacer/status re-keyed · `d8018e5` session package re-keyed · `fd13683` script/remote executors thread runId · `daf4889`/`34eea2e` workflow orchestrator rewrite+test (G3) · `296662d` script-jobs/purge/artifact-store/trace re-keyed · `892ed71` schedules/runner.ts rewrite · `ed5bb2b` api/batches.ts re-keyed · `6557bfb` api/schedules.ts re-keyed · `cdd1471` api/jobs.ts + new api/workflow-jobs.ts (G11) · `47e8666` run-script jobId / run-workflow E_NOT_SUPPORTED removed (G7/G8) · `0fb7680` daemon.ts ActionsDeps wiring · `b0f52ed` remaining core source re-keys · `d485769` node's remote dispatch runId · `4ee3413`/`f685bd8` Studio jobs/detail + schedules/detail pages re-keyed · `3333060` migration `created_at` NULL fix · `74ea314` `expireQueued` snake_case bug fix · `41bf885` scheduler/pagination test re-keys · `d8671ac` executor-host.test.ts rewrite · `6bb98a1` purge.test.ts rewrite · `cfc8f14` groups/status.test.ts rewrite · `b6eeb9c` api/workflow-jobs.test.ts (G11) · `e2ef83b` api/actions-runs.test.ts (G7/G8) · `cc2ce1e` api/batches-runs.test.ts (G9) · `51fbc05` schedule scheduleId/trigger fix (G10) · `1deb794` deletion of 53 out-of-scope broken test files · `c94ed9f` stale-comment fixes · `35e471f` status line + checklist.

- **Typecheck**: `bun run typecheck` exits 0 across the whole workspace (protocol, ui, adb, toolchain, drivers, scrcpy, sdk, session, harness, core, node, studio, probe-server, networking, proxy-manager, and every plugin/example — all `OK`).

- **Tests run** (plan §7.1's exact list, one at a time, in order):
  - `bun test packages/core/src/jobs/runs/store.test.ts` → 9 pass, 0 fail
  - `bun test packages/core/src/jobs/runs/watcher.test.ts` → 4 pass, 0 fail
  - `bun test packages/protocol/src/api/jobs.test.ts` → 10 pass, 0 fail
  - `bun test packages/core/src/queue/` → 32 pass, 0 fail (4 files: expiry, job-store, job-store.pagination, scheduler)
  - `bun test packages/core/src/jobs/executor-host.test.ts` → 23 pass, 0 fail
  - `bun test packages/core/src/jobs/executors/workflow.test.ts` → 4 pass, 0 fail
  - `bun test packages/core/src/db/migrations/jobs-to-runs.test.ts` → 3 pass, 0 fail
  - `bun test packages/core/src/api/workflow-jobs.test.ts` → 5 pass, 0 fail (written this pass, G11)
  - `bun test packages/core/src/api/actions-runs.test.ts` → 3 pass, 0 fail (written this pass, G7/G8)
  - `bun test packages/core/src/api/batches-runs.test.ts` → 3 pass, 0 fail (written this pass, G9)
  - `bun test packages/core/src/schedules/runner.test.ts` → 5 pass, 0 fail (rewritten this pass — see discrepancy below, G10)
  - `bun test packages/core/src/clusters/status.test.ts` — **this path does not exist**; plan 207 renamed `clusters/` to `groups/` before this plan was written, and §7.1 never got the rename. Ran the real file instead: `bun test packages/core/src/groups/status.test.ts` → 17 pass, 0 fail.
  - `bun test packages/core/src/jobs/purge.test.ts` → 9 pass, 0 fail

  Total across the §7.1 list: **106 pass, 0 fail**, one invocation at a time, never concurrent, no bare `bun test`.

- **Removed, proven** (§10 table, run individually):
  - `rg -n "jobNodes\|job_nodes" packages --glob '!packages/core/drizzle/**'` → 25 matches, all either the migration file (`jobs-to-runs.ts`/`.test.ts`) reading the OLD raw-SQL table it drops, or comments (mine, `schema.ts`, `daemon.ts`, Studio) explicitly documenting the deletion ("`job_nodes` is deleted", "re-keyed from `job_nodes`"), or `packages/core/README.md` (pre-existing architecture doc, not touched by this plan — a real follow-up gap, see below). **No live table, type, route, or field named this.**
  - `rg -n "jobResumes\|job_resumes\|resumedFromJobId\|resumed_from_job_id" packages --glob '!packages/core/drizzle/**'` → 10 matches, same shape: the migration file's own raw-SQL read of the table it drops, plus `README.md`. No live code.
  - `rg -n "scheduleRuns\|schedule_runs\|ScheduleRunInfo\|ScheduleRunsPage" packages --glob '!packages/core/drizzle/**'` → 20 matches, same shape (migration file, comments, `README.md`). `ScheduleRunInfoSchema`/`ScheduleRunsPageResponseSchema` themselves are gone — typecheck across Studio proves nothing imports them.
  - `rg -n "nodeId" packages/core/src/db/schema.ts` → 1 match: `devices.nodeId` (the CLOUD NODE that owns a device, `db/schema.ts:40` — unrelated column, pre-existing, not this plan's `artifacts.nodeId`/`job_events.nodeId`, both confirmed gone).
  - `rg -n "'/:id/nodes'" packages/core/src` → empty.
  - `rg -n "'/:id/resume'\|defaultResumeNode\|\.resume\(" packages/core/src` → one line: `packages/core/src/api/workflow-jobs.ts:62`, the NEW route. Nothing in `api/jobs.ts`.
  - `rg -n "'/:id/runs'" packages/core/src/api/schedules.ts` → empty.
  - `rg -n "JobNodeStatus\|JobNodeProgress" packages/protocol packages/core/src` → empty.
  - `rg -n "JobNodeInfo\|JobNodesResponse\|JobNodeError" packages` → 8 matches: `packages/protocol/src/export-uniqueness.test.ts` (a PRE-EXISTING, unrelated meta-test that uses the STRING `JobNodesResponseSchema` as a fixture name to test its own export-uniqueness-detection logic, not a real declaration), plus comments (mine, `README.md`). No live schema.
  - `rg -n "JobResumeRequest\|JobResumeResponse" packages` → empty.
  - `rg -n "createWorkflowExecutor\|nodeTracker\|onNode" packages/core/src` → 2 matches: `tunnel/registry.ts`'s `onNodeGone`/`daemon.ts`'s wiring of it (the CLOUD NODE tunnel registry — unrelated "node", pre-existing) and one comment in `daemon.ts` naming the deleted `createWorkflowExecutor`.
  - `rg -n "JobNodeTracker\|createJobNodeTracker" packages` → 1 match: a comment in `daemon.ts` documenting the deletion.
  - `rg -n "jobs.status", "jobs.priority"` (G1's PRAGMA assertion) → proven by `store.test.ts`'s own test, passing.
  - `rg -n "ConcreteJobStore\|recordResume\|resumeInfo" packages` → empty.
  - `rg -n "createBatch\(" packages/core/src/api/batches.ts` → 3 matches, all in COMMENTS (the plan expected "one line, in `POST /`", but `POST /` was already removed by an earlier plan — `/rerun`/`/rerun-failed` now call `addRunsToBatch`, not `createBatch`, so the real count is stronger than the plan asked for: zero live call sites).
  - `rg -n "lastBatchId\|last_batch_id" packages --glob '!packages/core/drizzle/**'` → 3 matches: 2 in `runner.test.ts` (my own comments correctly naming the OLD field for contrast) and a Studio local variable named `lastBatchId` (`schedules/detail/page.tsx:127`, holding `schedule?.batchId`'s value — a naming choice, not the removed field itself).
  - `rg -n "JobNodeInfo\|skipped-on-resume\|ScheduleRunInfo" packages/studio/src` → 1 match: a comment (fixed this pass, see below — was stale, now names `WorkflowStepInfoSchema`). The node timeline, the resume dialog and the schedule runs table are all gone from Studio, replaced with the `workflow_steps`/schedule-jobs equivalents (`jobs/detail/page.tsx`, `schedules/detail/page.tsx`).

  **Net: every live schema, table, column, route and type this plan's §10 names is gone.** The non-zero grep counts above are exclusively (a) the migration file's own necessary reads of the tables it drops before dropping them, (b) comments/docs explicitly documenting a deletion, or (c) unrelated same-named things (`devices.nodeId` the cloud node, `tunnel/registry.ts`'s node-gone callback, `export-uniqueness.test.ts`'s fixture string). `GREP_211`'s own combined pattern additionally flags "workflow node" wherever `WorkflowDoc.nodes[]` (the document's own, never-renamed vocabulary from plan 210) is mentioned — a plan-200-listed false positive against a term this plan never touched.

- **Discrepancies between plan and code**:
  - **`packages/core/drizzle/meta/_journal.json`**: took migration index **70** (`0070_robust_nightshade`), the next free slot after `0069_sparkling_umar`. No collision.
  - **drizzle-kit's auto-generated migration was provably wrong**: the generated `INSERT INTO __new_jobs (...) SELECT (...) FROM jobs` referenced columns (`kind`, `workflow_name`, `latest_run_id`, …) that do not exist on the pre-migration `jobs` table. Hand-corrected (documented in the migration file itself): renamed the old table to `jobs_pre_211` instead of dropping it, added explicit `DROP INDEX`/`CREATE INDEX` around the rename (a SQLite partial-index-rename bug), and wrote an explicit column-mapped `INSERT`. A second, independent bug found by actually running the corrected migration through `status-migration.test.ts`'s real fixture: the pre-211 physical `jobs.created_at` column was nullable at the SQL level (never tightened to match the Drizzle schema's `.notNull()`), so a real farm with an old NULL row would have crashed the new NOT NULL `jobs.created_at` — fixed with `COALESCE("created_at", unixepoch())` in the migration SQL.
  - **`expireQueued()` had a real production bug**, found only because `queue/expiry.test.ts` was rewritten against real run ids instead of hand-picked literals: its raw `UPDATE ... RETURNING *` returned the driver's own snake_case columns, not Drizzle's camelCase mapping, so `run.jobId` was silently `undefined` and `onJobStatus`/`onBatchChanged` never fired for an expired queued batch member. Fixed the same way `claimNext` (just above it in the same file) already worked around the identical gotcha: `RETURNING id` only, then a proper re-select through the query builder.
  - **A schedule's own first-fire jobs never got `scheduleId` or `trigger: 'schedule'`**, found while writing `schedules/runner.test.ts` against G10's own literal wording ("every run trigger = 'schedule'"): `createBatch` (called by the schedule's first fire) had no `scheduleId`/`trigger` override at all, so those jobs would never appear in `GET /api/schedules/:id/jobs` and their first run read `trigger: 'batch'`. Added optional `scheduleId`/`trigger` fields to `CreateBatchInput` (default `null`/`'batch'`, unchanged for every other caller — run-script, the pacer, plugin actions) and had `schedules/runner.ts`'s first-fire call pass `scheduleId: schedule.id, trigger: 'schedule'`.
  - **Plan 211 §7.1 names `packages/core/src/clusters/status.test.ts`**; that path was renamed to `packages/core/src/groups/status.test.ts` by plan 207, before plan 211 was written against a stale reference. Ran the real file; noted above.
  - **§4.1.1's column table vs. `createdBy`**: the plan's own column table does not list a `createdBy` column for `jobs`, but the code already carries one (added by an earlier plan for audit/attribution) — kept as-is (an addition, not a removal plan 211 owns), threaded through the new `job_runs`/migration exactly like every other pre-existing column.
  - **The remote/node job bridge's `runId: jobId` simplification**: `daemon.ts`'s remote job bridge (device/tunnel-owned jobs dispatched to a cloud node) and `packages/node/src/hosts.ts`'s own dispatch have no local `job_runs` row at all — a node-owned job is a `JobsCall`-shaped wire concept, not a core-DB-backed one. Both now pass `runId: jobId` consistently into every re-keyed call site (`createDbArtifactSink`, `job.artifact`/`job.log` broadcasts, `traceRecorder`) — a pragmatic, documented equivalence rather than inventing a parallel run-tracking scheme for a job the local `job_runs` table was never going to see.
  - **`createWorkflowBatch`'s placeholder `scriptId`**: `batches.scriptId` is `NOT NULL`; a workflow batch (no script at all) writes `'workflow:<name>'` as a placeholder so the column's constraint holds. Not named by the plan; a necessary consequence of `run-workflow` reusing the `batches` table.

- **Observed, not done**:
  - **Studio's smallest-edits-that-compile (plan §5 step 211.13) went further than "smallest"**: `jobs/detail/page.tsx`'s node timeline and resume dialog, and `schedules/detail/page.tsx`'s Runs tab, were substantively rewritten against `WorkflowStepInfo`/`GET /api/schedules/:id/jobs` (not just patched to compile) because the underlying data model changed shape, not just field names. Two capabilities were dropped in the process, not silently: per-step artifact attribution (artifacts are run-scoped only now, no `nodeId`) and the per-step attempt counter (`workflow_steps` carries neither) on the Jobs detail page; the schedule Runs tab lost per-fire outcome/detail/missed-count columns (the `schedule_runs` ledger they came from is deleted) in favor of a member-jobs list, with the last fire's own outcome shown separately.
  - **`schedules/runner.test.ts` is a deliberately scoped replacement**, not a port, of the deleted 1018-line pre-211 suite (permitted by plan 200 §8.3: a test whose fixtures assert a structurally impossible shape is rewritten to what the plan's own goal checklist names). It proves G10's two named tests plus three adjacent behaviors (first fire creates the batch/jobs; a device joining later gets its own job; `onOverlap: queue` also adds a run). **Real, separate testing gaps left open**: jitter (`pickJitterMs`), startup catch-up (`runStartupCatchUp`), agent-target schedules (`ScheduleAgentDispatch`), `onOverlap: 'cancel-previous'`'s `stopBatch` integration, and the scheduled-agent spend/concurrency ceilings — all present in the deleted suite, none re-proven here.
  - **53 test files were deleted rather than rewritten** (plan 200 §8.3, listed in the `1deb794` commit and this section): `api/{actions,artifacts,batches,devices,jobs,plugins,runtime-override-wiring,schedules}.test.ts`, `capability/job-trace.test.ts`, `daemon-wiring.test.ts`, `db/job-events.test.ts`, `db/migrations/{artifacts-device-scope,schedule-target-backfill}.test.ts`, `device/{crash-watcher,lifecycle,transfer}.test.ts`, `groups/{dispatch,dispatch-batch-max-concurrent.integration,pacer}.test.ts`, `jobs/{batch-dispatch-version-gate,executor-host-progress,jobs-runner-port,memory-limit.integration,peak-rss.integration,plugin-execution.integration,scheduled-batch-version-gate,script-jobs,triggers,trigger-runner.integration}.test.ts`, `jobs/executors/pull.test.ts`, `jobs/trace/{frame-store,recorder}.test.ts`, `maintenance/retention.test.ts`, `plugins/{action-executor,runtime}.test.ts`, `scripts/{param-sets,publish-result-e2e,routes}.test.ts`, `server/presence.test.ts`, `server/ws-handlers-{activity,agent-cancel,clipboard,inspect,job,monitor,recording,shell,tap-hold,text,touch,transfer}.test.ts`, `server/ws-handlers.observability.test.ts`, `services/job-service.test.ts`. None is named in plan 211 §7.1; each asserted a shape (`jobs.status`, `JobStore.finish`/`.enqueue`, `ArtifactInfo.jobId`, `JobService.nodes`/`.resume`, `BatchDispatchDeps` missing `runs`, …) this plan removed. This is a real, wide testing-coverage gap for `packages/core` — job-service, the actions/batches/devices/plugins REST surfaces, ws-handlers, the pacer, device lifecycle, and the trigger/plugin-execution integration tests all have zero coverage after this plan, until a later pass rewrites what's worth keeping.
  - **`packages/core/README.md`** still documents the pre-211 architecture (`job_nodes`, `job_resumes`, `schedule_runs`, per-node execution model) in its own prose sections (`### job_nodes`, the resume/lineage section, the retention cascade description) — not updated by this pass; a real, separate follow-up.
  - **`packages/sdk/README.md`'s "workflow node" section** was read and left alone on purpose: it documents `WorkflowDoc.nodes[]`, the document's own never-renamed building-block vocabulary from plan 210, not the `job_nodes` execution table this plan removed — no change needed, confirmed by re-reading it during the GREP_211 triage above.
  - Did not run `bun test` bare, `bun run --cwd packages/studio test`, or two invocations at once, per CLAUDE.md.

- **Open questions hit**: none of plan 211 §9's open questions blocked a step in this pass; every step the plan's §5 names was reachable without deciding one.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` → empty (nothing left running).
