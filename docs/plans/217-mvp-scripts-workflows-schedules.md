...
- **A schedule owns one batch; every fire adds a run to every member job** (plan 211 §3.2 decision 4, MVP 14 §1). *Superseded 2026-09-15: every fire now creates its own paced batch and `schedules.batch_id` names the newest (spec §4.7).* `GET /api/schedules/:id/jobs` (plan 211 §4.8) answers `JobsPageResponseSchema` — the same shape `JobsList` already renders elsewhere in Studio, fed here through its existing `fetchPage` override prop (`packages/studio/src/components/JobsList.tsx:108`, already present today, unrelated to plans 210/211/216).
