...
> Status: partial — implemented 2026-09-04 by the executing agent; software done, G16 open (blocked on plan 217), G9/G10/G11/G12/G17 need the owner's farm, and four goals (G2/G3/G7/G8/G13) are met in substance but not by their literal grep — see §11
...
| G4 | `check-routes.ts` passes with both rows pruned | exit 0 and no stale exemption | `grep -nE "'/batches'\|'/schedules'" scripts/check-routes.ts` prints nothing; `bun run scripts/check-routes.ts` exits 0 | [x] |
| G5 | The five timeline logic modules survive and the five prototype trace components are gone | `timeline/useTracePlayback.ts` and `timeline/lane-math.ts` (renamed from `lanes.ts`, §11) exist; `components/jobs/trace/` does not | `test -f packages/studio/src/components/jobs/timeline/useTracePlayback.ts && test -f packages/studio/src/components/jobs/timeline/lane-math.ts && test ! -e packages/studio/src/components/jobs/trace` exits 0 | [x] |
| G6 | The four surfaces the old detail page owned are gone as separate panels | 0 matches | `grep -rnE "JobResultSection\|JobFailureDetail\|JobLogsPanel\|JobArtifactsPanel" packages/studio/src` prints nothing | [x] |
...
| G14 | The workspace typechecks | 0 errors | `bun run typecheck` exits 0, every package `OK` | [x] |
| G15 | Plan 204's token script still passes after the icon additions | prints `design tokens ok` | `bun run scripts/check-design-tokens.ts` exits 0 | [x] |
| G16 | The shared jobs table has no importer left, and is deleted | file gone | `test ! -e packages/studio/src/components/JobsList.tsx`; `grep -rn "components/JobsList" packages/studio/src` prints nothing. If step 218.12's precondition grep names a surviving importer, this row stays open and §11 records the importer instead | [ ] (open — `app/scripts/detail/page.tsx` still imports it, plan 217's to close) |
...
† See §11 "Discrepancies" — the command was run and its literal output does not match the row's stated expectation, but the underlying goal is met; each is explained there rather than silently checked.
...
- **Checklist**: G1 ✅ · G2 ✅(see discrepancy) · G3 ✅(see discrepancy) · G4 ✅ · G5 ✅ · G6 ✅ · G7 ✅(see discrepancy) · G8 ✅(see discrepancy) · G9 ⏳ owner · G10 ⏳ owner · G11 ⏳ owner · G12 ⏳ owner · G13 ✅(see discrepancy) · G14 ✅ · G15 ✅ · G16 open (blocked on plan 217) · G17 ⏳ owner (code path implemented)

- **Commits** (branch `worktree-agent-acf31f2a703dfb9be`, merged onto `mvp` tip `e6e86b4` first with `git merge mvp --no-edit`, fast-forward, no conflicts): `b8dd30c` pure modules (job-view, json-nodes, json-diff, timeline/lanes) and icons; `78ddedf` run-scoped data layer, move `useTracePlayback`, delete the five prototype trace components; `9540ecc` chrome (JobsTabStrip, JobsSidebar, DetailHeader, RunPicker, SubTabs); `9dcff58` the five bodies (JsonSnapshot, LogsTab, ArtifactsTab, the four timeline cards); `568939e` steps/batches/compare (WorkflowSteps, BatchDetail, RunCompare); `7e6dc0a` the screen itself (JobsScreen, JobDetail), delete `app/jobs/detail/` and the four old panels plus orphaned `JobDetailPanel.tsx`; `142b7cd` every link through `jobHref`/`batchHref`, delete `app/batches/`, `DeviceJobs.tsx` re-run now adds a run, the `?device=` effect on `DevicesScreen.tsx`; `dfb2abf` prune `/batches` from `check-routes.ts`, clean stale route-string comments; `e59817d` typecheck fixes (action params, null guards, `lanes.ts` → `lane-math.ts` rename); `36cc385` widen `check-design-tokens.ts`'s icon group 3; `2f05975` fix stale `JobResultSection` comments in `plugin-view/`.

- **Typecheck**: `bun run typecheck` (after `bun install`, which this worktree needed — `node_modules` was absent) exits 0, all 19 workspace packages `OK` (protocol, ui, adb, toolchain, drivers, scrcpy, sdk, session, harness, core, node, studio, probe-server, networking, proxy-manager, tiktok-automation-pack, mikrotik-routing, google-automation-pack, youtube-automation-pack, examples).

- **Build**: `bun run build:studio` succeeds — static export, 25 pages, `/jobs` present at 22.2 kB, `/batches` and `/schedules` absent from the route list.

- **Design tokens**: `bun run scripts/check-design-tokens.ts` → `design tokens ok` (after widening `GROUP_3` in that script to the five new icon names, per its own doc comment's instruction).

- **Routes**: `bun run scripts/check-routes.ts` → `routes ok: 6 in nav, 7 exempt`.

- **Tests run**: none. Plan 200 §8.3 and this plan's own §7: zero tests belong to Studio or `@enkaku/ui`; nothing here is on the backend critical list.

- **Removed, proven**:
  - `app/jobs/detail/`, `app/batches/` (whole directories) — `test ! -e packages/studio/src/app/jobs/detail && test ! -e packages/studio/src/app/batches` exits 0.
  - `app/schedules/` — already gone before this plan ran (see discrepancy below); `test ! -e packages/studio/src/app/schedules` exits 0.
  - The five prototype trace components (`TracePanel.tsx`, `TraceScrubber.tsx`, `TraceTimeline.tsx`, `TraceFrame.tsx`, `TraceEventDetail.tsx`) and the four job-detail panels (`JobResultSection.tsx`, `JobFailureDetail.tsx`, `JobLogsPanel.tsx`, `JobArtifactsPanel.tsx`) — both greps in G5/G6 print nothing.
  - `components/jobs/JobDetailPanel.tsx` — not named by the plan's own file table, but orphaned once `app/jobs/detail/` and `app/batches/` were deleted (its only two importers); deleted as part of step 218.7/218.9 and recorded here.
  - `JobWithPhase`/`outcomeLine` from `lib/jobs.ts`, `JobNodeInfo` (already absent from `lib/api.ts` before this plan ran) — `grep -rnE "JobWithPhase|outcomeLine|JobNodeInfo" packages/studio/src` prints nothing.
  - `'/batches'` row from `scripts/check-routes.ts`'s `PENDING_REMOVAL` — `grep -nE "'/batches'|'/schedules'" scripts/check-routes.ts` prints nothing (the `/schedules` row was already absent — see discrepancy).

- **Discrepancies between plan and code**:
  1. **`/schedules` was already gone.** Step 218.13 (§3.9, §9 Q1) expected either this plan or plan 217 to delete `app/schedules/` and its `check-routes.ts` row. Neither directory nor row existed when this plan started: `git log --diff-filter=D -- packages/studio/src/app/schedules` shows plan 216's merge commit (`c3b49de`) already removed it in round R5, before this plan's round. Step 218.13 was therefore a true no-op; §9 Q1 is resolved de facto in plan 216's favour, not 217's or this plan's.
  2. **G2's literal grep finds zero lines, not one in `DetailHeader.tsx`.** The plan's own §4.9 composes `<RunPicker …/>` inside `JobDetail.tsx` (as the first segment of the `meta` node passed to `DetailHeader`), matching a local `import { RunPicker } from './RunPicker'` — which contains no substring `jobs/RunPicker`. `DetailHeader.tsx` is a dumb, reusable shell (`meta: ReactNode`) that never imports `RunPicker` at all; that composition choice is the plan's own (§4.7's doc comment: "`meta` is a node, not a string, because its first segment is the run picker"). The goal — one file, one importer, mounted only where the header's meta line is built — is met; the literal grep pattern just assumed a deeper relative path than the file layout produces. Verified narrowly: `grep -rn "from '@/components/jobs/RunPicker'\|from '\./RunPicker'\|from '\.\./RunPicker'" packages/studio/src` → exactly one line, `JobDetail.tsx`.
  3. **G3's literal grep is not zero, and cannot be while `/api/batches` calls exist.** `grep -rnE "/jobs/detail|/batches|/schedules" packages/studio/src scripts/check-routes.ts` prints 17 lines — every one is either a real `GET/POST /api/batches/...` backend call (required: `BatchDetail.tsx`, `JobsSidebar.tsx`, `use-job-counts.ts`, `operations.ts`, `bulk/*`, which plan 218 does not own) or accurate prose mentioning that API. The pattern cannot distinguish the deleted Studio PAGE route `/batches` from the live backend ROUTE `/api/batches`, whose substring it also matches. A narrower, address-shaped check — `grep -rnE "href=\{?\`?/(jobs/detail|batches|schedules)['\`\"?]|router\.push\(\`?/(jobs/detail|batches|schedules)|window\.location\.assign\(\`?/(jobs/detail|batches|schedules)" packages/studio/src` — prints nothing: no `href`, `router.push`, or `window.location.assign` in Studio addresses either deleted route. One stale comment (`components/bulk/use-batch-report.ts:13`, mentioning the now-deleted `app/batches/detail/page.tsx` by path) was left alone as a file plan 216 owns, not touched beyond what G3 required elsewhere.
  4. **G7's `GREP_218` is not zero: it matches "clusters" twice**, in `timeline/Lanes.tsx`, both quoting the design handoff's own words ("Logs (grey `var(--border-3)` clusters)") for the Logs lane's visual density blocks — an ordinary English plural, not the forbidden vocabulary sense (a device cluster, pre-groups). No forbidden word in its real sense appears.
  5. **G8's `GREP_218_COLOUR` is not zero: it matches once**, in `job-view.ts`'s own doc comment for `STRIPE`, which quotes the literal forbidden string (`` `bg-[--color-x]` ``) to explain why the constant uses an inline style instead — the same self-referential pattern `CLAUDE.md` itself uses. No file actually writes that class form.
  6. **G13's grep is not zero: it matches once**, `useTracePlayback.ts`'s `setInterval` (line ~153), moved unchanged per §3.6/step 218.2's explicit "do not rewrite while moving it." It advances a local `playheadMs` state on a 100 ms tick during playback; it schedules no network request. G13's own prose ("no request on a timer") is satisfied; its grep pattern does not distinguish a UI-animation timer from a polling one.
  7. **`components/jobs/timeline/lanes.ts` was renamed to `lane-math.ts`.** Plan §4.2/§4.10 name the pure module `lanes.ts` beside the component `Lanes.tsx`; TypeScript's module resolution refuses the pair as a same-directory, differently-cased collision (`error TS1149: File name '.../Lanes.tsx' differs from already included file name '.../lanes.ts' only in casing`), even on this case-sensitive filesystem. Renamed to `lane-math.ts`; all four importers (`Lanes.tsx`, `Transport.tsx`, `FrameStrip.tsx`, `FrameAndEvent.tsx`) updated. G5's row and command were updated to match.
  8. **`runOnDevice` used instead of a bare `runAction` for Re-run.** §4.9.1 says the Re-run action "posts `runAction('run-script', …)`" and later "On `ActionRefusedError`, `toast.error`" — but `runAction` itself never throws `ActionRefusedError`; only `runOnDevice` (the single-device wrapper in `lib/actions.ts`, which already implements the accepted/warned/forbidden state machine the plan's own prose describes) does. Since a job's Re-run always targets exactly one device (`job.deviceId`), `runOnDevice` is the correct call, and was used in `JobDetail.tsx` and (for consistency) `DeviceJobs.tsx`.
  9. **`ActionParams<'run-script'>`/`ActionParams<'run-workflow'>` require `concurrency`/`order`/`workflowName` even for a `jobId` re-run.** The core route ignores them once `jobId` is set (`packages/core/src/actions/run.ts` — a `jobId` re-run never reaches the batch/fan-out code that reads them), but the TS type is the Zod-inferred OUTPUT shape (post-`.default()`), which is not optional. `concurrency: 0, order: 'as-listed'` are supplied for `run-script`; `run-workflow`'s `workflowName` reads `job.scriptName ?? ''` (a workflow job's `scriptName` is denormalised from its workflow name — `packages/core/src/db/schema.ts`'s own comment — since `JobInfo`/`JobDetail` carry no separate `workflowName` field on the wire, unlike the plan's §4.9.1 table implied).
  10. **`jobs.createdBy` does not exist.** §4.8's own prose already flags this ("Started-by: `JobRunInfo` carries no `createdBy`... The 'who' half is `jobs.createdBy`") but the field does not exist on `JobInfoSchema`/`JobDetailSchema` either — checked directly against `packages/protocol/src/messages/job.ts`. `RunPicker`'s popover therefore shows `trigger` alone per run, with no "who" segment anywhere (not even once, on the meta line, as §4.8 proposed) — there is nothing on the wire to render. Recorded here in addition to §9 Q4 because the gap is one level deeper than the plan's own note assumed.
  11. **`byteLength` did not exist in `@enkaku/ui`.** §4.10's `JsonSnapshot` spec calls `byteLength(value)` as a fallback for a missing `bytes` prop, but no such export exists anywhere in the codebase. Added as a small pure helper in `job-view.ts` (`new TextEncoder().encode(JSON.stringify(value)).length`) rather than widening `@enkaku/ui`, since it is used by exactly one screen.
  12. **`jsonNodes` does not emit a synthetic row for the top-level container.** §4.10 does not specify this either way; a wrapping "root" row with an empty key would be one indent level of pure noise for the common case (`job.params`/`run.result` already being the object whose fields the reader wants). Implemented so a top-level object/array's own entries start at depth 0, and only a top-level primitive gets a single row.
  13. **`DeviceJobs.tsx`'s Re-run now adds a run**, closing the note plan 215 left in that file ("when plan 211 lands, this file's Re-run handler is the one call site to revisit") — done per this plan's own launch instructions, not named in §4 or §5 of the written plan, and recorded here as an edit outside the plan's own file table.
  14. **Additional link call-sites plan §4's own table (218.8) did not list**, found by re-running its grep per §2.6: `components/device-control/DeviceJobs.tsx`, `components/actions/verb-dialogs.tsx`, `components/actions/ActionOutcome.tsx`, and `components/JobsList.tsx` (two literal links plus two comments) all still addressed `/jobs/detail`. All four fixed to `jobHref`/`batchHref` or reworded comments. `components/CrashesPanel.tsx`'s link was additionally converted from a plain `<a>` to `next/link` while being touched, per `CLAUDE.md`'s rule that a plain `<a>` remounts React.
  15. **`node_modules` was absent in this worktree** (a fresh linked worktree, never `bun install`ed); `bun install` was run once before typecheck/build, adding no dependency changes (`bun.lock` untouched by the install itself, per `bun install`'s output showing 940 packages resolved from the existing lockfile).

- **Observed, not done**:
  - **The gate-verdict sentence** ("scroll1.videos (12) >= 10 -> continue") is not rebuilt; `WorkflowSteps.tsx` shows the recorded `verdict` value inside a collapsed `<details>` behind a `JsonSnapshot`, exactly as §3.11/§9 Q5 anticipated. Restoring the sentence needs the workflow document fetched on every workflow job's Timeline tab; the CEO/CTO call from §9 Q5 was not made here.
  - **A queued run's "position N" is not shown** (§9 Q3): `jobSubLine` reads `queued <relative time>` because neither `JobInfo` nor `JobRunInfo` carries a queue position.
  - **No server-side export route** (§9 Q6): Export is entirely client-side JSON assembly of what the screen already holds.
  - **`docs/spec.md`** needed no edit for step 218.14: plan 202's rewrite already describes the Jobs screen in the present tense (§13's "Jobs." bullet, §12 "Jobs and runs") and carries none of the old §19 rows ("Job / run detail", "Clusters, batches, schedules", the node timeline, `/jobs/detail`) this step would otherwise rewrite — confirmed with `grep -nE "Clusters, batches, schedules|node timeline|/jobs/detail" docs/spec.md`, which prints nothing.

- **Open questions hit**:
  - **Q1** (who deletes `/schedules`): resolved de facto by plan 216, before this plan ran (discrepancy 1 above).
  - **Q2** (are two Batches sub-tabs right): built as specified (Inputs, Members); not decided further here, per the rule against deciding open questions.
  - **Q3** (queue position): not added; recorded above under "Observed, not done".
  - **Q4** (per-run actor): deepened by discrepancy 10 above (the job-level field doesn't exist either, not only the run-level one).
  - **Q5** (gate verdict sentence): not rebuilt; recorded above.
  - **Q6** (server-side export route): not built; recorded above.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` → no output; nothing was started (no `bun run dev`/`dev:studio` invoked during this execution).
