...
> Status: partial — the registry, publish/verify/activate pipeline, and fault isolation are real and tested; live job execution of a plugin-bundled script is not wired end-to-end (see below). `@enkaku/sdk`'s `definePlugin`/`isPlugin` (`plugin.ts`) validates id shape, semver, at least one script, unique member ids, and a member version matching the plugin's, stamping the plugin's version onto every member — deliberately NOT by making `ScriptDefinition.version` optional in `types.ts` (owned by a concurrent plan in this pass); a plugin member is instead authored against the new `PluginMemberScript` type (`Omit<ScriptDefinition, 'version'> & { version?: string }`) and `definePlugin` returns a real, fully-typed `ScriptDefinition[]`. Schema: `plugins` table plus `scripts.pluginId`/`.exportId` plus `jobs.scriptName`/`.scriptVersion` (migrations `0038_curly_marauders.sql`, `0039_ambitious_karen_page.sql` — both hand-checked, pure `CREATE TABLE`/`ALTER TABLE ADD`/`CREATE INDEX`, no rename, no data loss). `packages/core/src/scripts/registry.ts`'s `ScriptRegistry` is the merge point (§3.3): `resolve()` wraps `resolveScriptRef` unmodified for the persisted half, overlays `plugins/dev-slots.ts`'s in-memory slots for the dev half, and — a gap the plan itself did not specify — translates a plugin-scoped `@latest` to the plugin's ACTIVE version before ever calling `resolveScriptRef` (whose own `@latest` only knows "highest enabled semver" and would otherwise pick a `superseded` version's number back up after a rollback); an exact pinned reference is untouched either way, which is what makes `superseded` still resolving pinned refs true. `plugins/runtime.ts`'s `PluginRuntime` (stage/verify/activate/rollback/disable/remove/reload/restart/dev slots) is real: `activate`'s CAS is a `WHERE status = 'staged'` UPDATE inside `db.transaction`, tested with two real overlapping calls (criterion 9); a name conflict is detected by looking up the OWNING PLUGIN of a colliding `scripts` row and comparing its `name` (not its row id — the first version of this check was wrong and is now regression-tested: a new version of the SAME plugin republishing the same script names is not a conflict, only a genuinely different plugin — or a standalone script — already owning the literal name is, criterion 23). `plugins/verify-child.ts` spawns a REAL bounded child (`verify-child-entry.ts`, dispatched via `packages/core/src/index.ts`'s new `--plugin-verify` flag mirroring `--job-child`) that imports a staged bundle and reports its shape; a hang is killed at its timeout (default 15s), a throw is reported verbatim — both real child processes in the test, not simulated. `scripts/bundle-cache.ts` is re-keyed on the bundle's own sha256 alone (`materializeBundle`/`materializeBundleText`), so twenty scripts sharing one plugin bundle write ONE file, proven with 20 real `Bun.write`s to a temp dir. `@enkaku/session`'s `child-entry.ts` now selects a plugin bundle's member by `ENKAKU_SCRIPT_EXPORT_ID` (an env var, not a new IPC field) and merges the plugin's own `reset.packages` with the selected member's — proven by spawning the REAL `child-entry.ts` against a real plugin bundle and reading its `ready` IPC message; a pre-plan bundle (no `scripts` array) is unaffected (criterion 27). `api/plugins.ts` (mounted at `/api/plugins`) covers stage+verify-in-one-call, activate, rollback, disable, reload, restart, remove (`?deleteKv=`), and the dev slot lifecycle (`POST/DELETE /api/plugins/dev`, both front-ends — a workspace `entryPath` or a pre-built `bundle`), reusing `script.publish`/`script.delete` rather than a new permission (§3.5's own words). Five of the plan's 8 `resolveScriptRef` call sites now go through the registry (`daemon.ts`'s `findScript` closure, `capability/context.ts`, `schedules/runner.ts`, `api/schedules.ts` at all four of its own call sites, `api/jobs.ts` via its existing injected-function seam) — each individually tested, including a schedule refusing a dev-only target with `script_is_dev` (criterion 18) and dispatching a published plugin script exactly like a standalone one; the remaining 15 direct `scripts`-table reads (§3.3) were migrated only where reachable outside the excluded directory (see below) — `scripts/routes.ts`/`service.ts` were deliberately left untouched (plugin rows already appear correctly there, being ordinary `scripts` rows; only a DEV row is invisible to them, which is `/api/plugins`'s job). `docs/spec.md` §11.6 states the plan's own naming decision (`definePlugin` vs `defineAgentPlugin`) in the product spec, not just this plan doc. `bun run typecheck` is 11 of 12 packages OK; `packages/core` fails on exactly one pre-existing-pattern line in an EXCLUDED file (see Not done). Root `bun test` is 2369 pass / 0 fail (baseline 2240 + 129 net new, plus two pre-existing migration-snapshot tests in `db/migrations/backfill-schedule-refs.test.ts` fixed to seed via raw SQL instead of the Drizzle query builder, which broke the moment `scripts` gained new columns — a structural fragility of that test's own historical-snapshot style, not specific to this plan). `packages/studio` is unchanged at 312 pass / 0 fail — no Studio UI shipped this pass (see Not done). `scripts/check-harness-provenance.sh` exits 0 (`packages/harness/src` untouched). **Not done, and why:** (1) **End-to-end execution of a plugin-bundled script through a real queued job.** `packages/core/src/jobs/` (executor-host.ts, executor.ts, executors/script.ts, executors/remote.ts, validate-script.ts) and `packages/session/src/runner/job-runner.ts` were BOTH hard-excluded for this pass (a concurrent plan's exclusive territory) — and those are exactly the files that would pick a `ScriptRegistry` entry's `exportId`, materialise ITS bundle, and set `ENKAKU_SCRIPT_EXPORT_ID` on the spawned child's env. `child-entry.ts`'s selection logic is real and tested by spawning it directly; `registry.bundlePath()` is real and tested; the missing link is entirely inside the two excluded files. Criteria 3, 6 (its "let it run" half), 16, and 26 (the "does not disturb a RUNNING job" half) are consequently not provable end-to-end and are marked accordingly below. (2) The CLI (`enkaku publish` detecting a plugin entry, `--stage-only`, a new `enkaku dev` command) — step 12 — was not built; a plugin author must call `/api/plugins`/`/api/plugins/dev` directly today. (3) Automatic dev-slot rebuild triggered by a workspace WRITE (front-end A's "no file watcher" — the write itself should signal the rebuild) was not wired into `workspace/store.ts`; `POST /api/plugins/dev` with `entryPath` builds on demand (tested end-to-end against a real `WorkspaceStore`, including editing a shared helper and proving the rebuilt bytes changed — criterion 15), but nothing calls it automatically on every subsequent write. (4) Studio (Plugins page, Scripts page Plugin column/origin filter, `RunScriptDialog` grouping — criteria 11's UI half, 29, 30) — a deliberate scope cut given the size of the backend surface actually needed to be correct and tested, the same call plan 79 made for its KV panel. (5) A daemon-boot integration test (criteria 20-24, 26's full form) — the guarantee itself (`PluginRuntime`/`verify-child.ts` never throwing) is tested directly and thoroughly; a real `createDaemon().start()` asserting `/api/health` beside a deliberately broken plugin fixture was not attempted. (6) `queue/job-store.ts`'s `scriptNames()` and `api/topology.ts` were left as direct table reads — correct for plugin (published) scripts, which are ordinary rows, but blind to a dev-origin id; `jobs.scriptName`/`.scriptVersion` denormalisation (criterion 13) IS wired at enqueue through `services/job-service.ts`'s new `scriptNameOf`, so a job's name/version survive a dropped dev slot at the RECORD level even though nothing can yet actually EXECUTE a dev job (see (1)).
