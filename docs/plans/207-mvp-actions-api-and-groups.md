...
- Test file: `packages/studio/src/lib/actions.test.ts` (`runAction` posts `{ target, ...params, force }` to `/api/actions/<verb>`; `awaitOperation` polls until `settled`; `runOnDevice` throws `ActionRefusedError` on `forbidden`, toasts once and re-sends with `force: true` on `warned`, returns the `done` result), `packages/studio/src/components/actions/ActionResults.test.tsx` (one row per result, the message shown), and each existing test that mocked a deleted route, run one file at a time: `components/FilesPanel.test.tsx` (if present), `components/ForgetDeviceDialog.test.tsx`, `components/BulkForgetDialog.test.tsx`, `components/DisconnectDeviceDialog.test.tsx`, `components/device/CutoverDialog.test.tsx`, `components/device/BulkCutoverDialog.test.tsx`, `components/BulkPrepDialog.test.tsx`, `components/InstallBatchDialog.test.tsx`, `components/BulkTransferDialog.test.tsx`, `components/network/BulkProxyDialog.test.tsx`, `components/RunScriptDialog.test.tsx`, `components/device-popup/AdbCommandDialog.test.tsx`, `components/device-popup/ActionsList.test.tsx`, `components/device-popup/DevicePopup.test.tsx`, `components/terminal/TerminalPane.test.tsx`, `components/guest-agent/NetworkRouteForm.test.tsx`, `components/AdmitDeviceDialog.test.tsx`, `lib/labelling.test.ts`, `lib/operations.test.ts`, `lib/use-preparation.test.tsx`, `app/page.test.tsx`, `app/device/page.test.tsx`, `components/operations/OperationTray.test.tsx`, `components/layout/AppShell.test.tsx` (the `/console` entry and the `app/console/page.test.tsx` mention at `:17`).
...
- Test file: `components/target/useTargetSelection.test.ts`, `components/target/TargetPicker.test.tsx`, `components/GroupEditorDialog.test.tsx`, `components/GroupMembersDialog.test.tsx`, `app/groups/page.test.tsx`, `components/ScheduleEditorDialog.test.tsx`, `app/schedules/detail/page.test.tsx`, `app/schedules/page.test.tsx`, `app/plugins/view/page.test.tsx`, `components/plugin-view/ViewRenderer.test.tsx`, `components/plugin-view/ActionRunner.test.tsx`, `components/DeviceCard.test.tsx`, `components/DevicePicker.test.tsx`, `components/device/DeviceHeader.test.tsx`, `components/device/DeviceNumberField.test.tsx`, `components/wall/{Wall,WallTile,WallTile.quarantine,DeviceContextMenu}.test.tsx`, `components/wall/tile-identity.test.ts`, `components/wall/useLiveSet.test.ts`, `components/TileChips.test.tsx`, `components/host/DeviceWallWithPicker.test.tsx`, `components/device-popup/{SidePanel,DevicePopup.escape,DevicePopup.escape-precedence}.test.tsx`, `app/page.wallNode.test.tsx`, `app/batches/page.test.tsx`, `app/batches/detail/page.test.tsx`, `app/scripts/page.test.tsx`, `packages/ui/src/components/device-picker.test.tsx`, each run one file at a time after its fixtures move from `cluster`/`clusterId` to `group`/`groupId`.
...
bun test packages/studio/src/lib/actions.test.ts
bun test packages/studio/src/components/actions/ActionResults.test.tsx
bun test packages/studio/src/components/device-popup/AdbCommandDialog.test.tsx
bun test packages/studio/src/components/target/useTargetSelection.test.ts
bun test packages/studio/src/app/groups/page.test.tsx
