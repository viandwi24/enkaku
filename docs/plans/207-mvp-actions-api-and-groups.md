...
> Status: implemented (software) — 2026-09-03/04, executed in worktree branch `worktree-agent-adfde09ff7a38a89c` (based on `main`). G1–G16 verified by the commands named in §0, including two test files this plan named but the branch had not yet created (`api/actions.test.ts`, `capability/actions.test.ts`) and a genuine gap found and closed while closing out (`network/route-service.ts` still mounted the five per-device `PUT/POST/DELETE /:id/network*` routes G7/§10 call for removed, and roughly a dozen Studio call sites this plan's own §4.9 table names had not yet been migrated to `runOnDevice`/`runAction`). G3 has one uncovered verb (`run-script`) and G13 has no Studio test file (CLAUDE.md's zero-Studio-tests policy, dated the same day as this plan, wins). G17 stays `owner` (needs the lab device). See §11 for the full handoff, discrepancies, and observed gaps.
...
| G3 | Every verb dispatches to the existing per-device implementation, never to an HTTP route | 26 verbs, each with a test that asserts the injected implementation was called with the device id | `bun test packages/core/src/api/actions.test.ts` → the `describe('verbs')` block has 26 passing tests | [x] (§11: `ACTION_VERBS` has 25 entries, not 26 — file wins. The `describe('verbs' …)` block has 24 tests: 23 real verbs dispatched and asserted against their fake leaf dependency, plus `run-workflow`'s own `E_NOT_SUPPORTED` refusal. `run-script` has none — it dispatches once for the whole candidate set through `batchesFor(actor)`, a per-request `BatchDispatchDeps` factory (the F10 fix, `run.ts`'s own doc comment) too large to fake usefully alongside the other 24 in one fixture; the gap is real, not covered elsewhere in this session. 29 tests total in the file, the other 5 across G2/G4's own describe blocks.) |
...
| G5 | The target resolver answers `deviceIds`, `groupId` and `tags` and reports unknown or unavailable devices as `skipped` | `resolveActionTarget` in `packages/core/src/groups/resolve.ts` | `bun test packages/core/src/groups/resolve.test.ts` passes (the three shapes, dedupe, unknown id, offline, quarantined, unknown group throws `group_not_found`) | [x] |
| G6 | `GET /api/operations/:id` returns the same array with final statuses and forgets it after one hour | `OPERATION_TTL_MS = 3_600_000`; `accepted` becomes `done` or `failed` when the dispatched promise settles | `bun test packages/core/src/actions/operations.test.ts` passes (fake clock) | [x] |
| G7 | No per-device action route survives in the device routers | the only `/:id/` write left in `packages/core/src/api/devices.ts` is `POST /:id/monitor/save` | `rg -n "app\.(post|put|delete)\('/:id/" packages/core/src/api/devices.ts` prints exactly one line, containing `'/:id/monitor/save'`; `rg -n "app\.(post|put|delete)\('/" packages/core/src/api/transfer.ts packages/core/src/api/device-preparation.ts packages/core/src/api/topology.ts packages/core/src/api/command-runs.ts packages/core/src/api/saved-commands.ts` → `No such file` for all five | [x] (§11: this goal's own command never checked `network/route-service.ts`, which still had `PUT/POST/DELETE /:id/network*` live, mounted into `guest-agent.ts`'s app tree — found while closing out, and fixed) |
| G8 | The bulk twins and the public enqueue routes are gone | 0 matches | `rg -n "'/labels/apply'\|'/prep/apply'\|'/network/apply'" packages/core/src` → empty; `rg -n "app\.post\('/', " packages/core/src/api/jobs.ts packages/core/src/api/batches.ts` → empty | [x] |
| G9 | The word cluster is gone from live code, with the one exception §10 names | 0 matches | `GREP_207_CLUSTER` (§10) prints nothing | [x] (2 additional necessary exceptions beyond the ones §10 names — see §11) |
| G10 | The console is gone: routers, runner, store, tables, messages, settings, page, components | 0 matches; three tables absent | `GREP_207_CONSOLE` (§10) prints nothing; `bun test packages/core/src/db/groups-migration.test.ts` → test `command_runs, command_run_members and saved_commands no longer exist` passes | [x] |
| G11 | `clusters` is `groups`, `devices.cluster_id` is `devices.group_id`, `batches.cluster_id` and `schedules.cluster_id` are `group_id`, and existing rows survive | one migration, generated by `db:generate` (rename answers) or hand-written per the `0023` precedent (§4.6) | `bun test packages/core/src/db/groups-migration.test.ts` passes (a pre-existing group, its member device, a batch and a schedule read back through the renamed Drizzle tables) | [x] |
| G12 | `/api/groups` replaces `/api/clusters` with no alias, and its member writes are verbs | routes `GET /`, `POST /`, `PATCH /:id`, `DELETE /:id`, `GET /:id/devices` only | `rg -n "app\.(get|post|patch|delete)\('" packages/core/src/api/groups.ts` prints exactly five lines; `rg -n "/api/clusters" packages` → empty | [x] |
| G13 | The old Studio compiles against the new routes through one client | `packages/studio/src/lib/actions.ts` exports `runAction`, `fetchOperation`, `awaitOperation`, `runOnDevice`, `ActionRefusedError` | `bun run typecheck` exits 0; `bun test packages/studio/src/lib/actions.test.ts` passes; `rg -n "/api/devices/\\$\{[^}]*\}/(install|push|pull|label/|preparation|connection/|network|block|unquarantine|cluster|tags|readiness)" packages/studio/src --glob '!*.test.*'` prints only the two reads (`/label` in `PhysicalLabellingPanel.tsx`, `SettingsPopup.tsx`, `DevicePopup.tsx`, `app/device/page.tsx`; `/network` in `lib/api.ts`'s `fetchNetworkStatus`; `/readiness` nowhere; `/preparation` in `lib/use-preparation.ts`'s GET only) | [x] (§11: no `lib/actions.test.ts` — CLAUDE.md's zero-Studio-tests policy, dated the same day as this plan, supersedes that one clause; typecheck and the grep both pass — the grep found roughly a dozen call sites the branch this plan builds on had NOT yet migrated, fixed here) |
| G14 | The capability broker exposes `actions.run` | capability id `actions.run`, input `ActionCapabilityInputSchema`, output `ActionResponseSchema` | `bun test packages/core/src/capability/actions.test.ts` passes; `rg -n "id: 'actions.run'" packages/core/src/capability/actions.ts` prints one line | [x] |
...
| G16 | The workspace typechecks | 0 errors | `bun run typecheck` exits 0 | [x] |
...
**Branch: `worktree-agent-adfde09ff7a38a89c`, based on `main`.**

- **Checklist**: G1–G16 all `[x]`, verified by running every command §0 names (not merely reading the code) — see §0's own table for the per-row detail added alongside each checkbox, including the two genuine discrepancies (G3's uncovered `run-script` verb, G7's own literal command missing the `route-service.ts` gap G10's table caught). G17 stays `owner` (needs the lab device, unavailable in this environment).

- **Commits** (this session; six commits on `worktree-agent-adfde09ff7a38a89c` carrying the `mvp-207`/`m07` prefix):
  1. `91f3b05` wip(mvp-207): actions API, groups rename, console removal — mid Studio rewiring (207.8/207.9)
  2. `2bab543` wip(mvp-207): groups rename sweep across Studio dialogs and pages
  3. `45a5d3e` feat(mvp-207): finish Studio rewiring — actions API + groups rename complete
  4. `5dac992` chore(mvp-207): close GREP_207_CLUSTER and GREP_207_CONSOLE to their named exceptions
  5. `29410c1` chore(mvp-207): plugin version bumps for the groups rename (207.7)
  6. `57bb48b` test(m07): actions API dispatch/policy coverage, actions.run capability, adb-stats stale-shape fix
  7. (this commit) fix + docs(mvp-207): close the network route-service gap, migrate ~a dozen remaining Studio call sites, status line + §0 checklist + this §11 report

- **Typecheck**: `bash scripts/typecheck.sh` — clean across all 19 packages (protocol, ui, adb, toolchain, drivers, scrcpy, sdk, session, harness, core, node, studio, probe-server, networking, proxy-manager, tiktok-automation-pack, mikrotik-routing, google-automation-pack, youtube-automation-pack, examples). Run repeatedly through this closing session (after each file edit batch) — clean every time it was checked.

- **Tests run** (this closing session; scoped, one file at a time, never concurrently):
  - `packages/core/src/api/actions.test.ts` — **new file**, 29 pass (G2/G3/G4)
  - `packages/core/src/capability/actions.test.ts` — **new file**, 4 pass (G14)
  - `packages/core/src/api/adb-stats.test.ts` — 10 pass, after fixing a pre-existing stale-shape bug in two tests (see Discrepancies)
  - `packages/protocol/src/actions.test.ts` — 42 pass (G1)
  - `packages/core/src/groups/resolve.test.ts` — 9 pass (G5)
  - `packages/core/src/actions/operations.test.ts` — 9 pass (G6)
  - `packages/core/src/db/groups-migration.test.ts` — 3 pass (G10/G11)
  - `packages/core/src/network/route-service.test.ts` — 57 pass (rewritten this session — see Discrepancies)
  - `packages/core/src/api/guest-agent.test.ts` — 96 pass (rewritten this session — see Discrepancies)
  - `packages/core/src/network/credential-reveal.test.ts` — 13 pass (regression check after the route-service.ts edit)
  - `packages/core/src/network/reverse-registry.test.ts` — 28 pass (regression check)
  - `packages/core/src/network/route-checks.test.ts` — 33 pass (regression check)
  - `plugins/tiktok-automation-pack` (`bun test ./src/index.test.ts` — bare relative path is rejected by this Bun version's filter matcher, `./`-prefixed works) — 34 pass (G15)

  Total: 13 files this session, 0 failures. Earlier-session runs (actions/verbs impl, daemon-wiring, http, mikrotik-routing, jobs/triggers, etc.) are recorded in the prior compacted summary and were not re-run here since their files were not touched again this session.

- **Removed, proven**: `GREP_207_CLUSTER` and `GREP_207_CONSOLE` (§10, exact commands, run via `/opt/homebrew/bin/rg` directly — the shell's own `rg` alias intermittently routed through plain `grep` and rejected `--glob`, a local environment quirk, not a code defect) both return only the named exceptions:
  - CLUSTER: `plugins/mikrotik-routing/src/index.ts:231` and `plugins/tiktok-automation-pack/src/index.ts:782` (changelog/comment prose citing the old module names, from the version-bump commits), plus **two additional, previously-undocumented exceptions** in `scripts/check-dead-code.sh:107,118` — these are DEAD-SYMBOL SEARCH STRINGS (`"ClusterSection|DevToolsPage"`, `"...TopologyCluster..."`) the script greps FOR, not live code; confirmed pre-existing via `git show f84e7ae:scripts/check-dead-code.sh`, predating this session entirely.
  - CONSOLE: only the accepted `ACTION_FANOUT_CONCURRENCY` citations (`verbs.ts`'s own doc comment already names this as the one unavoidable `fanout` hit; `run.ts`, `packages/protocol/src/settings.ts`, `packages/core/README.md`, `InstallBatchDialog.tsx` each cite the same constant) plus one **additional, previously-undocumented exception**: `packages/core/src/doctor/context.ts:123`, a literal factual example of Windows `tasklist`'s real CSV output (`"adb.exe","21440","Console",...`) — reworded, it would misrepresent third-party tool output, so left as-is.
  - `bun test packages/core/src/db/groups-migration.test.ts` passes (G10's own table row).
  - `rg -n "app\.(put|delete)\('/:id/network'|'/:id/network/(enable|disable|retry)'|'/network/apply'|applyRouteInBulk|BULK_SKIP_FOR_CODE" packages/core/src --glob '!**/*.test.*'` → empty (this row's own proof, restricted to non-test files the same way every other §10 row is — see Discrepancies for why the un-restricted form still shows one line).

- **Discrepancies between plan and code**:
  1. **G3/G7's literal commands undercount / undercheck.** `ACTION_VERBS` has 25 entries, not the 26 G1/G3 assume — `run-script` and `run-workflow` are both counted in the plan's "26", and the file is authoritative (`packages/protocol/src/actions.ts`). Separately, G7's own "Verified by" command only checks `devices.ts` plus five named files; it never checks `packages/core/src/network/route-service.ts`, which is where a REAL, previously-unnoticed gap was found this session (next item). §10's own removal-register table is more precise than G7's summary command and was the one that actually caught it.
  2. **`packages/core/src/network/route-service.ts` still had the five per-device network write routes live** (`PUT/POST/DELETE /:id/network*`), mounted into `guest-agent.ts`'s app tree via `app.route('/', service.routes)`, itself mounted under `/api/devices` — reachable on a real running core. The functions themselves (`setRouteFromRequest`/`clearRouteFromRequest`/`enableRoute`/`disableRoute`/`retryRoute`) had already been refactored into plain functions and exposed as `service.actions` (`NetworkActionsDoor`, consumed by the `set-network` verb) — only the OLD HTTP route registrations survived alongside the new door. Removed the five `app.put/post/delete(...)` registrations from `route-service.ts` (kept the function bodies and the surviving `GET /:id/network` read, per the "reads stay per device" rule). This broke `route-service.test.ts` (57 tests, all going through the deleted routes via `h.app.request`) and `api/guest-agent.test.ts` (its own ~80 calls to the same paths) — both fixed: `route-service.test.ts` got a small `{status, json()}`-shaped adapter (`asResponse`) wrapping direct `service.actions.*` calls, touching only 5 helper functions rather than every assertion; `guest-agent.test.ts` got a TEST-FILE-ONLY local Hono router (`withTestOnlyNetworkActionRoutes`) re-exposing the same 5 routes over `routeActions` purely so its ~80 pre-existing `app.request(...)` call sites (several depending on GET's own read-throttle-window ordering relative to a PUT/DELETE) needed no rewrite — this router is never mounted anywhere the real app tree can reach, so it does not reopen the surface plan 207 closes; confirmed by re-running G7's own proof command restricted to non-test files (empty) versus unrestricted (one line, inside `guest-agent.test.ts` only).
  3. **`packages/core/src/actions/run.ts`'s `disconnect` dispatch never forwarded `request.force` to `disconnectDevice`'s own internal job-running check** — found while wiring `DisconnectDeviceDialog.tsx`'s force checkbox: `disconnect`'s `policyKind` is `null` (VERBS table), so the outer warn/force layer never runs for it, and the impl's own `{}` opts meant a running job made disconnect UNCONDITIONALLY forbidden, with no way to force it — a real regression against the old `POST /:id/connection/disconnect { force }` route this verb replaces. Fixed: `dispatchSyncVerb`'s disconnect case now passes `{ force: request.force }`.
  4. **Roughly a dozen Studio call sites this plan's own §4.9 table names were still on the OLD per-device routes**, discovered via `rg -n -P 'api/devices/\$\{[^}]*\}/(install|push|pull|label/|preparation|connection/|network|block|unquarantine|cluster|tags|readiness)' packages/studio/src --glob '!*.test.*'` (G13's own command) — the earlier session's "Studio fully rewired" claim covered the majority of call sites but missed these: `ForgetDeviceDialog.tsx` (`forget`, `block`), `DisconnectDeviceDialog.tsx` (`disconnect`, plus the `job_running`→`E_DEVICE_CONFLICT` refusal-code rename the new verb requires), `CutoverDialog.tsx`/`BulkCutoverDialog.tsx` (`cutover` start/cancel), `PhysicalLabellingPanel.tsx` (`set-label`/`clear-label`), `TagEditor.tsx`/`AdmitDeviceDialog.tsx` (`set-tags`), `PreparationPanel.tsx`/`AgentAlertDetail.tsx`/`VpnAgentPrecondition.tsx` (`prepare`/`retry-prepare`), `lib/readiness.ts` (`wake`/`sleep`, with `'hot'` throwing `Error('hot is not an action')` exactly as this plan's own §4.9 table prescribes — plan 206 §9 Q3 owns `hot`, not decided here), `lib/api.ts`'s `postNetworkAction` (`set-network` op enable/disable/retry), `HttpProxyFields.tsx`/`VpnRouteFields.tsx` (`set-network` op set), `NetworkRouteForm.tsx` (`set-network` op clear), and `app/device/page.tsx`'s `releaseQuarantine`/`reconnectDevice`. All now call `runOnDevice`/`runAction` from `lib/actions.ts`. Every one of these was in this plan's own §4.9 table with the exact target verb already named — none of this was a design decision, only finishing wiring the plan itself already specified.
  5. **`packages/core/src/api/adb-stats.test.ts`'s stale-shape bug** (genuinely pre-existing, confirmed via `git show f84e7ae:...` — predates this session and plan 205's own narrowing of the `input` stats shape): two tests asserted the old, wider shape (`assistsActive`/`mirrorGroups`/etc.) instead of the real `{lanes:{...}}`-only shape `ZERO_INPUT`/`inputStats()` actually produce; masked because `typedJson()` never validates at runtime (`_schema` unused, confirmed by reading `api/typed-json.ts`). Fixed both assertions.
  6. `packages/core/src/api/actions.ts`'s `E_UNKNOWN_VERB` refusal answers 404 (confirmed against `ACTION_ERROR_CODES`), matching G2 exactly as written.

- **Observed, not done**:
  1. **`run-script` has no test in `api/actions.test.ts`.** It dispatches once for the whole candidate set through `batchesFor(actor)`, a per-request `BatchDispatchDeps` factory (the F10 security fix an earlier session made — see the doc comment on `ActionsDeps.batchesFor` in `run.ts`) that is too large to fake usefully alongside the other 24 verbs in one shared fixture without either a second, much larger fixture file or weakening the F10 guarantee the fixture is supposed to exercise honestly. Left as a genuine, named gap rather than a fake `batchesFor` that would let the test pass without proving anything.
  2. **`scripts/check-design-tokens.ts`'s G9 check fails**: `@enkaku/ui barrel does not export setCoreBase`. Confirmed pre-existing and unrelated to plan 207 — `setCoreBase` is mentioned only in a doc comment in `packages/ui/src/index.ts` at the branch's own merge-base with `main` (`8fb7b4bdb333c820080f6eb5c2d28b517662f974`), never as a real export, and the `check-design-tokens.ts` G9 check itself did not even exist yet at that commit (added later, by a concurrent plan). Not fixed — out of scope for actions/groups/console.
  3. **`ForgetDeviceDialog.tsx`'s `refusal.code === 'device_online'` branch ("Block instead" button) is dead** — the new `forget` verb's refusal codes are `device_busy`/`device_in_use` (`device/lifecycle.ts`'s `checkRemovable`), never `device_online`; this predates plan 207 entirely (plan 56 already removed the "refuse to forget a connected device" premise the dialog's own comment describes, per that file's own doc comment: "Plan 56 removed that premise… forget is allowed at any status"). Left as-is — an unrelated, already-stale affordance, not something this plan's own wiring change caused or is responsible for correcting.
  4. Everything the earlier compacted session already recorded as observed-not-done (see the prior summary) was not re-audited this session.

- **Open questions hit**: none decided. §9 Q1–Q7 (plugin-declared verbs, `settings` verb scope, `clear-cache`'s shell command, `screenshot` artifact retention, operation TTL/cap, `reprofile`, `?groupId=none`) were not touched. `hot`'s own place in `lib/readiness.ts` (§9 Q3, owned by plan 206) is implemented exactly as this plan's own §4.9 table already prescribed (`Error('hot is not an action')`), which is following the plan's written answer, not deciding the open question.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` — empty. No dev server, test runner, or other openpf-related process left running by this session.
