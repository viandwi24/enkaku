...
> **Plan 318 (2026-09-14) supersedes parts of this section.** The CLI no longer depends on the workflow for a farm that has its own whisper-cli: `ai.whisperCliPath` (Settings → AI) points at it, and the model is picked from four (`ai.whisperModel`). The doctor this plan lacked is `media.transcribe.check`. Item 1 below still holds for a farm that wants the MANAGED build; item 2 was already done (`api/cap.ts` maps all four codes); item 3's real smoke is `media.transcribe.check`'s run on the owner's Mac, recorded in plan 318 §11.
