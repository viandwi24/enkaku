...
> Status: implemented (software) — G9 to G12 are owner rows, open (no lab device in this environment); executed 2026-09-04 on branch `plan-208-inspector-phase-1`
...
| G13 | The bench prints the cold-attach rows | `--attach-cycles` flag; rows `ui-server attach cold p50`, `ui-server attach cold max` | `bun run scripts/bench-device-nfrs.ts --help` lists `--attach-cycles` | [x] |
| G14 | `bun run typecheck` is clean | 0 errors | `bun run typecheck` → exit 0 | [x] |
| G15 | The forbidden words of §10 are gone | 0 matches | `GREP_208` → empty | [x] |
...
- **Plan 129.** §0.1 (`docs/archive/plans/129-...md:14-34`): three attaches of 31 957, 32 010 and 31 986 ms on the owner's API 36 farm, then dumps failing in 5 ms with `Unable to connect`. Step 129.4 (`:219-220`): "Why ui-server does not start on Android 16, diagnosis, owner-gated"; suspicion on record: 2.3.3 on API 36.
...
Branch: `plan-208-inspector-phase-1`, cut from `mvp` at `ed87537` (fast-forwarded onto the `mvp` tip named in the task — plans 201–207, 213 already landed there).

- **Checklist**: G1 ✅ G2 ✅ G3 ✅ G4 ✅ G5 ✅ G6 ✅ G7 ✅ G8 ✅ G9 ⏳ owner (no lab device) G10 ⏳ owner G11 ⏳ owner G12 ⏳ owner G13 ✅ G14 ✅ G15 ✅
- **Commits**:
  - `561aa11` — wip(mvp-208): inspector lifecycle, mid step 208.12 (bench script)
  - `01ecee3` — wip(mvp-208): docs, GREP_208 clean, watchdog wording fixes
  - (this report and status line committed alongside the code, see the final commit on this branch)
- **Typecheck**: clean (`bash scripts/typecheck.sh` → every package `OK`, run repeatedly through the work, last run after all edits).
- **Tests run** (one file/directory at a time, per CLAUDE.md and plan §7; no bare `bun test`):
  - `bun test packages/drivers/src/inspector/ui-server/lifecycle.test.ts` → 15 pass, 0 fail, 45 expect
  - `bun test packages/drivers/src/inspector/ui-server/launcher.test.ts` → 31 pass, 0 fail, 74 expect
  - `bun test packages/drivers/src/inspector/ui-server/watchdog.test.ts` → 15 pass, 0 fail, 66 expect
  - `bun test packages/drivers/src/inspector/ui-server/client.test.ts` → 17 pass, 0 fail, 37 expect
  - `bun test packages/drivers/src/inspector/` (the whole directory) → 121 pass, 0 fail, 286 expect
  - `bun test packages/adb/src/client.test.ts` → 38 pass, 0 fail, 87 expect
  - `bun test packages/session/src/session.test.ts` → 36 pass, 0 fail, 91 expect
  - `bun test packages/session/src/inspector-factory.test.ts` → 2 pass, 0 fail, 10 expect
  - `bun test packages/session/src/device-executor.test.ts` → 45 pass, 0 fail, 91 expect
  - `bun test packages/session/src/runner/trace.test.ts` → 38 pass, 0 fail, 114 expect
  - `bun test packages/core/src/capability/context.test.ts` → 24 pass, 0 fail, 48 expect
  - `bun test packages/core/src/server/ws-handlers-inspect.test.ts` → 18 pass, 0 fail, 51 expect
  - `bun test packages/core/src/api/adb-stats.test.ts` → 11 pass, 0 fail, 19 expect
  - `bun test packages/core/src/doctor/` → 80 pass, 0 fail, 265 expect
  - `bun test packages/core/src/server/` (the whole directory — every other server test file this plan's `ws-handlers.ts` edit could have broken) → 139 pass, 0 fail, 361 expect
  - `bun test packages/toolchain/src/` — **not run**: step 208.11 (the manifest pin) did not execute (no lab device), so the manifest is untouched and this scoped check has nothing new to prove.
  - `bun run typecheck` → clean.
  - `bash scripts/check-dead-code.sh` → "no dead code found".
  - `bun run scripts/check-design-tokens.ts` → all sections "ok" (this plan touches no Studio/`@enkaku/ui` file, so this is an unaffected-baseline confirmation, not new coverage).
  - `bun run scripts/check-routes.ts` → "routes ok: 6 in nav, 11 exempt".
  - `bash scripts/check-plan-status.sh` → "every plan that declares an artefact agrees with the code".
  - Manual smoke (no lab device attached to this environment, so only the device-free half of §7's smoke ran): `ENKAKU_PORT=7791 ENKAKU_DATA_DIR=<scratch> bun run dev` booted cleanly, `curl http://127.0.0.1:7791/api/adb/stats` returned `"streams":{"maxStreams":8,"maxStreamsPerDevice":4,"active":0,"pinned":0,"perDevice":{}}` — the `pinned` field is present and reports `0` with no device attached, matching acceptance criterion 7's shape without a phone to drive `active`/`perDevice` any further. The process was killed and `ps -Ao pid=,command= | grep -i "[o]penpf"` confirmed clean before continuing (see below).
- **Removed, proven** (§10, `GREP_208` and every row's own proof, re-run after all edits):
  - `GREP_208 = rg -n "releaseInspector|inspectorRefCounts|detachInspector|ad-hoc dump|new UiautomatorDumpInspector\(" packages apps plugins scripts --glob '!**/*.test.ts' --glob '!packages/session/src/inspector-factory.ts'` → empty.
  - `rg -n "new UiautomatorDumpInspector\(" packages --glob '!**/*.test.ts'` → one line, `packages/session/src/inspector-factory.ts:67`.
  - `rg -n "inspectorRefCounts|detachInspector" packages/core/src` → empty.
  - `rg -n "releaseInspector" packages apps plugins scripts` → empty (the word survives only inside `*.test.ts`/`session.test.ts`'s own doc comments describing the removal, which the plan's own `--glob '!**/*.test.ts'` exemption excludes from the gate).
  - `rg -n "onData: \(\) => \{\}" packages/session/src/inspector-factory.ts scripts/bench-device-nfrs.ts` → empty.
  - `rg -n "was not ready within the start timeout" packages/drivers/src/inspector/ui-server/watchdog.ts` → empty (the silence reason is now `no ping answered within the ${startTimeoutMs}ms silence ceiling`, and the fatal-line reason is the launcher's own label).
  - `rg -n -i "ref-counted|last one leaves" packages/protocol/src/messages/inspect.ts packages/protocol/src/messages/device-event.ts` → empty.
  - `rg -n "TODO-M4.5" packages/toolchain/manifest/enkaku-tools.json` → **still present** (2 hits, lines 95 and 119) — step 208.11 did not run (no lab device); the pin stays at 2.3.3, recorded honestly rather than silently removed.
  - `rg -n "TODO-verify" packages/drivers/src/inspector/ui-server/client.ts` → empty.
- **Discrepancies between plan and code**:
  - **§4.4's proposed `DEFAULT_START_TIMEOUT_MS = INSTRUMENTATION_START_SILENCE_MS` (a value re-export from `lifecycle.ts` into `watchdog.ts`) would have created a genuine circular ES-module dependency**, not the type-only cycle the plan intends (`watchdog.ts` already exports the value `createWatchdog`, which `lifecycle.ts` imports; the plan's own re-export would have `watchdog.ts` import a value back from `lifecycle.ts`). Depending on which module a caller loads first, this risks a `ReferenceError: Cannot access '...' before initialization` on Bun's module loader for `const` exports in a cycle. Implemented instead: `watchdog.ts` keeps its own local `DEFAULT_START_TIMEOUT_MS = 15_000` (no import), and `lifecycle.ts`'s `INSTRUMENTATION_START_SILENCE_MS` stays independently defined at the same value `15_000`, with a comment on each explaining why they are not import-linked and that both must change together if either ever does. `createUiServerLifecycle` still forwards `opts.watchdog?.startTimeoutMs` through unchanged when a caller sets one; the two constants agree today by value, not by reference.
  - **The verified `ConfiguratorInfo` field names matched §4.5's assumption exactly** (step 208.4's own verification, done against the pinned `2.3.3` tag's real source before any code was written): `AutomatorService`/`AutomatorServiceImpl`/`ConfiguratorInfo.java` under `app/src/androidTest/java/com/github/uiautomator/stub/` declare `setConfigurator(ConfiguratorInfo)`/`getConfigurator()`, and `ConfiguratorInfo`'s public getters/setters are `waitForIdleTimeout`, `waitForSelectorTimeout`, `actionAcknowledgmentTimeout`, `scrollAcknowledgmentTimeout`, `keyInjectionDelay` — camelCase, no underscore, despite the private fields themselves carrying a `_` prefix (Jackson, via `jsonrpc4j` 1.5.0, serialises by the public accessor name, not the field name). `AutomatorHttpServer.java` routes through plain `com.googlecode.jsonrpc4j.JsonRpcServer` with no `@JsonRpcParam` annotations, confirming the positional-array `params: [info]` shape the plan assumed. No `uiAutomationFlags` on this tag, matching R5. **No discrepancy — the plan's §4.5 code block shipped unchanged.**
  - **The watchdog's own "not ready" reason string changed wording** (`ui-server was not ready within the start timeout` → `no ping answered within the ${startTimeoutMs}ms silence ceiling`), beyond what §4.4's prose literally quoted. This was necessary to satisfy §10's own proof command literally: the plan's §4.4 text keeps the *old* phrase as (part of) the new composite message ("`ui-server did not start: <reason>`, where `<reason>` is ... or the silence sentence"), but §10's proof (`rg -n "was not ready within the start timeout" ... → empty`) greps for exactly that phrase with no qualifier — and a substring match inside the new composite message still matches. Reworded the silence-case reason so the phrase is genuinely retired, keeping the same meaning. Two `watchdog.test.ts` assertions were updated to match (`'ui-server did not start: no ping answered within the'`, `.toContain('silence ceiling')`).
  - **`docs/spec.md` §8 "Inspector" already exists and already describes plan 208's phase 1 accurately** (session-scoped, fail-fast via `INSTRUMENTATION_STATUS`/`ClassNotFoundException`, the JSON-RPC configurator, `deviceCall()` awaiting the session's inspector, the trace reuse, a failed start never reported as `ready`) — this section was evidently written in anticipation of this plan. No `DIV-` row was needed and no edit to `docs/spec.md` was made; §4's instruction to "edit that section instead" did not apply because the existing text required no correction.
  - **Line numbers throughout §3.1/§4 had drifted** from the plan's 2026-09-03 citations (expected, per plan 200 §2.2) — matched every one on quoted content rather than the stated line number; no case where the file's content disagreed with the plan's description of it, beyond the two items above.
  - Plan 206 had already landed on `mvp` before this plan started (unlike §4.8's "if plan 206 has not landed" contingency): `prewarmInspector` already existed as the documented no-op, `DeviceSession.close()` already awaited `inspectorHandle?.release()`, and `always-on.ts` already called `prewarmInspector()` `INSPECTOR_PREWARM_DELAY_MS` after the first frame — exactly the seam §5 step 208.7 expected to fill in, with no seam-shape surprises.
- **Observed, not done** (noted, deliberately left, per plan 200 §2.1):
  - §4.10's mention of `setText`, `longClick`, `doubleClick` "using the same accessor" (`inspectorOrThrow()`) applies only to `setText` in the current codebase: `device-executor.ts` has no dedicated `longClick`/`doubleClick` `DeviceCall` methods today (only `InspectorElementActions` on the engine interface, reached through `type`'s `setText` shortcut). Nothing was added for methods that do not exist; `setText`'s own read is deliberately the *non-throwing* `deps.session.inspector` (not `inspectorOrThrow()`) because `type` is not in `INSPECTOR_METHODS` and must still fall through to the text ladder when the engine is not ready yet, rather than throwing.
  - The Studio-side fixture edits §4.1 lists (`packages/studio/src/app/settings/page.test.tsx`, `packages/studio/src/components/AdbServerCard.test.tsx`) were not touched, per §12's amendment (both already deleted by plan 201, which merged into `mvp` before this plan started) — confirmed absent, nothing to delete.
  - The bench script's `--attach-cycles` cold-cycle loop (§4.13) force-stops both openatx packages between cycles but was written and typechecked without a device to run it against; its shape follows the plan's §5 step 208.13 description exactly (stop, force-stop both packages, re-`start()`), and its correctness on real hardware is unverified — it is exercised, along with G10/G11/G13's numeric thresholds, only when an owner runs `ENKAKU_TEST_DEVICE=1 bun run scripts/bench-device-nfrs.ts --serial <serial> --skip-video --attach-cycles 3` on the lab device.
- **Open questions hit** (§9):
  - **Q1 (does openatx 2.4.0 start on API 36?)** — **not answered**: step 208.11 requires a physical lab device (Android 16, API 36) attached over adb; this execution environment has none (`adb` is not even on `PATH` here, and no `.dev-data` toolchain state exists). The manifest stays pinned at `2.3.3` with `TODO-M4.5` left in place, exactly as §5 step 208.11's "if it did not: leave the manifest untouched ... and keep 2.3.3" branch prescribes for an unmeasured outcome — except here the measurement itself never ran, rather than running and failing. **Plan 129 step 129.4 remains unanswered by this execution**: the "why ui-server does not start on Android 16" diagnosis is still open, owner-gated, and nothing in this repo should be read as having resolved it. No toolchain pin was changed on a guess.
  - **Q2 (`compatibleCoreRange` value)** — moot: the manifest was not touched (Q1 above), so no `compatibleCoreRange` was ever written.
  - **Q3 (fatal pattern table confirmation)** — not confirmed on a lab device; the table (`INSTRUMENTATION_FATAL_PATTERNS`) ships as written in §4.2, with only the first row (`stack=`/`ClassNotFoundException`) backed by the repo's own prior measurement (`launcher.ts`'s comment) and the rest sourced from the platform's documented raw output, exactly as §9 Q3 describes. Owner-gated confirmation (the wrong-class command, and the test package uninstalled) did not run.
  - **Q4 (`uiAutomationFlags` and plan 221)** — correctly left undecided; the field is not sent (2.3.3 has no such field per the verified source, §5 step 208.4's own finding matches R5's caveat), and the decision of whether/when to send it is deferred to plan 221 as the plan directs.
  - **Q5 (plan 206 order)** — plan 206 had already landed; `prewarmInspector` was the already-shipped no-op seam this plan filled in, not something this plan had to add from scratch. See "Discrepancies" above.
  - **Q6 (warm-attach threshold)** — left at the plan's own `≤ 3000 ms` in G10; no measured value exists to tighten it against (no lab device), so it was not touched.
- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` → empty (checked immediately after killing the smoke-test core process, and again at the end of this session). No process left running by this execution.
