...
- `adb-proxy`: apply = `settings put global http_proxy host:port`; revert = `settings put global http_proxy :0` (the reliable reset — `delete` alone is not enough on many builds); observe = `settings get global http_proxy`.
