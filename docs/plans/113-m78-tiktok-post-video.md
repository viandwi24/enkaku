...
> Status: implemented — **all eleven steps, including 113.4, are done (2026-08-18).** The hardware run happened: `tiktok/post-video@1.11.0` walked all six screens unattended and published a real video to a real account, twice in a row, and `outcome` reported `posted` — the first time this plan's own §3.6 rule allowed that word to be written. **What the hardware taught that no fixture could, in the order it hurt:** (1) *The screens that PLAY VIDEO cannot be read at all.* E3 recorded this for the feed; preview and editor animate too, so `dump` returns a stale tree or fails outright. The owner, watching the phone, described it as the run "getting stuck on the AutoCut/Berikutnya and Story Anda/Berikutnya screens" — and the taps were landing correctly every time; only the reading was blind. Those two screens now proceed unconfirmed and are checked at the post screen, which is static and dumps reliably. (2) *Aiming a blind tap needs the real screen size.* It was being measured from a dump of the feed — the one screen already known to be unreadable — and a partial tree reading 720x1556 instead of 720x1640 moved the aim point 76px above a button spanning y1451–1528. It now comes from `device.get`, and the fallback fraction is calibrated from the device's OWN picker screen rather than from this pack's reference panel. (3) *Three modals exist only AFTER a successful post* — a widget offer and a contacts pitch — and the 2026-08-17 walk could not have seen them because it stopped at the Post button. A run that knew only the pre-post modals sailed through the whole flow and stalled on the first screen it reached after succeeding. (4) *The caption field is not reliably empty*, and typing into a dirty field produced `#test #video #fy#test #video fyp` — two captions interleaved into one post. It is cleared first, every time. (5) *`BACK` after typing left the post screen entirely*, discarding the caption, because a hashtag opens a suggestion panel over the bottom bar; the Post button simply moves to the top right with the IME open, so the keyboard is now left alone. (6) *A KV key may not contain `/`*, so `caption-cursor:/videos/captions.txt` failed the very first run before TikTok was ever opened. **Also answered: §9 Q1.** Confirmation opens the profile and looks for a grid cell. It is honest but WEAK — an account with older videos has grid cells regardless, so it proves "the profile is reachable and populated", not "this exact video landed". The independent visual check was the orchestrator's, not the script's. A duration- or count-delta check would be stronger and is the obvious next improvement. **Universality, after the owner asked for it:** screen size per device, blind-tap position self-calibrated, and every text selector widened from one Indonesian literal to a case-insensitive list with English spellings. The English candidates are reasoned, not tested — this pack has run in exactly one locale, and saying otherwise would be the fiction it refuses everywhere else. **Verified 2026-08-18 at 1.11.3:** pack 238 pass / 0 fail; `bun run typecheck` clean; three unattended `outcome: posted` runs — two in FOLDER mode (each picking a different video at random) and one in QUEUE mode driven end to end through the plugin's own `TikTok Posts` screen: upload through the `kind: 'artifact'` picker, an entry appearing as `pending`, a failed attempt showing its own error text in the table, `Retry` returning it to `pending` while keeping `attempts`, and a final run settling it to `posted` with a stamp. **Two flakes the repeated runs exposed, both now fixed and both invisible to a single lucky run:** the post screen is the slowest transition in the flow and was being given half the time it needs (its settle budget is now ten rounds, not five); and the re-tap recovery only watched for the `editor` screen, so a run left sitting on `preview` — which a diagnostic screenshot from the failing job proved was exactly what happened — never recovered. It now watches both video screens. **The flakiness was measured, not guessed at, and then removed (1.12.0).** Repeated dry runs gave 3/4, then 2/5 — all failures at the same editor-to-post transition. A diagnostic screenshot from a stuck run settled it: the editor was fully drawn, "Berikutnya" solid and enabled, at exactly the coordinates the tap used. The tap was neither missing nor hitting a disabled control. What separated the successful MANUAL walk from the failing automated one was patience — by hand there were ~10s of looking at the screen before pressing, while the script pressed the moment the screen could be identified, with the video still loading behind it. TikTok drops taps in that window. A 4s dwell before pressing on either video screen took the same measurement to **5/5**, and — this is what makes the number mean something — that batch ran with NO reboot, so the improvement cannot be credited to a fresh device. A real post at 1.12.0 then reported `outcome: posted` across all six screens. **Two measurement traps caught along the way, both worth more than the fix:** a run that never started (device unplugged) returns `queued` with zero screens and must NOT be counted as a failure of the automation; and the `heal.sh` the orchestrator wrote to 'fix' a wedged inspector was force-stopping `com.github.uiautomator`, which is precisely what strands an `adb forward` — the ui-server launcher's own doc comment predicts the exact error string that produced. Five runs were lost to that self-inflicted fault before the pattern showed: every genuinely successful run had followed a device REBOOT, never a heal.
>
> Written 2026-08-18 from a hardware walk of the real upload flow (2026-08-17, moto g06 power `ZP2222RMBS`, Android 15 / API 35, `com.ss.android.ugc.trill`, locale id-ID) and a re-read of the plugin runtime as it stands after plans 108–112. Nothing here is built. The walk is recorded in §0.2 because the selectors it found are the plan's actual substance: every screen, every blocking modal, and the two traps (`ambiguous` selectors, an inspector that cannot see an animated screen) were observed on the device, not inferred from the app. **One correction this plan makes to the code it extends:** `post-video.ts`'s own doc comment says posting cannot be built because `ctx.device.push()` takes an `artifactId` rather than a path. That reason no longer holds and probably never did — `resolveArtifact` accepts any artifact row, including the ownerless ones `POST /api/artifacts` creates, and §0.2's walk pushed one and saw it in TikTok's picker. The real blockers were, and mostly still are, elsewhere.
...
| **G6** | ~~No `artifact` param kind, so an operator pastes a UUID into the run form.~~ | **CLOSED by step 113.9** — `kind: 'artifact'` in `PARAM_KINDS`, rendered by `ArtifactControl` | C9 |
...
| **113.9** | **DONE 2026-08-18.** `kind: 'artifact'` in `PARAM_KINDS` + `WorkspacePathControl`'s sibling in `SchemaForm`, rendering the existing `ArtifactPicker` (G6/C9). | A protocol change with its own compatibility surface — it does not belong inside a pack step. |
| **113.10** | **DONE 2026-08-18.** A `content` view on the plugin's surface: upload a video, give it a caption, see the queue and its history. Plus the service and its timer (§4.6). Unproven against a real `source: 'queue'` post, since 113.3/113.6 are not built yet (see status line). | The queue is unusable by a human until this exists. |
