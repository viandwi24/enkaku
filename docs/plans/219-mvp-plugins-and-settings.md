...
> Status: implemented (software) — G1, G6, G7 need the owner smoke (§7.3, a running core + browser); every other §0 row is checked by its own command. Executed 2026-09-04.
...
> Ships: packages/studio/src/components/settings/ToolchainSection.tsx
...
| G2 | Exactly 15 visible fields plus one Advanced section of 11 render | 15 leaf inputs outside Advanced; 11 inside it | `rg -c 'ui\(\{ title:' packages/protocol/src/settings.ts` prints `26` (plan 212's own gate, re-proved here because this plan is what renders every one of them) | [x] |
| G3 | No Scripts settings section exists anywhere in Settings | 0 matches | `rg -n -i "script.*auto-update\|version pinning\|run-as" packages/studio/src/app/settings packages/studio/src/components/settings` → empty | [x] |
| G4 | No settings field UI is hardcoded; every field renders through `SchemaForm` | 0 bespoke field components in the new Settings sections | `rg -n "narrowSchema" packages/studio/src/app/settings/page.tsx` → one call per schema-backed section; `GREP_219_HARDCODE` → empty | [x] |
| G5 | The plugin table has exactly five columns in the handoff's order | Plugin · Status · Scripts · Verified · Actions | owner smoke §7.3 step 2; `rg -n "<TableHead>Plugin</TableHead>" -A6 packages/studio/src/app/plugins/page.tsx` shows the five headers in order | [x] (grep half; owner smoke still open) |
...
| G8 | The KV browser is reachable from the Plugins page and nowhere on farm Settings | `KvPanel` has zero importers under `app/settings/`; one importer under `app/plugins/` | `rg -l "components/kv/KvPanel" packages/studio/src` prints exactly `packages/studio/src/app/plugins/page.tsx` | [x] |
| G9 | The two restart dialogs still reach their audited entry points | `AdbRestartDialog` still wraps `POST /api/tools/adb/restart`; neither dialog's source changed | `git diff --stat main -- packages/studio/src/components/AdbRestartDialog.tsx packages/studio/src/components/AppRestartDialog.tsx` prints nothing | [x] (see §11 discrepancy: `main` already differs from earlier landed plans; verified instead by `git status --short` showing neither file touched by this plan) |
| G10 | `/tools` is gone and its content lives in Settings → Farm → Toolchain | route absent; content present | `test ! -d packages/studio/src/app/tools` exits 0; owner smoke §7.3 step 4 | [x] (dir absent; owner smoke still open) |
| G11 | `scripts/check-routes.ts` has no stale exemption and passes | `/tools` row pruned from `PENDING_REMOVAL` | `rg -n "'/tools'" scripts/check-routes.ts` → empty; `bun run scripts/check-routes.ts` exits 0 | [x] |
| G12 | No v3 bracket colour form, `dark:` variant or hex literal in this plan's new files | 0 matches | `GREP_219_COLOUR` → empty | [x] |
| G13 | No forbidden vocabulary (plan 200 §2.4) in this plan's new files or copy | 0 matches | `GREP_219_VOCAB` → empty | [x] |
| G14 | The old Plugins page's Scripts tab and every script-version control are gone | 0 matches | `rg -n "ScriptGroupsPageResponseSchema\|ScriptToggleResponseSchema\|RunScriptDialog" packages/studio/src/app/plugins/page.tsx` → empty | [x] |
| G15 | The workspace typechecks | 0 errors | `bun run typecheck` exits 0 | [x] |
| G16 | The Agents settings tab is not built here | 0 matches | `rg -n "AgentSettingsSchema\|api/agents/settings" packages/studio/src/app/settings/page.tsx` → empty (plan 220 owns it) | [x] |
...
- **Checklist**: G1 ⏳ owner · G2 ✅ · G3 ✅ · G4 ✅ · G5 ✅ (grep half; owner smoke §7.3 step 2 still open) · G6 ⏳ owner · G7 ⏳ owner · G8 ✅ · G9 ✅ (see discrepancy below — verified against working-tree state, not the plan's stale `main` diff) · G10 ✅ (dir absent; owner smoke §7.3 step 4 still open) · G11 ✅ · G12 ✅ · G13 ✅ · G14 ✅ · G15 ✅ · G16 ✅

- **Commits**: `d36a204` feat(mvp-219) — Plugins and Settings pages, ActionRunner migration, GET /api/plugins/dev caller; this report's own commit follows.

- **Typecheck**: clean. `bash scripts/typecheck.sh` — all 19 workspace packages plus `examples` print `OK` (the worktree had no `node_modules`; `bun install` was run first, which also regenerated `bun.lock` picking up upstream plugin `package.json` version bumps the checked-in lockfile predated — see discrepancy below).

- **Tests run**: `bun test packages/core/src/api/plugins-route-parity.test.ts` → 8 pass, 0 fail, 46 `expect()` calls (this is the file plan 200 §8.11 names; it now asserts `UNREACHABLE_PENDING` is empty and that `GET /dev` and `DELETE /dev/:name` both have real Studio call sites). No other backend file was touched, so no other scoped test was run. Studio and `@enkaku/ui` have zero tests (plan 200 §8.3) — none written.

- **Removed, proven**:
  - `rg -n "ScriptGroupsPageResponseSchema|ScriptToggleResponseSchema|RunScriptDialog" packages/studio/src/app/plugins/page.tsx` → empty
  - `test ! -d packages/studio/src/app/tools` → exit 0
  - `rg -n "AdbServerCard|AppRestartCard" packages/studio/src` → two matches, both in files this plan does not own: `AppRestartDialog.tsx` (G9 forbids touching it; the mention is its own pre-existing header comment) and `components/video/useAdbVideoStatsPoll.ts` (not named by this plan). Every file this plan actually wrote or rewrote is clean of both names.
  - `rg -n "'/tools'" scripts/check-routes.ts` → empty; `bun run scripts/check-routes.ts` → `routes ok: 6 in nav, 5 exempt`, exit 0
  - `rg -n "FARM_SECTION_DEFS" packages/studio/src` → empty
  - `rg -l "components/kv/KvPanel" packages/studio/src` → exactly `packages/studio/src/app/plugins/page.tsx`
  - `GREP_219_HARDCODE` (§10.3) → empty
  - `rg -rln "components/target/TargetPicker|components/target/useTargetSelection" packages/studio/src` → empty (both files deleted; `ActionRunner.tsx` was the last importer)
  - `bun run scripts/check-design-tokens.ts` → `design tokens ok` (all sub-checks pass, including the "no v3 bracket / no dark: / no hex" scan over `packages/ui/src`)
  - `bash scripts/check-dead-code.sh` → `no dead code found`

- **Discrepancies between plan and code**:
  1. **§8.10's own table said `plugin-view/ViewRenderer.tsx` also references `TargetPicker.tsx`.** Reading the file (full read, 2026-09-04) shows no such import or usage — only `ActionRunner.tsx` did. Followed plan 200 §2.2 (file wins for facts): migrated `ActionRunner.tsx` only, left `ViewRenderer.tsx` untouched, and record the correction here rather than editing a file that needed no edit.
  2. **§4.4's worked example hardcodes `RemovePluginAction`'s `scopes={['version']}` inside the row's overflow menu, commented "unchanged from today's file", but the pre-219 file computed `scopes` from `versions.length > 1 ? [...] : ['version']` (the bulk "remove all versions" / "remove all except latest" options).** The two are not the same value, and the plan's own comment is wrong about that. Kept the plan's hardcoded `['version']` anyway, because it is not a style choice: `RemovePluginAction` renders its own nested `<DropdownMenu>` when `scopes.length > 1`, and that cannot sit inside the outer overflow's `<DropdownMenuItem asChild>` (a single child is required by Radix's `Slot`). Passing more than one scope there would have been a silent runtime defect, not a feature. **Net effect: the plugin table's row-level overflow menu no longer offers bulk version removal** (only "Remove" for the one version in view) — this used to be reachable from the row and no longer is. Nothing in this plan's non-goals names `app/plugins/detail/page.tsx` as owning that capability instead, and that page renders through the same shared `PluginActions` component, so the capability is gone from the product, not merely relocated. Flagged for the programme owner rather than decided unilaterally: `describeRemoveScope`/`RemoveScope`/bulk-preview logic is untouched and still exported from the new `RemovePluginAction.tsx`, so restoring multi-scope removal (behind a different composition than a nested menu-in-a-menu) is a small follow-up if wanted.
  3. **§3.0.1's own history note is confirmed again**: this plan's `Ships:` line already named a real, not-yet-existing artefact (`ToolchainSection.tsx`) correctly — `test -e` on it failed before this execution and passes now, so no correction was needed here (unlike plans 217/219's earlier documented mistake in plan 200 §3.0.1, which this plan's *own* `Ships:` line had already been fixed to avoid, per the commit `90d35fa docs(plans): 219 — Ships names a new artefact so check-plan-status passes`).
  4. **G9's proof command (`git diff --stat main -- .../AdbRestartDialog.tsx .../AppRestartDialog.tsx`) does not print nothing** — both files differ from `main` because five earlier MVP plans (vocabulary and token sweeps) already touched them on `mvp` before this plan ran. That is not this plan's doing: `git status --short` at the end of this execution shows neither file as modified in this plan's own worktree. Verified the goal's actual intent ("neither dialog's source changed **by this plan**") that way instead.
  5. **The worktree had no `node_modules` at all** (a bare checkout of the nested worktree), so `bun run typecheck` and `bun run build:studio` both failed with module-resolution errors before any of this plan's own edits. Ran `bun install` first (outside the plan's own steps, but required "to compile", per plan 200 §2.1's exception) — it also rewrote `bun.lock` to match `package.json` version bumps four plugins already carried on `mvp` (tiktok, mikrotik-routing, proxy-manager, instagram) that the previously-committed lockfile in this worktree predated. Included in the commit since a stale lockfile would otherwise reintroduce the same failure for the next reader of this worktree.
  6. **Task 2 (plan 200 §8.11) needed a real Studio call site to `GET /api/plugins/dev`, not merely the same data already visible through `GET /api/plugins`'s embedded `dev` field.** `packages/core/src/api/plugins-route-parity.test.ts` matches on the literal URL shape Studio's source builds, and `/api/plugins` and `/api/plugins/dev` are different shapes to that matcher (correctly — they are different routes, `runtime.list()`+`runtime.devSlots()` combined vs. `runtime.devSlots()` alone). The Plugins page now issues both requests on load; `PluginDevSlotsResponseSchema` (already in `@enkaku/protocol`, unused until now) is the schema for the second one.

- **Observed, not done**:
  - The bulk plugin-version removal regression above (item 2) — flagged, not fixed, pending an owner decision on whether it is restored and how.
  - `KvPanel.tsx`, `InstallPluginDialog.tsx`, `ResetPluginAction.tsx`'s own confirm bodies, and `AdbRestartDialog.tsx`/`AppRestartDialog.tsx` still carry `theme.css` block D tokens (`readout`, `text-led-*`, `bg-led-*`, `rack-label`) — untouched per §2's own non-goals table and G9.
  - `app/plugins/detail/page.tsx` and `app/plugins/view/page.tsx` are untouched, exactly as §2 requires.
  - The Settings page's old, already-orphaned `ConnectorsSection`/`WebhooksSection`/`BlockedDevicesSection`/`GuestAgentSummarySection`/`ObservedSpendPanel`/`AdbDiagnosticsPanel` function bodies (defined in the pre-219 file but never rendered by any `id === ...` branch, a byproduct of plan 212's rewiring) are gone along with the wholesale rewrite of `app/settings/page.tsx` — they were dead code before this plan touched the file, not a capability this plan removed. `AdbDiagnosticsPanel`'s adb-queue-depth diagnostics specifically had no reachable render path in the file this plan inherited; if that per-device adb queue view is still wanted somewhere, it needs a new home, not a restoration of the old file.

- **Open questions hit**: none of §9's three items blocked a step. Q1 (whether to re-skin `KvPanel.tsx`) and Q3 (whether to extend the icon barrel for `ToolchainSection`) were both answered by following the plan's own stated default (leave `KvPanel` alone; ship the two labels text-only). Q2 (URL-syncing the Plugins page's Installed/KV toggle) was likewise left as local component state, per the plan's own simplest reading.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` → one line, an `adb ... com.genymobile.scrcpy.Server` process against device `ZP2222RMBS`. This executor never ran `bun run dev`, `bun run dev:studio`, or anything that would start a device session (§7.3's owner smoke was not performed — see the owner rows in the checklist), so the process pre-dates and is unrelated to this execution; it is left running rather than killed, since it may belong to another agent or the owner's own session on the shared machine.
