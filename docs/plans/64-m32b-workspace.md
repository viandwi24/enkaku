...
> Status: implemented — a database-backed virtual filesystem, never the real one. `packages/core/src/workspace/path.ts` is the pure path-normalisation/validation function (step 64.1, absolute-only, NFC-normalised, `..`/`.`/empty segments/trailing slash all rejected — never resolved — 512-byte and 32-segment caps), with a 35-case traversal table in `path.test.ts`. `packages/core/src/workspace/store.ts` (`createWorkspaceStore`) is the only code that ever touches the new `workspace_files` table (`packages/core/src/db/schema.ts`, migration `packages/core/drizzle/0025_charming_blob.sql`): read/write/list/delete/move, compare-and-swap on `hash` (overwrite requires `ifMatch`, create forbids it — `E_EXISTS`/`E_STALE` distinguished), and three quotas read live from `FarmSettings.workspace` (`packages/protocol/src/settings.ts`) — `maxFileBytes`/`maxFilesPerScope`/`maxTotalBytesPerScope`, scoped per top-level directory or per-agent home (`scopeOfPath`), all raising `E_QUOTA` naming the limit and current usage. `store.test.ts`'s own `acceptance #2` suite chmods a real temp directory `0o444`, runs a full write/read/move/delete cycle against the store, and asserts the directory stays empty — proving nothing ever reaches `node:fs`. Five capabilities in `packages/core/src/capability/fs.ts` (`fs.list`/`.read`/`.write`/`.delete`/`.move`) are registered in the real registry (`capability/index.ts`) exactly like every other plan-63 entry — no second path to a file, everything through `invoke()`. Scope enforcement (`E_OUT_OF_SCOPE`, acceptance #6) reads `CapabilityContext.workspaceScope()`, a new accessor that returns `{read:['/'], write:['/']}` for today's only actor (a human) — the mechanism Plan 65's agent grants will narrow is in place and tested (`fs.test.ts`) by constructing a context with an explicit narrower scope, since no agent record exists yet to produce one honestly. Server-side bundling (`packages/core/src/scripts/build.ts`, step 64.5) statically walks the workspace import graph with `Bun.Transpiler.scanImports` and validates every specifier — relative/absolute against the workspace only (clamped at the root, never disk), bare specifiers against a two-entry allowlist (`@enkaku/sdk`, `zod`) — **before** any `Bun.build` call; this was load-bearing, not stylistic: Bun's bundler auto-externalises recognised `node:*` builtins under `target: 'bun'` without ever invoking a plugin's `onResolve`, so a plugin that only rejects imports it is asked to resolve would let `node:fs` through silently (confirmed by spiking it) — the static pre-scan closes that gap regardless of build target. Bounded at 30s (`withTimeout`, exported and tested directly with a real delayed promise, the same pattern `capability/invoke.ts` uses) and 20 MiB of output; the source is never executed (`build.test.ts`'s acceptance #9 sets a `globalThis` marker a top-level statement would flip if run, and asserts it never does). `script.publish` (`capability/script.ts`) now takes `{bundle,source?}` **or** `{path}` as a Zod union, both funnelling into the SAME `ctx.scripts.publish` → `scripts/service.ts`'s `publishScript` (acceptance #7) — `capability/script.test.ts` publishes a real `defineScript()` source through the path form, then materialises and imports the resulting bundle exactly like the runner's own `bundle-cache.ts` does, and calls `.run()` on it to prove it is genuinely executable, and separately proves a `node:fs`-importing source publishes no script row at all (acceptance #8). `packages/studio/src/app/workspace/page.tsx` is a breadcrumb-navigated directory browser (not a full recursive tree — proportionate to "no syntax highlighting" already being out of scope) plus a `<textarea>` editor, calling `fs.*` through `POST /api/v1/cap/:id` exactly like any other capability caller (`packages/studio/src/lib/workspace.ts`) — save sends the held `hash` as `ifMatch`; an `E_STALE` response shows a conflict banner ("reload and lose my edits" / "copy mine to the clipboard"), never a silent overwrite; every file's `createdBy`/`updatedBy` (`"user:<id>"`/`"agent:<id>"`) render as "written by user u1 · last edited by …"; a "Publish as script" action appears for `/scripts/` paths and shows a build failure verbatim. `bun run typecheck` and `bun run build:studio` (a real static export, `/workspace` included at 8.69 kB) are both green; `bun test` is 1698 pass / 0 fail (baseline 1611 + 87 new, all in this plan's own files). **Deviations, recorded rather than silent:** (1) `packages/core` did not depend on `@enkaku/sdk` before this plan — a workspace-authored script needs `import.meta.resolveSync('@enkaku/sdk', ...)` to find it on disk for bundling, so this plan adds `"@enkaku/sdk": "workspace:*"` to `packages/core/package.json` (`bun install` re-linked it); nothing else in `00-overview §3`'s stack table changes. (2) `fs.list`'s output is one level of children only (files plus synthesised directory entries), not a full recursive tree — this is what §4.5's "lazily loaded per prefix" actually asks for, and Studio's browser calls it once per navigation rather than fetching the whole tree up front. (3) `fs.delete`'s `ifMatch` is optional, not required — the plan's §4.2 table and acceptance criteria never call for CAS on delete the way §3.4 mandates it for write, so an unconditional delete (the common case: "remove this file") stays one call, while a caller that wants to guard against a concurrent edit still can. (4) Paths reachable during server-side bundling that resolve outside any workspace file (e.g. a relative import climbing past the root) are not specially detected as "escaped" — they are simply not found in the workspace and fail with the SAME `E_BUILD_FAILED` a missing file would, which is the honest answer since the resolution never touches real disk in the first place (there is nothing to "escape" to).
> Ships: packages/core/src/workspace/store.ts
