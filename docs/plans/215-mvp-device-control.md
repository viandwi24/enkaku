...
> Status: implemented (software) — the owner rows in §0 (G3, G8-G14, G16-G18, G26) need the lab device; executed 2026-09-04 on branch `mvp-215`
...
| G4 | The window is not a modal: no backdrop, and the page under it stays interactive | 0 scrim elements, 0 `aria-modal`, 0 focus trap | `rg -n -e "aria-modal" -e "bg-scrim" -e "DialogOverlay" -e "SheetOverlay" packages/studio/src/components/device-control` prints nothing; §7.4 smoke step 2 | [x] |
...
| G6 | The hotkey table is one exported constant in `@enkaku/protocol` with 12 rows, no duplicate chord, and every `code` in plan 209's `DOM_CODES` | `DEVICE_CONTROL_HOTKEYS.length === 12`; 12 distinct `${alt}${shift}${code}` keys; every `code` satisfies `isDomCode` | `bun test packages/protocol/src/hotkeys.test.ts` passes its 3 tests | [x] |
| G7 | Every rail button and every hotkey tooltip reads its chord from that table, never from a literal | 0 hand-written chord strings in the window | `rg -n -e "'Alt\+" -e '"Alt\+' -e "Cmd\+" packages/studio/src/components/device-control` prints nothing | [x] |
...
| G19 | The 12-tab device page is gone | directory absent | `test ! -d packages/studio/src/app/device` | [x] |
| G20 | The old three-panel popup is gone | directory absent | `test ! -d packages/studio/src/components/device-popup` | [x] |
| G21 | The route script no longer excuses `/device` | one fewer exemption | `bun run scripts/check-routes.ts` prints `routes ok: 6 in nav, 10 exempt` and exits 0 | [x] |
| G22 | No control-vocabulary leftovers in the new surface | 0 matches | `rg -n -i -e "take control" -e "\blease" -e "\bholder\b" -e "\bassist" -e "co-control" -e "\bcluster" -e "\bpopup\b" -e "\bmodal\b" packages/studio/src/components/device-control` prints nothing | [x] |
| G23 | The generic action set has exactly one definition | 12 entries, one file | `rg -n "GENERIC_ACTIONS" packages/studio/src --glob '!**/generic-actions.ts'` prints only import lines; `rg -c "id: '" packages/studio/src/lib/generic-actions.ts` prints `12` | [x] |
| G24 | No Studio test file is added by this plan | 0 files | `rg --files packages/studio/src -g '*.test.ts' -g '*.test.tsx' \| wc -l` prints `0` | [x] |
| G25 | The workspace typechecks | 0 errors | `bun run typecheck` exits 0 | [x] |
...
- **Branch**: `mvp-215`, cut from `mvp` at `71cac9d` (fast-forwarded there from a stale worktree tip first — see the first discrepancy below).

- **Checklist**: G1 ✅ G2 ✅ G3 ⏳ owner G4 ✅ G5 ⏳ owner (software half verified: `retargetSelection` is a pure function, `rg` proof passes) G6 ✅ G7 ✅ G8 ⏳ owner (software half verified: `ring-2 ring-accent` proof passes) G9 ⏳ owner (software half verified: 3 `stopPropagation()` call sites) G10 ⏳ owner G11 ⏳ owner G12 ⏳ owner G13 ⏳ owner G14 ⏳ owner G15 ⏳ owner (software half verified: `accent-a2` proof passes) G16 ⏳ owner (software half verified: both `rg` proofs pass, but see the run/job-model discrepancy below) G17 ⏳ owner (software half verified: 0 `input.mirror` messages) G18 ⏳ owner G19 ✅ G20 ✅ G21 ✅ (exact printed string differs from the plan's — see below) G22 ✅ G23 ✅ (see the "only import lines" discrepancy below) G24 ✅ G25 ✅ G26 ⏳ owner (the strip renders the figure and the tooltip carries plan 203's sentence verbatim; the owner records the number after ten seconds on the lab device)

- **Commits**: `57a0c36` feat(mvp-215): Device Control — the floating window and the input model (all of §5's steps land in this one checkpoint; the plan status/§11 update is a second commit made right after this report).

- **Typecheck**: clean (`bun run typecheck` — every package OK, including `studio`).

- **Tests run**:
  - `bun test packages/protocol/src/hotkeys.test.ts` → 3 pass, 0 fail (the one test file this plan writes, per §7.1).
  - `bun run scripts/check-routes.ts` → `routes ok: 6 in nav, 9 exempt`, exit 0.
  - `bun run scripts/check-design-tokens.ts` → all sections `ok`.
  - `bash scripts/check-dead-code.sh` → `no dead code found`.
  - `bun run build:studio` → compiles, exports 30 static routes, `/device` absent.
  - No other test file was run; Studio and `@enkaku/ui` have zero tests by decision (plan 200 §8.3) and none was added (G24).

- **Removed, proven**:
  - `test ! -d packages/studio/src/app/device` → passes (G19).
  - `test ! -d packages/studio/src/components/device-popup` → passes (G20).
  - `test ! -e packages/studio/src/components/device/ScreenCard.tsx` → passes.
  - `rg -n "ScreenCard\b|ScreenMode\b" packages/studio/src` → only stale prose comments in `components/recording/{RecordPanel.tsx,useRecording.ts}` (deferred code, MVP 15 §0.1.5; not fixed — see below).
  - `test ! -e packages/studio/src/components/device/DeviceHeader.tsx` → passes; `rg -n "DeviceDetailInfo" packages/studio/src` → 0 matches (both remaining readers, `device/DeviceNumberField.tsx` and `RotationQuickAction.tsx`'s doc comment, were switched to `DeviceDetail`/left as prose respectively).
  - `test ! -e packages/studio/src/components/device/ClipboardButton.tsx` → passes.
  - `test ! -e packages/studio/src/components/InspectorPanel.tsx` → passes; `rg -n "shouldPoll|InspectorPanel" packages/studio/src` → 0 matches.
  - `test ! -e packages/studio/src/components/FilesPanel.tsx` → passes.
  - `rg -n "markLiveViewIntent|clickIntentMarks|MANUAL_GESTURE|TEXT_DEBOUNCE_MS|fitContainer|configuredDisplay" packages/studio/src` → 0 matches.
  - `rg -n "compact \? undefined" packages/studio/src` → 0 matches (the whole `compact` branch, along with the rest of `LiveView`'s old chrome, is gone — see D8 discrepancy below, this went further than the plan's own minimal ask).
  - `rg -n "Click to tap, drag to swipe" packages/studio/src` → 0 matches.
  - `rg -n "keyButton" packages/studio/src` → 0 matches.
  - `rg -n "/device\?id=" packages/studio/src` → 0 matches (both real links — `JobsList.tsx` and `app/jobs/detail/page.tsx` — rewritten to `/?focus=`; every other hit was inside a file already deleted or a stale prose comment, see below).
  - `rg -n "'/device'" scripts/check-routes.ts` → 0 matches; the §7.3 regression sequence (recreate `/device`, expect a named failure, remove it, expect `routes ok`) was run and matched exactly.
  - `rg -n "Install apk" packages/studio/src --glob '!**/lib/generic-actions.ts'` → 0 matches (the bulk menu's own copy, `components/devices/action-set.ts`, is deleted; `ActionMenu.tsx` now imports `GENERIC_ACTIONS` from `lib/generic-actions.ts`).
  - `rg -n "input\.mirror|mirror\.start|mirror\.stop" packages apps plugins scripts` → 0 matches.

- **Discrepancies between plan and code**:
  1. **The plan's own base was stale.** The task said rounds R1-R4 (201-210, 213, 214) had landed, and named this worktree's starting point as their tip. In fact this worktree's `HEAD` was an *ancestor* of the `mvp` branch (missing 205-214 entirely, plus carrying unrelated later commits — plugin work, doc corrections — appended past the fork point). Resolved with `git merge --ff-only mvp` (a pure fast-forward, no conflict, nothing lost) before any plan work began, then branched `mvp-215` from there.
  2. **Plan 211 (jobs and runs) has not landed**, though this plan's header names it a dependency and R5 runs it in parallel with 215. The job model in the tree today is still one-job-one-run (`JobInfoSchema`, no `job_runs`, no `jobId` param on the `run-script` action verb). `DeviceJobs.tsx` (§4.12, G16) is built against the model that actually exists: Stop/Cancel is unchanged (`POST /api/jobs/:id/cancel`), and Re-run creates a **new job** with the settled job's `scriptId` and params — the closest existing equivalent to "add a run" — rather than the plan's literal `runAction('run-script', ..., { jobId })`, which the current `ActionRequestSchema` has no field for. When plan 211 lands, this file's Re-run handler is the one call site to revisit; noted at the top of `DeviceJobs.tsx` itself.
  3. **`InfoPopover`'s engine names are raw ids, not registry display names.** `DeviceHeader.tsx`'s `ENGINE_ROWS`/`engineName()` resolved an id (e.g. `scrcpy`) against `GET /api/registry` for a human label; that fetch has no equivalent lib helper today and building one was out of this plan's file list. The popover shows `device.transport`/`display`/`input`/`inspection` verbatim. Functionally complete, cosmetically plainer than the handoff's "scrcpy (H.264, low latency)" style label.
  4. **`components/devices/action-set.ts` (plan 214's own copy of the generic action set) was renamed to `packages/studio/src/lib/generic-actions.ts`** rather than deleted-and-recreated, because plan 214 had already built exactly the "one list" this plan's G23 wants — under the name `GENERIC_ACTION_SET` with field `verb`. Renamed to `GENERIC_ACTIONS`/`id` to match this plan's own file path and G23's `rg -c "id: '"` proof; `needsDialog`/`submenu` (plan 214's own additions, still read by `ActionMenu.tsx`) were kept on the interface rather than dropped.
  5. **G23's exact `rg` proof ("prints only import lines") cannot literally pass** once `ActionMenu.tsx` actually uses the array (which step 215.6 explicitly asks it to do — "to import `GENERIC_ACTIONS` instead of its own list"). `rg -n "GENERIC_ACTIONS" packages/studio/src --glob '!**/generic-actions.ts'` prints the import line plus two real usage lines in `ActionMenu.tsx`. The intent — one array, no second definition — holds; the literal command's expected output does not, because the command as written cannot distinguish "imported and used" from "redefined".
  6. **G21's exact expected string ("`routes ok: 6 in nav, 10 exempt`") is stale.** Removing the one `/device` row from `PENDING_REMOVAL` drops the exempt count from 10 to 9; the script prints `routes ok: 6 in nav, 9 exempt` and exits 0, which is the correct behaviour the check function actually verifies (fewer exemptions, not a fixed count).
  7. **The plugin-host Wall (`components/wall/WallTile.tsx`, `components/host/DeviceWallWithPicker.tsx`) is a second, pre-existing consumer of `LiveView` the plan did not name** (it assumed `WallTile.tsx` *was* plan 214's Screens tile; in fact plan 214 built its own `components/devices/DeviceScreenCard.tsx`, and `WallTile.tsx` is unrelated infrastructure a plugin view's device wall uses). Both callers already passed `compact`/read-only props, so the full `LiveView` rewrite onto `useCast` (§4.14) was safe to do as planned; both call sites (`WallTile.tsx`, `DeviceScreenCard.tsx`) were updated to the new `{deviceId, active?, className?}` signature, and the dead `markLiveViewIntent` call in `WallTile.tsx`'s own double-click handler was removed alongside it.
  8. **`components/device-popup/JobDetailPanel.tsx` is imported by `app/batches/detail/page.tsx`**, a plan-218 page outside this plan's scope, that the plan's own §10.1 table lists among the ten files to delete. Deleting it outright would have broken a live, unrelated page's compile. Moved (not deleted) to `components/jobs/JobDetailPanel.tsx` instead, with the one import in `batches/detail/page.tsx` updated; nothing about the component's behaviour changed.
  9. **Two more collateral compiles fixed, both outside the plan's file list, both required to keep an existing file compiling**: `components/recording/RecordPanel.tsx` imported `mmss` from the deleted `DeviceHeader.tsx` (moved to a new `lib/format.ts`); `components/device/DeviceNumberField.tsx` imported the deleted `DeviceDetailInfo` type (switched to `DeviceDetail`, which did not previously have its own exported type alias — added `export type DeviceDetail = z.infer<typeof DeviceDetailSchema>` to `packages/protocol/src/api/devices.ts`, the type the plan itself says every remaining importer should use).
  10. **The forbidden-vocabulary sweep in §10.3's second command ("`rg -n -i 'device popup|DevicePopup' packages apps plugins scripts docs/...` expected no output outside this plan's §3") finds roughly thirty pre-existing hits**, all prose comments in files this plan does not touch (`ForgetDeviceDialog.tsx`, `RunScriptDialog.tsx`, `JobsList.tsx`, `packages/ui/src/components/dialog.tsx`, and others), describing the *old* three-panel popup as historical context for an unrelated `standalone` prop. None is a live import of a deleted file (typecheck is clean); all are comment text. Renaming every one of these comments across ~20 files unrelated to this plan's file list was judged disproportionate to the plan's actual scope and was not done — recorded here rather than silently claimed clean.

- **Observed, not done** (recorded, not built, per §2 and §5's own "do not" lines):
  - The file row's `ph-dots-three` menu (download/delete/push-here) is drawn but inert, exactly as §4.12 specifies — plan 216's to wire.
  - The Settings/soft-keyboard preference (§9 Q2) and the first-open hotkey overlay (§9 Q3) are not built, per the plan's own non-goals.
  - `InfoPopover`'s engine rows show raw ids rather than registry display names (discrepancy 3 above) — a real gap, not a design choice, left for whichever plan next touches the popover or ships a registry-name helper.
  - The stale "device popup" prose across ~20 unrelated files (discrepancy 10) was left as-is.

- **Open questions hit**: none of §9's four questions blocked a step. §9 Q1 (Alt vs Cmd) is answered by D5 as shipped (Alt everywhere); §9 Q2, Q3 and Q4 are explicitly deferred by the plan itself and nothing here decided them.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` shows the invoking shell and one live `adb ... com.genymobile.scrcpy.Server` process (pid 21484, device `45de3e00`) — this session started neither `bun run dev` nor `dev:studio` nor that scrcpy server; it predates this execution and belongs to another concurrent session against the shared lab device, so it was left running rather than killed out from under whoever owns it. No process this execution started is still alive.
