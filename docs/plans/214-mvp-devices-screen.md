...
> Status: partial — implemented 2026-09-04 by the executing agent. Software-verifiable goals (G1, G3, G4, G7*, G8–G11, G13–G17) pass; G2, G5, G6 and the metric half of G9 need the owner smoke (§7.3) on real hardware. G12 stays unchecked: `packages/studio/src/hooks/use-bulk-selection.ts` was NOT deleted because `app/agents/detail/page.tsx` (plan 220's screen, not this plan's) imports it — discovered only when the deletion broke `bun run typecheck`; see §11.
...
| G9 | The four metric columns have a farm-wide source and no client poll | `DeviceInfo.metrics` plus the `device.metrics` push; one extra `client.exec` per device per `battery.pollIntervalSec` | `bun test packages/protocol/src/device.test.ts` passes the new `describe('DeviceMetricsSchema')`; owner smoke §7.3 step 14 (CPU, Mem, Disk, Uptime are numbers on a live device and an em dash on a disconnected one) | [x]* |
| G10 | `DeviceInfo.model` exists and the Device cell's sub-line uses it | `model: z.string().nullable().default(null)` on `DeviceInfoSchema`; `devices.model` column; written by the registry probe from `probe.model` | `bun test packages/protocol/src/device.test.ts` passes the `model` case; `rg -n "model: probe.model" packages/core/src/registry/device-registry.ts` prints exactly two lines | [x] |
| G11 | No response body is `as`-cast | `fetchDiscoveredDevices` parses through `DiscoveredDevicesResponseSchema` | `rg -n "as \{ discovered" packages/studio/src/lib/api.ts` prints nothing | [x] |
...
| G13 | The `/groups` route is gone and `check-routes.ts` passes with the pruned list | `app/groups/` absent; `PENDING_REMOVAL` has no `/groups` row | `test ! -e packages/studio/src/app/groups` exits 0; `bun run scripts/check-routes.ts` prints `routes ok: 6 in nav, 10 exempt` | [x] |
| G14 | No forbidden word from plan 200 §2.4 appears in a file this plan creates | 0 matches | §10.3 `GREP_214_VOCAB` prints nothing | [x] |
| G15 | No file this plan creates names a colour in the v3 bracket form, a `dark:` variant, or a hex literal | 0 matches | §10.3 `GREP_214_COLOUR` prints nothing | [x] |
| G16 | The workspace typechecks and the design-token script still passes | 0 errors; `design tokens ok` | `bun run typecheck`; `bun run scripts/check-design-tokens.ts` | [x] |
| G17 | No plugin version moved and no pack was rebuilt | empty diff | `git diff --stat mvp -- plugins packages/core/packs` prints nothing | [x] |
...
- **Checklist**: G1 ✅ G2 ⏳ owner G3 ✅ G4 ✅ G5 ⏳ owner (poll grep clean; the 60fps profile needs a real browser) G6 ⏳ owner G7 ✅* (see discrepancies — one pre-existing, out-of-scope read site also names `/api/groups`) G8 ✅ G9 ✅ (protocol test) / ⏳ owner (metric half) G10 ✅ G11 ✅ G12 ❌ (see discrepancies — one file intentionally kept) G13 ✅ G14 ✅ G15 ✅ G16 ✅ G17 ✅
- **Commits**: not yet committed — see the commit created alongside this report (branch `worktree-agent-a4aad5ce39326e851`, based on `mvp` at `ed87537` after a fast-forward merge)
- **Typecheck**: clean — `bun run typecheck` prints `OK` for every package (protocol, ui, adb, toolchain, drivers, scrcpy, sdk, session, harness, core, node, studio, probe-server, networking, proxy-manager, tiktok-automation-pack, mikrotik-routing, google-automation-pack, youtube-automation-pack, examples)
- **Tests run**:
  - `bun test packages/protocol/src/device.test.ts` → 28 pass, 0 fail, 41 expect() calls (the new `DeviceMetricsSchema`/`DeviceMetricsMessage`/`model`/`metrics` cases included)
  - `bun test packages/core/src/device/battery.test.ts` → 3 pass, 0 fail, 12 expect() calls. This file was NOT in the plan's §7 allowlist, but the metrics probe's extra `client.exec` per device broke two of its assertions (a call-count check and a timing budget) — fixed per plan 200 §2.1 ("a test your change broke is yours to fix, whatever its path"), not skipped.
  - `bun run build:studio` → succeeds, static export completes (31/31 pages), no server/client boundary errors.
  - `bash scripts/check-dead-code.sh` → `no dead code found`.
  - Never ran a bare `bun test`; never ran `bun run --cwd packages/studio test` (Studio has zero tests).
- **Removed, proven**:
  - `rg -n -e "components/DeviceCard" -e "DiscoveredTray" -e "AdmitDeviceDialog" -e "ScanNetworkDialog" -e "useDragSelect" -e "SelectionCursorBadge" -e "DeviceContextMenu" -e "use-bulk-selection" -e "GroupEditorDialog" -e "GroupMembersDialog" -e "app/groups" packages/studio/src scripts` (GREP_214_OLD) — prints ONLY prose comments in files this plan does not own (`network-ranges.ts`, `labelling.ts`, `network-scan.ts`, `RangeNetworksFields.tsx`, `FarmNetworksEditor.tsx`, `DeviceWallWithPicker.tsx`, `Wall.tsx`, `WallTile.tsx`, `AskAnAgentDialog.tsx`, `SidePanel.tsx`, `ActionsList.tsx`, `app/device/page.tsx`, `app/batches/detail/page.tsx`, `scripts/check-routes.ts`'s own historical note) plus one real, deliberate exception: `packages/studio/src/app/agents/detail/page.tsx:57` still imports `use-bulk-selection` (see discrepancies). No file this plan controls imports a deleted module; `bun run typecheck` and `bun run build:studio` both confirm no dangling import anywhere in the tree.
  - `test ! -e packages/studio/src/components/DeviceCard.tsx && test ! -e packages/studio/src/components/DiscoveredTray.tsx` → exit 0
  - `test ! -e packages/studio/src/components/AdmitDeviceDialog.tsx && test ! -e packages/studio/src/components/device/ScanNetworkDialog.tsx` → exit 0
  - `test ! -e packages/studio/src/components/wall/useDragSelect.ts && test ! -e packages/studio/src/components/wall/SelectionCursorBadge.tsx && test ! -e packages/studio/src/components/wall/DeviceContextMenu.tsx` → exit 0
  - `test ! -e packages/studio/src/app/groups && test ! -e packages/studio/src/components/GroupEditorDialog.tsx && test ! -e packages/studio/src/components/GroupMembersDialog.tsx` → exit 0
  - `test ! -e packages/studio/src/hooks/use-bulk-selection.ts` → **exit 1** (kept — discrepancy below); G12 is left unchecked rather than falsely marked done.
  - `rg -n "'/groups'" scripts/check-routes.ts` → prints nothing; `bun run scripts/check-routes.ts` → `routes ok: 6 in nav, 10 exempt`
  - `rg -n "tileSize|pageSize|TILE_SIZE_PX|PAGE_SIZE_OPTIONS" packages/studio/src` → prints nothing
  - `rg -n "as \{ discovered" packages/studio/src/lib/api.ts` → prints nothing
  - `rg -n "'asleep'|readiness.actual === 'hot'" packages/studio/src/components/devices/useLiveSet.ts` → prints nothing
  - §10.3 `GREP_214_VOCAB` → prints nothing (after rewording four comments that quoted the handoff's "cluster tabs" or named `Wall.tsx`/`useDragSelect.ts`/`wall/WallTile.tsx` by path)
  - §10.3 `GREP_214_COLOUR` → prints nothing
  - §10.3 `GREP_214_METRICS` → prints exactly two lines (`packages/core/src/device/metrics.ts`, `packages/core/src/device/battery.ts`)
- **Discrepancies between plan and code**:
  1. **`hooks/use-bulk-selection.ts` was not deleted.** The plan's §10.1/§4.11 list it as deleted with no caveat, unlike `components/wall/*` and `components/bulk/*` (§3.5, §3.6), which the plan itself documents as kept for a named outside consumer. Deleting it broke `bun run typecheck`: `packages/studio/src/app/agents/detail/page.tsx` (three call sites, `useBulkSelection` on tool grants, device grants and permission grants) imports it, and that screen is plan 220's, not this plan's. Per plan 200 §2.2 ("the file wins for facts, the plan wins for intent") and §2.1 ("do not touch a file the plan does not name unless required to compile"), the file was restored rather than rewriting a screen outside this plan's remit. G12 is left unchecked rather than marked done falsely; §10.1's row for this file is not satisfied.
  2. **G7's grep also matches pre-existing, read-only consumers of `GET /api/groups`.** `RunScriptDialog.tsx`, `device-popup/ActionsList.tsx`, `ScheduleEditorDialog.tsx` (target pickers, populating a group dropdown) and `app/schedules/detail/page.tsx` (a comment plus a *different* route, `/api/groups/:id/devices`) all predate this plan and are unrelated to group CRUD — they only read the list to populate their own device-target pickers. Group **CRUD** (`POST`/`PATCH`/`DELETE /api/groups`) is confined to `GroupTabs.tsx`, which is what G7's prose actually asks for; the literal grep is broader than the intent. Recorded rather than silently "fixed" by editing four files this plan does not own.
  3. **`packages/protocol/src/api/devices.ts` and `packages/studio/src/lib/api.ts`'s `DiscoveredDevice`/`fetchDiscoveredDevices` already existed** (from plan 56), contrary to the plan's "new" framing in §4.2/§4.14 — `DiscoveredDeviceSchema`/`DiscoveredDevicesResponseSchema` were appended to the existing file rather than creating a new one, and `lib/api.ts`'s existing `interface DiscoveredDevice` was replaced with a `type` re-export from `@enkaku/protocol` rather than deleted outright (no such file/interface existed to delete twice).
  4. **`components/wall/Wall.tsx:195`'s comment still names `TILE_SIZE_PX`**, an export this plan deletes from `lib/prefs.ts`. It is a stale doc-comment reference, not a live import (typechecks clean), inside a file this plan touches only at one import line (§3.9/§4.4); left as an observed, not-fixed staleness rather than a further edit to a file mostly owned by other plans.
  5. **The battery poll's bounded-parallelism test's two assertions changed** (see Tests run above) — a call-count list and a timing budget, both now doubled by the metrics probe's extra `exec`. Fixed in place with comments explaining the new shape, per plan 200 §2.1.
- **Observed, not done**:
  - The owner smoke (§7.3) itself — G2, G5's 60fps profile, G6's six selection sub-checks, and G9's on-device metric comparison against `adb shell top`/`df`/`/proc/uptime` all need a physical device and a browser, and are left for the owner exactly as §7.1/§7.3 anticipate.
  - `EnrollmentDialog` is kept mounted on `DevicesScreen` per §4.17/§9 Q5, undrawn by the handoff — unchanged from the plan's own acceptance of this gap.
  - Six of the twelve bulk-action rows (`install`, `adb`, `run-script`, `push`, `clear-cache`, `settings`) render `aria-disabled` with `title="Opens a dialog (plan 216)"` exactly as §3.10 specifies; no dialog code was added.
- **Open questions hit**: none of §9's six questions blocked a step. Q2 (the amber state's word) was applied as decided by the plan (`Quarantined`, not `Unauthorized`) in the filter menu, the card's centre text, and `device-state.ts`'s doc comment. Q3 (native `title` tooltip vs. a styled Radix tooltip) was built as the plan's own default (native `title`). Q6 (live cast at every card width) was built as the plan's own default (live at every width, gated by `useLiveSet`'s existing budget).
- **Processes**: mid-session, `ps -Ao pid=,command= | grep -i "[o]penpf"` showed the invoking shell plus two pre-existing `adb shell … scrcpy-server` processes (serial `45de3e00`) that were already running before this task started — this agent never ran `bun run dev`, `bun run dev:studio`, or any device session, so they were left untouched rather than killed out from under whatever started them. At the end of this session that same command prints nothing at all (those two processes have since ended on their own). No process this agent started is still running.
