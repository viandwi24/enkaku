...
After this plan, the farm can run a local proxy and a device can dial it. The farm still cannot make an app *use* it. Pointing traffic at a proxy on Android means either the app's own settings, a per-app configuration, or a VPN-level tunnel the app cannot bypass — and the last one is `vpn-helper` in the `network` driver layer (spec §7.9), which no plugin can reach and which the owner has temporarily removed. That distinction is the entire content of the Assignments tab's standing note and it survives this plan verbatim, narrowed only where 112.11 makes something specific true.
