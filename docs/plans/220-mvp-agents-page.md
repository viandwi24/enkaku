...
> Status: implemented (software) — G12 (owner smoke) is the only open row; everything else verified below (executed 2026-09-04)
...
|---|---|---|---|---|
| G1 | One route, `/agents`, renders a five-tab page | tabs `roster`, `runs`, `approvals`, `files`, `settings`, in that order, `roster` is the default with no `?tab=` | `rg -n "key: '(roster\|runs\|approvals\|files\|settings)'" packages/studio/src/components/agents/AgentsPage.tsx` → 5 lines in that order (discrepancy: `EntityTab`'s real field is `key`, not `id` — plan's own command corrected, §11) | [x] |
| G2 | `/workspace` and the three `/agents/*` sub-routes are gone | 4 directories | `test ! -d packages/studio/src/app/workspace && test ! -d packages/studio/src/app/agents/approvals && test ! -d packages/studio/src/app/agents/runs && test ! -d packages/studio/src/app/agents/thread` → exit 0 | [x] |
| G3 | `check-routes.ts` no longer exempts `/workspace` | 0 matches | `rg -n "workspace" scripts/check-routes.ts` → empty; `bun run scripts/check-routes.ts` → exit 0, prints `routes ok: …` | [x] |
| G4 | The Settings tab is schema-generated for `defaults`/`scheduled`; no hardcoded field UI for either | 0 hand-written `<Label>`/`<Input>` pairs for a field `AgentSettingsSchema` declares | `rg -n "AgentDefaultsSchema\|maxOutputTokens\|spendCapOutputTokensPer24h" packages/studio/src/components/agents/SettingsTab.tsx` → empty | [x] |
| G5 | The `ai-elements` trim leaves only what `Chat.tsx` imports, a derived count | `conversation.tsx` 4 value exports (was 6), `message.tsx` 3 (was 12), `prompt-input.tsx` 19 (was 40), `reasoning.tsx` 3 exported + 1 module-private (was 4 exported), `shimmer.tsx` unchanged (1) | discrepancy: the trim itself was already done before this plan ran (§11); this plan did the icon swaps and `useReasoning`'s privatisation; `rg -c "^export " conversation.tsx/message.tsx/prompt-input.tsx` → 8/6/20 (matches the plan's own derived totals) | [x] |
| G6 | `motion` still resolves for `shimmer.tsx` | `shimmer.tsx` untouched, `motion` stays a `packages/studio/package.json` dependency | `bun run typecheck` clean; `rg -n "\"motion\"" packages/studio/package.json` → 1 line | [x] |
| G7 | `lucide-react` has zero importers in `packages/studio/src` and is removed from `package.json` | 0 matches | NOT achieved — 49 files outside the agent subsystem still import it (owned by plans 213/214/215/217/218/219, not this plan); dependency kept. See §11 | [ ] |
| G8 | The `spawn-grants` routes are deleted, decision recorded | 0 matches | `rg -rn "spawn-grants" packages/core/src packages/studio/src` → one hit, a pre-existing doc comment in `packages/core/src/agent/runner.ts:111` this plan's own non-goals forbid touching (§11); the routes themselves are gone | [x] |
| G9 | Every internal link that pointed at a route this plan deletes now points at the merged page | 0 matches for the old paths outside this plan's own new files | `rg -n "'/workspace\|/agents/approvals'\|/agents/runs\?agent" packages/studio/src --glob '!**/agents/**'` → empty | [x] |
| G10 | The workspace typechecks | 0 errors | `bun run typecheck` → every package `OK` | [x] |
| G11 | No forbidden word from plan 200 §2.4 appears in a file this plan creates or rewrites | 0 matches | `rg -n -i "\blease\b\|\bcluster\b\|co-control\|\bassist\b\|\bgrant\b" packages/studio/src/components/agents packages/studio/src/components/ai-elements` → empty | [x] |
...
- **Checklist**: G1 ✅ G2 ✅ G3 ✅ G4 ✅ G5 ✅ G6 ✅ G7 ❌ (scope, see below) G8 ✅ G9 ✅ G10 ✅ G11 ✅ G12 ⏳ owner

- **Commits**: `8da69d1` (wip, icon swaps 220.6), `0acdf7e` (feat, the page and its removals). Both on branch `worktree-agent-ae5b81ca1304dbb82`, fast-forwarded from `mvp` tip `de4a8f9` before either.

- **Typecheck**: clean — `bun run typecheck` → every one of the 19 workspace packages `OK`, both mid-execution and at the end.

- **Tests run**:
  - `bun test packages/core/src/api/agents.test.ts` → 9 pass, 0 fail (the spawn-grants `describe` block removed, per step 220.18).
  - `bun test packages/core/src/api/agent-settings.test.ts` → 4 pass, 0 fail (its own `setUp` broke when `createAgentRoutes` dropped the `tree` param — fixed, since a test my change broke is mine to fix per plan 200 §2.1).
  - `bun run scripts/check-routes.ts` → `routes ok: 6 in nav, 5 exempt`.
  - `bun run scripts/check-design-tokens.ts` → `design tokens ok` (after widening `GROUP_3` — see discrepancies).
  - `bun run build:studio` → succeeds; `/agents` (20.2 kB) and `/agents/detail` (527 kB, unchanged) both export; `/workspace`, `/agents/approvals`, `/agents/runs`, `/agents/thread` are absent from the route table.
  - No `*.test.tsx`/`*.test.ts` written under `packages/studio` or `packages/ui` (plan 200 §8.3).

- **Removed, proven**:
  - `test ! -d packages/studio/src/app/workspace && test ! -d packages/studio/src/app/agents/approvals && test ! -d packages/studio/src/app/agents/runs && test ! -d packages/studio/src/app/agents/thread` → all pass.
  - `rg -n "spawn-grants" packages/core/src` (recursive) → one hit, `packages/core/src/agent/runner.ts:111`, a pre-existing doc comment on the runner's own tree-store field — not this plan's route, and `runner.ts` is explicitly off-limits (§5 step 220.18 "Do not"). The routes themselves (`GET/POST /:id/spawn-grants`, `DELETE /:id/spawn-grants/:childId`) and `mustGetTree` are gone from `packages/core/src/api/agents.ts`.
  - `rg -n "createAgentRoutes\(\{ store: agentStore, tree" packages/core/src` → empty (`daemon.ts`'s wiring updated).
  - `rg -l "from 'lucide-react'" packages/studio/src/components/agent packages/studio/src/components/agents packages/studio/src/components/ai-elements packages/studio/src/app/agents/detail/page.tsx` → empty.
  - `rg -n "'/workspace'" scripts/check-routes.ts` → empty.
  - `rg -c "^export " packages/studio/src/components/ai-elements/{conversation,message,prompt-input}.tsx` → 8 / 6 / 20 (matches §3.3's derived totals exactly).
  - Forbidden-word grep (§10) → empty.

- **Discrepancies between plan and code** (plan 200 §2.2 — file wins for facts, plan wins for intent):
  1. **The worktree started 242 commits behind `mvp` tip `de4a8f9`.** The task named `de4a8f9` as the tip to execute against; the assigned worktree's own branch (`worktree-agent-ae5b81ca1304dbb82`) was a strict ancestor of it with zero divergent commits. Fast-forwarded (`git merge --ff-only mvp`) before any edit, since it was a pure fast-forward with nothing to lose. Recorded here because every fact below assumes the post-fast-forward tree.
  2. **`FarmAgentSettingsSchema`, not `AgentSettingsSchema`.** Plan 220's header and §4.7/§4.14/§4.8 assume plan 212 shipped `AgentSettingsSchema`/`AgentSettingsResponseSchema`/`UpdateAgentSettingsResponseSchema` and a bare `GET/PATCH /api/agents/settings`. What actually shipped (`packages/protocol/src/agent-settings.ts`, with its own doc comment explaining the rename: `./agent.ts` already exports `AgentSettingsSchema` for ONE agent's own per-agent overrides, so the farm-wide schema needed a different name) is `FarmAgentSettingsSchema`/`FarmAgentSettingsResponseSchema`/`UpdateFarmAgentSettingsResponseSchema`. `SettingsTab.tsx` uses the real names; `agents/detail/page.tsx` and `Chat.tsx`'s own `agentDefaults` call-site fixes (§4.8 item 1, §4.14) had, in fact, **already been made** by whoever landed plan 212 — this plan only had to fix the Runs link, the Connectors link, and the icon import in `agents/detail/page.tsx`, and the icon import in `Chat.tsx`.
  3. **The `ai-elements` trim (steps 220.2–220.4) had already been done.** `conversation.tsx`, `message.tsx`, and `prompt-input.tsx` already exported exactly the plan's own derived keep-list (verified by `rg -c "^export "`: 8/6/20, matching §3.3 exactly) before this plan touched them — only the icon imports were still `lucide-react`, and `reasoning.tsx`'s `useReasoning` was still `export`ed. This plan did the icon swaps (steps 220.2, 220.5 partial, and the equivalent of 220.4's icon half) and downgraded `useReasoning` to module-private; it did not need to delete any export, because there was nothing left to delete.
  4. **`RobotIcon` and `CopyIcon` already existed in `packages/ui/src/icons.ts`'s Group 3** (added by plans 213 and 218 respectively, with their own dated comments). Plan 220 §4.1 assumed it was adding both; this plan added only the ten names genuinely missing (`ArrowCounterClockwiseIcon`, `ArrowDownIcon`, `ArrowSquareOutIcon`, `BrainIcon`, `EyeSlashIcon`, `FloppyDiskIcon`, `ImageBrokenIcon`, `PaperPlaneRightIcon`, `PaperclipIcon`, `RocketIcon`), and widened `scripts/check-design-tokens.ts`'s own `GROUP_3` array and exact-count assertion to match — the same additive pattern plan 200 §8.11 records for plans 217/218 doing the same thing to the same list.
  5. **§9 Q1 is answered, not open**: `@phosphor-icons/react@2.1.10` is installed (`bun pm ls --all`), and its own `.d.ts` confirms `PaperPlaneRightIcon` is a real export (`csr/PaperPlaneRight.d.ts`: `export { I as PaperPlaneRightIcon }`). No substitution needed.
  6. **G1's own verification command is stale against the real `EntityTab` type.** `packages/studio/src/components/layout/EntityTabs.tsx`'s `EntityTab` interface has never had an `id` field — it is `key`. The plan's `rg -n "id: '...'"` command would find zero matches against a correct implementation; corrected to `rg -n "key: '...'"` in §0 above, which finds the expected 5 lines in order.
  7. **`lucide-react`'s removal (G6/G7, steps 220.19) is not achievable within this plan's own scope, and the plan's own "one area no other 21x plan touches" premise is false.** `rg -rl "from 'lucide-react'" packages/studio/src` finds 49 files this plan does not own (`app/nodes`, `app/plugins/*`, `app/recordings/*`, `app/scripts/*`, `app/settings`, `app/tools`, and components under `components/settings`, `components/workflow`, `components/schema-form`, `components/guest-agent`, `components/device`, `components/monitor`, `components/kv`, `components/terminal`, `components/workspace/presenters`, `components/plugin-view`, and several top-level components). Converting them belongs to whichever plan owns each file (213/214/215/217/218/219, none of which this plan may touch per its own non-goals and the launching brief's "stay out of Plugins/Settings"). This plan converts every file it does own in full (the agent subsystem: `components/agent/*`, `components/agents/*`, `components/ai-elements/*`, `app/agents/detail/page.tsx`) and leaves `lucide-react` in `packages/studio/package.json`, since removing it while 49 files still import it would be a build break, not a style choice. G6/G7 are recorded as not achieved rather than falsely checked.
  8. **`app/settings/page.tsx` still has today's `ConnectorsSection`/`WebhooksSection`** (plan 219 has not run). Per §3.6's own fallback and step 220.10's own instruction, this plan transcribed both verbatim into new files (`ConnectorsSettingsSection.tsx`, `WebhooksSettingsSection.tsx`) without touching or deleting the originals — that deletion is plan 219's, once its rewrite supersedes them.
  9. **`ApprovalsTab`'s `decide()` calls `reload()` on success, not a local optimistic filter.** The original `app/agents/approvals/page.tsx` filtered its own local `items` state after a decision; now that `approvals` is owned by `AgentsPage` (fetched once so the tab-strip badge and the tab body agree), there is no local setter to filter — `reload()` re-fetches instead, the same pattern `RosterTab`'s delete already uses. A network round-trip instead of an instant client-side filter; not stated explicitly in the plan's §4.5, but consistent with its "approvals/error/reload come from AgentsPage" framing.
  10. **`AgentsPage.tsx`'s tab-resolution line was rewritten to call `useSearchParams()` once, not twice inline**, as the plan's own §4.3 code block did (`TABS.some(... useSearchParams().get('tab')) ? useSearchParams().get('tab') ...`). Behaviourally identical; avoids two hook calls per render for no reason.
  11. **The inherited item: `packages/studio/src/hooks/use-bulk-selection.ts` is kept, unchanged.** Its only Studio consumer remains `app/agents/detail/page.tsx` (verified: `rg -rln "use-bulk-selection\|useBulkSelection" packages/studio/src` → exactly those two files), which this plan touches only for three named call sites (§4.8 items 2–4) and does not rebuild. Plan 216's own settled reasoning (`docs/plans/216-mvp-action-dialogs.md:1028`) still applies verbatim: it is a generic tri-state-checkbox helper for arbitrary `string` id lists (the tool list, the device-grant list, the permission list on the per-agent Workbench/Settings page), not a device picker, and `useTarget` cannot replace it. Not migrated, not deleted.

- **Observed, not done**:
  - The 49-file `lucide-react` sweep outside the agent subsystem (see discrepancy 7) — left for the plans that own those files.
  - `app/settings/page.tsx`'s own `ConnectorsSection`/`WebhooksSection` functions are still there, now dead weight once `SettingsTab.tsx` exists as the real home — deletion is plan 219's (§3.6, step 220.10).
  - `fetchAllRuns`'s O(agents × threads) composition cost (§4.10's own documented gap) — a real fix needs a farm-wide runs endpoint, out of scope here.

- **Open questions hit**: none blocked a step. §9 Q1 (`PaperPlaneRightIcon`'s real name) is answered above rather than left open. §9 Q2 (Agents in the rail vs. plugin menu) and §9 Q3 (a live approval was not available to test with no lab agent run) remain genuinely open/deferred, exactly as the plan itself expected.

- **Processes**: `ps -Ao pid=,command= | grep -i "[o]penpf"` shows two lines: this shell's own tool-invocation process, and a pre-existing `adb ... com.genymobile.scrcpy.Server` session against a real device (`ZP2222RMBS`) that was already running in this shared environment before this execution started — not started by this plan's work (no `bun run dev`, `dev:studio`, or any core/session command was ever run here; only `bun install`, `bun run typecheck`, `bun test <file>`, `bun run scripts/check-*.ts`, and `bun run build:studio`, each to completion and none left running).
