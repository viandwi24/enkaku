...
- Verbs: `run-script`, `run-workflow`, `install`, `push`, `pull`, `adb`, `wake`, `sleep`, `reconnect`, `disconnect`, `cutover`, `forget`, `block`, `unquarantine`, `set-network`, `apply-screen-label`, `clear-screen-label`, `set-group` (shown as "Move group"), `set-labels`, `prepare`, `retry-prepare`, `reprofile`, `screenshot`, `clear-cache`, `settings`. `apply-screen-label`/`clear-screen-label` were `set-label`/`clear-label` until plan 225: they write the device's number onto its lock screen or wallpaper (§16), and a verb one keystroke from `set-labels` with no behaviour in common is a mistake waiting to be typed. Every action menu renders the same rows in the same order, grouped into runs separated by a rule, and the bulk menu and Device Control's Actions tab render the identical grouping (`packages/studio/src/lib/generic-actions.ts` owns it; neither screen hardcodes a row). The runs are ordered by when an operator reaches for them, not by subsystem: **connection** (Reconnect, Disconnect, Network), **device** (Install apk, Adb command, Wake, Sleep), **run** (Run script, Run workflow), **files** (Upload file, Download file, Install guest agent, Uninstall guest agent), **config** (Settings, Move group, Screen label), **rare** (Screenshot, Prepare, Clear cache), **danger** (Forget). Amended 2026-09-05 by the owner, replacing the handoff's flat twelve-then-overflow list: every verb with a working dialog now has a row (`wake`, `pull`, `run-workflow`, `prepare`, `apply-screen-label`, `set-network` had none and were unreachable), and Screenshot moved out of the first run because it writes a PNG into the device's artifacts rather than capturing the browser, which is not a daily action. Plugins add verbs as `<plugin>/<verb>`.
