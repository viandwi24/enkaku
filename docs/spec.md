...
**A plugin may also contribute a screen, not only scripts** *(added directly by plan 108 §3.2–§3.9, §4.1–§4.5, 2026-08-17 — new product surface, documented in the same pass that shipped it, not a register entry)*. `PluginDefinition.surface` is an optional block declaring **nav** (up to 8 sidebar entries, each an id, a label, an icon from a closed allowlist, and the view it opens), **views** (up to 16 screens), and **actions** (up to 32). A plugin that omits it is unaffected in every way. The surface is **data, not code**: there is no expression language, no string interpolation, and no author-supplied JavaScript in the operator's session on the default path.

**Two tiers, and each one's cost is stated rather than implied.** *Tier A* — the default — is a declared view: a data source, a table, a toolbar, and row actions, rendered by Studio's own components. A plugin's screen is not "styled consistently"; it is drawn by the same `Table`, `SchemaForm`, and `ConfirmDialog` every other screen uses, so it inherits every later change to them for free. *Tier B* is a sandboxed iframe (`sandbox="allow-scripts"`, no `allow-same-origin`, a strict CSP with `connect-src 'none'`) over assets the plugin shipped, for a layout the vocabulary genuinely cannot say. A tier-B view inherits Studio's design tokens as CSS custom properties — colours, spacing, radius, typography — and **inherits none of its components**, because `Table` and `Button` are React in the parent document and do not cross a frame boundary. It also gets no ambient fetch, no cookie, and no token: it reaches the host only through a typed `postMessage` RPC that maps onto the same declared actions and data sources a tier-A view uses, so the frame changes the *rendering* and never the *authority*. A third option — loading plugin JavaScript into the Studio bundle itself — is refused permanently: Studio is a static export served same-origin by the core, so third-party code in that page could call any API as the operator.
