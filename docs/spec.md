...
- Verbs: `run-script`, `run-workflow`, `install`, `push`, `pull`, `adb`, `wake`, `sleep`, `reconnect`, `disconnect`, `cutover`, `forget`, `block`, `unquarantine`, `set-network`, `set-label`, `clear-label`, `set-group` (shown as "Move group"), `set-tags`, `prepare`, `retry-prepare`, `reprofile`, `screenshot`, `clear-cache`, `settings`. Every action menu renders the same rows in the same order, grouped into runs separated by a rule, and the bulk menu and Device Control's Actions tab render the identical grouping (`packages/studio/src/lib/generic-actions.ts` owns it; neither screen hardcodes a row). The runs are ordered by when an operator reaches for them, not by subsystem: **connection** (Reconnect, Disconnect, Network), **device** (Install apk, Adb command, Wake, Sleep), **run** (Run script, Run workflow), **files** (Upload file, Download file, Install guest agent, Uninstall guest agent), **config** (Settings, Move group, Label), **rare** (Screenshot, Prepare, Clear cache), **danger** (Forget). Amended 2026-09-05 by the owner, replacing the handoff's flat twelve-then-overflow list: every verb with a working dialog now has a row (`wake`, `pull`, `run-workflow`, `prepare`, `set-label`, `set-network` had none and were unreachable), and Screenshot moved out of the first run because it writes a PNG into the device's artifacts rather than capturing the browser, which is not a daily action. Plugins add verbs as `<plugin>/<verb>`.
