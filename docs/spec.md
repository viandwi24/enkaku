...
- Fields an operator sets: `groupId` (at most one group), labels (`device_labels`, any number — §4.2a), number (`device_numbers`, the `#` shown everywhere and on the physical label), label.
...
- Tables kept from the prototype: `devices`, `device_endpoints`, `device_numbers`, `discovered_devices`, `blocked_devices`, `deleted_devices`, `device_events` (the event log, two streams: `main` and `input`), `network_credentials`, `sequences`, `migration_markers`, `tool_installs`. `device_tags` was one of them until plan 225 replaced it with `labels`/`device_labels` (§4.2a).
...
A label is a named, coloured thing an operator creates once and puts on any number of devices; a device carries any number of labels. It is the many-to-many counterpart to a group, and the two answer different questions — "which rack is this phone in" is a group, "is this phone on the smoke pool AND on Android 15" is two labels. Tables `labels` (id, name, colour from a closed eight-name palette, description) and `device_labels`; routes `/api/labels` (create, rename, recolour, delete); membership is the `set-labels` action verb (`add | remove | replace`), never a route on the labels router, so a bulk assignment across a selection goes through the same target-shaped door every other bulk action does. Target shape `{ labelIds }`, AND semantics: a device must carry every label named. Deleting a label takes it off its devices and leaves them otherwise untouched, exactly as deleting a group does.

Labels REPLACE the free-form device tags of plan 19 (plan 225). A tag was a bare string on a join row with no life of its own: it existed only while some device carried it, so there was nothing to create empty, rename, recolour or delete farm-wide, and `smoke-pool` and `smokepool` were two unrelated tags with no way to notice. Migration `0080` turns every distinct tag into one label of the same name and rewrites the memberships. `TargetSchema` keeps a legacy read-only `{ tags }` member so batch rows, operations and workflow documents persisted under the old shape still resolve — by label name, which the migration made exact.

Studio: chips on the device row and the Screens tile, a multi-select label filter in the Devices toolbar (its own control beside the status filter, because that one is single-choice and labels are not), a manager for create/rename/recolour/delete behind it, and one assignment panel reached from the device context menu that acts on the whole selection. Across a mixed selection a label renders three states, not two: a row that only some of the selection carries shows a dash and its first click ADDS to everyone, because the alternative reading of that click silently removes the label from the devices that had it.
...
A workflow is a document (schema v2), owned by the farm, with no version of its own. Table `workflows`: `name` (unique), `doc`, `createdBy`, `updatedAt`, `pluginName`. Enqueuing a workflow job copies the validated document onto the job (`jobs.workflow_doc`), so editing never changes a queued or running job, and a later replay of that run draws on the document it actually took, not the workflow's current one (§4.8, plan 307 §3.2).

**Two owners: operators and plugins (plan 315).** An operator writes a workflow in Studio; its name may not contain `/`. A plugin may also *ship* workflow documents (`definePlugin({ workflows })`): static data in its bundle, validated at verify, registered as `<plugin>/<name>` with `pluginName` set when that plugin's version is activated, and kept equal to the active version's declaration through every rollback, enable, disable, removal and boot. A plugin workflow is read-only — `PUT`/`DELETE` refuse `E_WORKFLOW_MANAGED` — and an operator changes one by duplicating it into a workflow of their own. A plugin ships documents, never a generator: nothing emits a workflow at runtime, and a shipped document uses the same closed node set as any other.

**The document.** `{ schema: 2, name, title, description, params[], entry, nodes[], maxSteps, onFail? }`. Control flow lives on the nodes, not on array order: every node with a successor names it by an explicit **edge** (a target node id per outgoing case — `next`/`onFailure` for a script, `then`/`else` for a gate, `case:<i>`/`default` for a switch), and array position carries no meaning (plan 300 D1, plan 301). Every node carries `ui: { x, y }` — its authored canvas position, stored on the document rather than recomputed on open, so a hand-made layout survives a reload and a run replays on the arrangement it was authored on (plan 300 D2, plan 301, plan 307 §3.2). `entry` names the one `start` node a run begins at.

**Six node kinds.** `start` and `finish` bound the run (neither is ever logged as a step, plan 301 §3.2/§3.4); `script` runs a plugin member script as its own child job; `gate` and `switch` are core-owned branches (`checkWorkflow` proves reachability and budget over a closed edge vocabulary, plan 300 D8 — a plugin may never define control flow); `delay` waits, pinnable like a script node. A plugin contributes a node **type** as an optional `node?: {...}` descriptor on the script member it already has — control shape (outputs) and presentation (icon, category) only, never a new catalog or a second execution path: a plugin node runs in the same child process a script does, behind the same crash containment, never a sandbox (plan 300 D6, D7, plan 303). Its version is the script's own `ScriptRef` version, pinned on the node the same way a script node's is.

**Data flow is named outputs**, not item arrays: a node's result is recorded under its own id for the whole run, and a later node reads it through a value expression — `{ from: nodeId, path? }` for a plain lookup, or `{ expr: <source> }` for a pure, bounded computation over it (plan 300 D3, D4). An expression is parsed into a closed AST by `@enkaku/expr` — a closed grammar, no regular expressions, prototype-free evaluation scope, a hard step/depth budget, and no I/O — never `eval`, never `new Function`, never a library that emits JavaScript (plan 300 D4, plan 302). The Studio editor previews an expression's resolved value against the last real run while it is typed; production never re-derives a value the run itself did not compute.

**The scope roots are `$params $nodes $input $run $now $random $device.** `$device` (plan 314) is five read-only facts about the phone this run is acting on — `number`, `stableId`, `label`, `group`, `labels` — and `number` is `device_numbers.number`, the `#` on the phone's own physical label.

The distinction between `$device.number` and `$run.index` is load-bearing, not stylistic. `$run.index` is `jobs.batchSeq`, a position INSIDE one batch, and it exists to divide a fleet into equal shares. Two of its properties are deliberate features and both make it unusable as anything durable: `order: 'random'` reshuffles the numbering, and `createBatch` numbers only the devices that resolved as usable, so one offline phone shifts every device after it. So a rotation that must mean the same thing across several batches — a warm-up split into a morning, an afternoon and an evening session — keys on `$device.number`; (that particular rotation is plugin code since plan 900, and keys on the same field there for the same reason, but the rule is about any composition that outlives one batch, wherever it is written); splitting one batch into equal shares keys on `$run.index`; and indexing a workflow parameter by `$run.index` (the shape of "give each phone its own account") raises `W_WORKFLOW_INDEX_AS_IDENTITY`. A device with no number reads `null`, and arithmetic on it fails the step by name rather than rotating everything alike.

**Pins are authoring state, outside the document**: a node's last output can be pinned so downstream nodes reuse it without touching the device again while iterating — visibly marked on the canvas, substituted only for a manual or node-test run. A scheduled or batch run never reads a pin at all (production ignores pins by not looking, not by a flag; plan 300 R6, plan 304).

**Still true, and still the reason plan 308 exists rather than shipping now:** one device, sequential steps, one cursor per run — two edges into one node already merge, because there is one cursor, and the honest form of parallelism on a device farm is across devices, not across branches on one phone (MVP 05 §4, plan 300 D5; the CEO decision is recorded in `docs/mvp/README.md`, Open decisions 4). Changing it is a spec amendment, argued in plan 308, which starts `draft` on purpose.

**Simulate** runs a whole document with no device contact at all — 0 adb calls, 0 child processes, 0 session opens (plan 309). It is the same control-flow engine every real run uses (`checkWorkflow`, `workflow-resolve.ts`, the successor logic shared with the executor), with the acting half — a `script` node's device call — replaced by a value: a pin, a sample derived from the script's declared `resultSchema` (bounded, honouring `default`/`examples`/`enum`/`const`), or an author-written mock, in that precedence. A node that yields none of the three stops the simulation there, naming the node and why, rather than inventing `{}` and carrying on — a `{}` flowing into a gate would produce a branch decision the author trusts and the farm will not reproduce. A `delay` resolves instantly and records the duration it would have waited, so a workflow with a wait does not become a real wait. A simulation runs on the editor's own unsaved document, never a stored name, so an author can simulate the graph in front of them before publishing it. The result is stored — never returned synchronously — as an ordinary `job_runs` row with `trigger = 'simulate'`, replayed through the SAME canvas overlay a real run uses: excluded from the Jobs list unless asked for, never a workflow's or a script's "last run", never satisfying a schedule, and pruned on a much shorter retention horizon than a real run, because it is scratch work, not history.

### 4.7 Schedule

A schedule names a script or a workflow (`target: { kind: 'script', ref } | { kind: 'workflow', name }`), a device target (`{ groupId }` | `{ labelIds }` | `{ deviceIds }`, columns `group_id`/`label_ids`/`device_ids`, exactly one populated), and a cron. The target is re-resolved at EVERY firing, never frozen at the first: a device that joins the group or gains the label between two runs is in the next one. Labels (§4.2a) are the target shape that makes this worth stating — a device belongs to one group, so "every phone on the smoke pool AND on Android 15" had no expressible schedule target before plan 225. Every fire creates its OWN batch — one member job per usable target device, `jobs.schedule_id` set, first runs with `trigger = 'schedule'` — planned exactly like a manual `run-script` or `run-workflow` batch with the schedule's pacing (order, sequential sub-groups or waves, repetitions and their intervals, device stagger and per-device delay). Earlier fires' batches are never touched, and `schedules.batch_id` names the newest. `onOverlap` applies while the previous fire's batch still has a queued or running member: `skip` creates nothing and records `skipped-overlap` ("previous run still active"), `queue` creates the new batch anyway (its runs wait behind the previous batch's on each phone), and `cancel-previous` stops the previous batch first. The prototype's `schedule_runs` table is deleted by plan 211; `schedule_agent_targets` stays for agent schedules, and `schedule_workflow_targets` (plan 314) is its twin for workflow ones. Schedules are listed on the third tab of Scripts & Workflows.

**The work target is a companion row, never a column.** A schedule's kind is decided by which companion table holds a row for it — `schedule_agent_targets` first, then `schedule_workflow_targets`, and only if neither is `schedules.script_ref` read at all. A workflow target stores a NAME: the document is re-resolved at every firing and snapshotted onto each member job of that firing's own batch, so editing a workflow changes what the next firing runs and never what a queued or running one does.

**Per-device pacing.** Beside `jitterSec` (which shifts the WHOLE firing) and `deviceIntervalMs` (a fixed ladder, which therefore ranks the devices), a schedule carries `device_delay_min_ms`/`device_delay_max_ms` — an independent draw per member, "start them together but not at the same instant" (plan 314). It is the only one of the three that gives every device its own start time without ordering them, and hour-scale windows are sound because the pacer writes the draw into each run's `notBefore` column rather than holding a timer.
...
- **Rotation coverage** (`GET /api/workflows/:name/coverage`, plan 314) reads `workflow_steps.taken_edge` for a `switch` node — the branch a run recorded when it chose — and reports, per device, which branches it took and which it missed over its last N runs. Never re-derived from the document (that would reproduce today's numbering, not the run's) and never from a `simulate` run. It exists because a rotation spread across several dispatches fails by looking like success: every run green, no red batch, and one phone that took the same branch twice and a third one never.
...
- The wake sequence lights the screen and then gets the lock screen out of the way with `wm dismiss-keyguard`, falling back to `KEYCODE_MENU` only for a ROM too old to have that command. `KEYCODE_MENU` was the whole of that step until 2026-09-18 and has not moved a keyguard since Android 9, so a phone that reconnected sat on its lock screen until an operator swiped it by hand inside Device Control. A device the farm still cannot get past — a PIN, pattern or password, which it never types — reports readiness `blocked: 'locked'`: the screen is on and the farm is holding it awake, so this is the one blocked reason that does not mean `actual` is behind `desired`.
...
{
  "target": { "deviceIds": ["…"] } | { "groupId": "…" } | { "labelIds": ["…"] },
...
- Verbs: `run-script`, `run-workflow`, `install`, `push`, `pull`, `adb`, `wake`, `sleep`, `screen-off`, `screen-on`, `reconnect`, `disconnect`, `cutover`, `forget`, `block`, `unquarantine`, `set-network`, `apply-screen-label`, `clear-screen-label`, `set-group` (shown as "Move group"), `set-labels`, `prepare`, `retry-prepare`, `reprofile`, `screenshot`, `clear-cache`, `settings`. `apply-screen-label`/`clear-screen-label` were `set-label`/`clear-label` until plan 225: they write the device's number onto its lock screen or wallpaper (§16), and a verb one keystroke from `set-labels` with no behaviour in common is a mistake waiting to be typed. Every action menu renders the same rows in the same order, grouped into runs separated by a rule, and the bulk menu and Device Control's Actions tab render the identical grouping (`packages/studio/src/lib/generic-actions.ts` owns it; neither screen hardcodes a row). The runs are ordered by when an operator reaches for them, not by subsystem: **connection** (Reconnect, Disconnect, Network), **device** (Install apk, Adb command, Wake, Sleep, Screen off, Screen on), **run** (Run script, Run workflow), **files** (Upload file, Download file, Install guest agent, Uninstall guest agent), **config** (Settings, Move group, Screen label), **rare** (Screenshot, Prepare, Clear cache), **danger** (Forget). Amended 2026-09-05 by the owner, replacing the handoff's flat twelve-then-overflow list: every verb with a working dialog now has a row (`wake`, `pull`, `run-workflow`, `prepare`, `apply-screen-label`, `set-network` had none and were unreachable), and Screenshot moved out of the first run because it writes a PNG into the device's artifacts rather than capturing the browser, which is not a daily action. `screen-off`/`screen-on` are the device's PANEL and are not `sleep`/`wake` (plan 227 §3.3): they set a display power mode over the session's existing scrcpy control socket — two bytes, no adb, so a farm-wide screen-off costs the same at 3 devices and at 73 — and the mirror keeps running while the phone is dark. `sleep` puts Android itself to sleep, which is state Android owns and which therefore outlives this core; a display power mode does not. Verified against scrcpy v3.3.1: `Controller.setDisplayPower(false)` arms `CleanUp.setRestoreDisplayPower(true)`, so a panel darkened this way is powered back on when the session dies for any reason, an unplugged cable included. That is why they are two pairs of verbs rather than two speeds of one, and why darkening a farm you are about to unplug is `sleep`. A device with no session, or one mirroring without scrcpy, answers `skipped` — never `done`. Plugins add verbs as `<plugin>/<verb>`.
- Nine verbs still act on a device that is OFFLINE or QUARANTINED: `reconnect`, `cutover`, `forget`, `block`, `unquarantine`, `set-group`, `set-labels`, `set-network`, `settings`. Every other verb reports such a device `skipped` before dispatch. The table is `ACTION_OFFLINE_REACH` in `@enkaku/protocol`, and it is in the protocol rather than in the core because Studio reads the same rows to decide how many devices a dialog's footer button is about to act on — a second, private copy of the answer in `target/useTarget.ts` (every offline or quarantined device unusable, for every verb) disabled Move group, Settings, Network and Forget on an all-offline selection, and disabled Reconnect and Return from quarantine on precisely the devices those two exist for, while the core had been accepting all six the whole time (owner, 2026-09-18). `quarantine` is deliberately not on the list: the state machine's `QUARANTINE` transition only leaves `online`, so an offline device is not a failure there, just not a transition.
...
- **Jobs.** Tabs Jobs and Batches; a 268 px left list with wrapping filter chips (All · Running · Queued · Success · Failed) and 12 rows per page; the right detail with a run picker in the header meta line ("run 3 of 3 ·"), Re-run, Open device, Export, and sub-tabs Inputs, Output, Logs, Timeline, Artifacts. A batch's own detail opens on **Report** — success rate, first-pass rate, attempts per member, run and queue timings, failures grouped by message, and the dispatch settings the batch ran under — beside Inputs and Members; a member row peeks its inputs, output, logs and artifacts in place, and only the timeline, run comparison and export need the job's own page.
...
Advanced (12), with defaults: max concurrent adb commands 8; max concurrent installs 4, farm-wide; turn off Play Protect's adb-install check on; session build concurrency per USB root 4; infrastructure retries and backoff base 3, 1 s; job memory limit 256 MB; push / pull / bulk download caps 512 MB, 512 MB, 2 GB; install timeout 120 s; adb health probe interval 30 s; failures before quarantine 5; Screens bandwidth budget on WAN 20 Mbit; recovery resets per hour 6. The per-USB-root serialisation §17 requires is a constant (`INSTALL_PER_USB_ROOT = 1`), not this setting: raising max concurrent installs parallelises devices on different hubs and never two devices on the same one.

The Play Protect setting is the one advanced field that changes a device rather than the core. `adb install` is not exempt from Play Protect on a device with Google services: `verifier_verify_adb_installs` defaults to 1, so an install can stop on a modal until a human taps "Install anyway" — which the pinned openatx ui-server pair draws and the guest agent does not. On because a farm's phones exist to have the farm's own APKs pushed to them; it disables verification of adb installs only, never Play Protect's Play Store scan or its ongoing scan of what is already installed. It is applied per device by the `adb-verifier` preparation component, which reads the value back to prove it took and reports `failed` on that device when it did not.
