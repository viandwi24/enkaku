...
A workflow is a document (schema v2), owned by the farm, with no version of its own. Table `workflows`: `name` (unique), `doc`, `createdBy`, `updatedAt`. Enqueuing a workflow job copies the validated document onto the job (`jobs.workflow_doc`), so editing never changes a queued or running job, and a later replay of that run draws on the document it actually took, not the workflow's current one (§4.8, plan 307 §3.2).
