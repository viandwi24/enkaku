...
**When it does not come up, the session says so and falls back** (plan 129, from a field report on a 20-device Android 16 farm where `ui-server` never started at all). Two rules make that true rather than aspirational:

- **A start that never became ready is a failed start.** The watchdog pings until `startTimeoutMs`; if the server never answers, `start()` **throws** and the inspector is marked dead. It used to resolve regardless — so a dead `ui-server` was reported as a live one, `createInspectorForSession`'s long-standing `uiautomator-dump` fallback was unreachable, and every `find`/`dump` for the rest of the session failed instantly against a forwarded port with nothing behind it. The restart cycle is deliberately NOT spent on the start path: the circuit breaker's budget belongs to runtime recovery, and a second wait doubles the time before the working engine takes over.
- **A degraded session is visible.** `device.inspector.fallback` is broadcast, the device header carries a warning chip, and the Inspect tab itself names the engine in use and why — a dump tree read on the fallback engine must never look like a ui-server tree.

The cost is honest and stated: `uiautomator-dump` is the slower engine (334–584 ms per dump against `ui-server`'s ~80 ms). It works; that is the point of falling back to it.

A related diagnostics rule, learned the same way: **an error never names a timeout that did not happen.** A refused connection reports the elapsed time it really took; only a request genuinely aborted by its own deadline says `did not respond within Nms`. The previous wording stamped the timeout budget onto every failure, so a connection refused in 5 ms read as a 20-second timeout — and sent an investigation after the wrong cause.
...
**A tier-C module may also import `@enkaku/host` — Studio's own components** *(plan 129 §3.4)*. `@enkaku/ui` is the component library Studio is built from and is pure; `@enkaku/host` is narrower and different in kind: Studio components that only mean anything **inside Studio** — ones that talk to `/ws`, hold a lease, or decode video. Both arrive the same way, through the host-module table, so a plugin's import is the host's live instance and not a bundled copy.

The split matters, and it is not cosmetic. A pure component belongs in `@enkaku/ui` and moving one there is right — the shared device picker was moved out of Studio for exactly this reason, so that a plugin could obey the rule "every place that chooses a device uses this component". A component that reaches Studio's WebSocket singleton does **not** belong there: putting it in a framework-agnostic package would make that package lie about what it is. `@enkaku/host` is where those live, and its barrel being small is the enforcement — its first export is a device picker that shows a wall of **live tiles**, so an operator picks a phone by looking at its screen rather than by reading a name.

`@enkaku/host` is never published to npm and never resolves on disk. Two consequences an author meets: it must appear in the SDK's `UI_EXTERNALS` or the bundler will try to resolve it and fail the build, and its types come from a hand-written ambient declaration that **nothing checks against Studio's real barrel** — there is no shared package both sides import from, so a drift there cannot fail a build and can only be noticed.
...
**Reading a job's trace is a capability, and therefore an MCP tool too** *(plan 130 §3.1)*. `job.trace` returns one job's timeline — every device action with its redacted arguments, duration and outcome, beside the log lines, phase boundaries and artifacts (§12's `job_events`). `job.trace.ui` returns one stored UI tree as structured JSON; `job.trace.frame` returns **one** frame, named by its content hash, as a declared image. All three read the same store the REST routes serve, carry `permission: 'job.view'`, and are registered in the `automation` agent plugin — so the built-in agent and any external MCP client reach them through the one `invoke` door, with no second implementation and no second permission model.

**A frame is read one at a time, deliberately.** Events carry `frameHash`/`uiHash` — identifiers, never bytes. A capability that returned a whole run's frames would put megabytes of base64 into a context window for a single job, which is not debugging but a denial of service against the agent's own attention. The UI tree is the high-value one for an agent in any case: it is text, it is queryable, and it answers "what was on screen" directly. *(Known limit, stated rather than discovered: MCP's `tools/call` path does not convert a declared image into an image content block — it JSON-serialises the base64 as text, exactly as it already does for `device.screenshot`. That is a general MCP-surface gap, not specific to the trace.)*

**Durable API tokens** *(plan 130 §3.5)*. `Authorization: Bearer <token>` previously validated only a **session**, so an external agent had to hold a human's login. `api_tokens` (§12) is a table of hashed, named, revocable credentials with an optional expiry, resolved by the same middleware after the session lookup misses, on both `/api/*` and `/mcp`. A token always carries a `userId`: every existing permission check and every audit row is unchanged, it grants nothing its user does not already have, and it is not a second identity system. It can only ever be the caller's own — the create route takes no user id, so `user.manage` cannot mint a credential for somebody else. The plaintext is returned once, at creation; only a hash is stored, following the enrollment token's rule (and the session's hashing, since a token is checked on every request rather than redeemed once). REST: `GET/POST /api/tokens`, `DELETE /api/tokens/:id`, all `user.manage`.
