...
2. ~~Should the bound reset when the device reconnects, or persist across reconnects? Resetting is friendlier and risks a reconnect loop hiding a permanent failure.~~ **Answered by plan 90 §3.7, step 90.4 (2026-08-13).** Both, conditionally — not a straight choice between the two: the bound resets on a reconnect, but only when the reconnect is genuinely new information (`offlineAt > exhaustedAt` — the device actually went offline after the bound was last reached, not merely a heartbeat tick finding the route still down), and a second, coarser breaker (`guestAgent.maxRecoveryCyclesPerHour`, decaying over a rolling hour) stops that reset from firing forever against a device flapping on a genuinely dead proxy — exactly the risk this question named. Past the breaker, resets stop and the plain `guestAgent.recoveryRearmSec` re-arm clock (replacing the old `max(lastBackoff * 5, 60)` derivation, plan 90 F15) takes over instead, so a permanent failure still converges on bounded, visible retries rather than an infinite loop. `handleDeviceOffline` stamps `offlineAt`; `restoreDeviceRoute`'s enabled branch calls the new `resetRecoveryOnReconnect`; `POST /:id/network/retry` is the operator's honest, lease-gated equivalent of the disable/enable workaround (F17) for when neither condition has fired yet. Implemented and tested in `packages/core/src/api/guest-agent.ts`; the "physically replug a device" half of its own verifiable result is recorded as pending hardware in plan 90 §5 step 90.4.
