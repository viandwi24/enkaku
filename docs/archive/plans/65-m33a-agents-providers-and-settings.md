...
> Status: implemented — `packages/protocol/src/agent.ts` (`AgentSchema`, `ConnectorSchema`, `AgentSettingsSchema`/`AgentDefaultsSchema`, `resolveAgentConfig`, all fully `.describe()`d), `FarmSettings.agentDefaults` added to `packages/protocol/src/settings.ts`. Storage: `ai_agents` and `connectors` tables in `packages/core/src/db/schema.ts` (migrations `0026_absent_shadowcat.sql`, `0027_foamy_prism.sql`). `packages/core/src/secrets/store.ts` generalises `network/credential-store.ts`'s AES-256-GCM design into a namespaced store (`'network'`/`'connector'`, folded in as AEAD associated data so one namespace's ciphertext cannot decrypt under another) under ONE shared key file; `credential-store.ts` becomes a thin wrapper preserving its exact pre-plan-65 exports and error codes. `packages/core/src/agent/agent-store.ts` and `connector-store.ts` are the CRUD + write-time-validation layer (unknown capability/device/permission/path all refused by name); `packages/core/src/agent/provider/{types,anthropic,index}.ts` is the `ProviderAdapter` seam plus the Anthropic implementation (`@anthropic-ai/sdk@0.115.0`, the BETA messages resource for `fallbacks`/`output_config.effort`/adaptive `thinking`, always streamed) and a TTL model-list cache with a pinned fallback. `packages/core/src/api/agents.ts` and `api/connectors.ts` are the REST surface, wired into `daemon.ts`/`server/http.ts` at `/api/agents` and `/api/connectors`. Studio: `packages/studio/src/app/agents/page.tsx` (list) and `agents/detail/page.tsx` (the seven `SectionNav` sections — Identity/Model/Instructions/Tools/Access/Limits/Connectors, each override field showing its farm default with an Override/Reset affordance)plus a Connectors section added to `app/settings/page.tsx`. `bun run typecheck` and `bun run build:studio` are both green; `bun test` is 1787 pass / 0 fail (baseline 1698 + 89 new, all in this plan's own files) — one pre-existing, unrelated flaky test in `packages/adb/src/client.test.ts` (a timing race in `execStream`/`PerDeviceQueue`, in a file this plan never touched) intermittently fails independent of this work. No real network call is made anywhere in the test suite — every provider/connector test injects a fake `fetch`. **Deviations, recorded rather than silent:** (1) two new ACL permissions, `agent.view`/`agent.manage` (both in the operator set), were added to `packages/core/src/auth/acl.ts` — the plan's §4.5 assumes agent CRUD is permission-gated but never names which permission; reusing `settings.manage` would have made agent creation admin-only, contradicting §3.5's "an agent created by an operator" wording, so a new pair was the only reading that fit. `ALL_PERMISSIONS`/`isPermission` were added alongside for runtime validation of an agent's caller-supplied `permissions` list. (2) `connectors.credentialHint` is a column the plan's §4.1 schema does not list — computed once at write time (`secretHint()`) so `GET` never has to decrypt just to redact; this is an implementation choice within "write-only", not a deviation from it. (3) Plan 61 §3.3's tracked redirect (Studio's `/agents` → `/nodes`) is REMOVED here, ahead of its stated v0.1.7 target — `/agents` is the path this plan's own §4.6 names for the agent list, so the redirect and the real feature cannot coexist at the same route; `00-overview.md` §9's row is updated to record the early removal. (4) `resolveAgentConfig` is mirrored client-side as `resolveForDisplay` in `packages/studio/src/lib/agents.ts` (same `??`-based merge) purely so the editor can show inheritance without a round trip per keystroke — the server (`@enkaku/protocol`'s real `resolveAgentConfig`, called from `agent-store.ts`'s callers) remains the only place that decides what actually runs. (5) The model-list cache (`agent/provider/index.ts`) is in-memory per boot, not persisted — restarting the core re-fetches once; this matches "a TTL" without inventing a persistence requirement the plan never asked for. (6) `ProviderAdapter`'s `ProviderEvent`/`ProviderRequest` types live in `packages/core/src/agent/provider/types.ts`, not `@enkaku/protocol` — like capability 63's `CapabilityContext`, they type driver/session-adjacent concerns (`AsyncIterable`, provider tool defs) that protocol's zero-non-Zod-dependency rule cannot host, and nothing crosses the wire in this provider-internal shape. (7) An agent's authority is enforced today only at write time (`agent-store.ts`'s `validatePermissions`/`validateDeviceGrants`/`validateWorkspaceScope`) and via two pure, directly-tested functions (`effectivePermissions`, `agentCanReachDevice`) — no live `CapabilityContext` is built for an agent actor in this plan, matching §2's non-goal ("no LLM call happens in this plan"); Plan 66's loop is expected to call these same functions when it builds the context that actually invokes capabilities.
> Ships: packages/core/src/agent/agent-store.ts
